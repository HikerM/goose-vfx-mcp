...
export class LuminaClient {
  ...
  private ext: LuminaExtClient;
  private mcpPlatformExt: McpPlatformClient;

  constructor(
    toClient: () => LuminaClientCallbacks,
    ...
    const toAcpClient = () =>
      installLuminaExtAgentRequestDispatcher(
        installLuminaExtNotificationDispatcher(toClient()),
      );
    this.conn = new ClientSideConnection(toAcpClient, stream);
    this.ext = new LuminaExtClient(this.conn);
