...
test.describe('Performance Tests', () => {
  test('measure end-to-end performance for prompt submission', async ({ luminaPage }) => {
    // Start Playwright tracing to capture all performance data
    await luminaPage.context().tracing.start({
      screenshots: true,
      snapshots: true,
      sources: true
    });
    ...
    await luminaPage.evaluate(() => performance.mark('app-ready'));
    ...
    // Mark: Prompt submit
    await luminaPage.evaluate(() => performance.mark('prompt-submit-start'));
    await chatInput.press('Enter');
    await luminaPage.evaluate(() => performance.mark('prompt-submitted'));
    ...
    const loadingText = await luminaPage.locator('[data-testid="loading-indicator"]').textContent();
    if (loadingText?.includes('loading conversation')) {
      await luminaPage.evaluate(() => performance.mark('loading-conversation-start'));
      console.log('✓ Loading conversation detected');

      // Wait for it to change or disappear
      await luminaPage.waitForFunction(() => {
      ...
      await luminaPage.evaluate(() => performance.mark('loading-conversation-end'));
      console.log('✓ Loading conversation complete');
    }

    await luminaPage.evaluate(() => performance.mark('loading-started'));
    ...
    const checkForFirstToken = async () => {
      while (!firstTokenDetected) {
        try {
          const messageContainers = await luminaPage.locator('[data-testid="message-container"]').all();
          if (messageContainers.length > 0) {
            const lastMessage = messageContainers[messageContainers.length - 1];
            const content = await lastMessage.textContent();
            if (content && content.trim().length > 0) {
              await luminaPage.evaluate(() => performance.mark('first-token-received'));
        ...
        await luminaPage.waitForTimeout(50);
      }
    };
    ...
    await luminaPage.evaluate(() => performance.mark('response-complete'));
    console.log('✓ Response complete');

    // Create performance measures
    await luminaPage.evaluate(() => {
    ...
    const metrics = await luminaPage.evaluate(() => {
    ...
    const response = await luminaPage.locator('[data-testid="message-container"]').last();
    ...
    await luminaPage.context().tracing.stop({ path: tracePath });
  ...
  test('measure cold start vs warm cache performance', async ({ luminaPage }) => {
    await luminaPage.context().tracing.start({ screenshots: true, snapshots: true });
    ...
    await luminaPage.evaluate(() => performance.mark('cold-prompt-start'));
    await chatInput.press('Enter');
    ...
    // Second prompt (warm)
    const chatInput2 = await luminaPage.waitForSelector('[data-testid="chat-input"]');
    await chatInput2.fill('Say goodbye');

    await luminaPage.evaluate(() => performance.mark('warm-prompt-start'));
  ...
  test('capture full performance profile with navigation timing', async ({ luminaPage }) => {
    await luminaPage.context().tracing.start({ screenshots: true, snapshots: true });

    console.log('\n=== Full Performance Profile ===\n');

    // Get navigation timing
    const navigationTiming = await luminaPage.evaluate(() => {
    ...
    // Wait for app ready
    await luminaPage.waitForSelector('[data-testid="chat-input"]', { timeout: 30000 });
    await luminaPage.evaluate(() => performance.mark('app-interactive'));

    // Measure time from navigation to interactive
    const appReadyTime = await luminaPage.evaluate(() => {
    ...
    await luminaPage.evaluate(() => performance.mark('user-interaction-start'));
    await chatInput.press('Enter');

    await luminaPage.waitForSelector('[data-testid="loading-indicator"]', { state: 'visible', timeout: 5000 });
    await luminaPage.waitForSelector('[data-testid="loading-indicator"]', { state: 'hidden', timeout: 60000 });
    ...
    const interactionTime = await luminaPage.evaluate(() => {
    ...
    const resourceStats = await luminaPage.evaluate(() => {
