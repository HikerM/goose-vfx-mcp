...
export interface AcpSessionNotificationAdapter {
  ...
  applyLumina(notification: LuminaSessionNotification_unstable): AcpChatStateChange[];
...
export function createAcpSessionNotificationAdapter(
  ...
  return {
    ...
    applyLumina(notification) {
      return applyLuminaSessionNotification(state, notification);
    },
...
function applyAcpSessionNotification(
  ...
  switch (update.sessionUpdate) {
    ...
    case 'session_info_update': {
      const activeRunId = getLuminaActiveRunId(update);
