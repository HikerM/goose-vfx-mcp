...
describe('httpBaseFromAcpWebSocketUrl', () => {
  ...
  it('preserves path prefixes before the ACP endpoint', () => {
    expect(httpBaseFromAcpWebSocketUrl('wss://example.com/lumina/acp?token=secret')).toBe(
...
describe('normalizeAcpHttpBaseUrl', () => {
  ...
  it('normalizes prefixed HTTPS base URLs', () => {
    expect(normalizeAcpHttpBaseUrl('https://example.com/lumina/')).toBe('https://example.com/lumina');
  });
...
describe('HTTP endpoint URLs from ACP HTTP base URLs', () => {
  it('builds status URLs from root and prefixed bases', () => {
    ...
    expect(statusHttpUrlFromHttpBase('https://example.com/lumina/')).toBe(
      'https://example.com/lumina/status'
    );
  });
  ...
  it('builds ACP URLs from root and prefixed bases', () => {
    ...
    expect(acpHttpUrlFromHttpBase('https://example.com/lumina/')).toBe(
      'https://example.com/lumina/acp'
    );
  });

  it('adds ACP query tokens when provided', () => {
    expect(acpHttpUrlFromHttpBase('https://example.com/lumina', 'test secret')).toBe(
      'https://example.com/lumina/acp?token=test+secret'
    );
  });
});
...
describe('acpWebSocketUrlFromHttpBase', () => {
  it('derives WSS ACP URLs from HTTPS base URLs', () => {
    expect(acpWebSocketUrlFromHttpBase('https://example.com/lumina', 'secret')).toBe(
      'wss://example.com/lumina/acp?token=secret'
    );
  });
