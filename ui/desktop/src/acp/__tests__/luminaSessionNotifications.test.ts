...
import { applyLuminaSessionNotification } from '../adapter/luminaSessionNotifications';
...
describe('applyLuminaSessionNotification', () => {
  describe('message_usage', () => {
    it('attaches usage to the message matching messageId', () => {
      const state = makeState();

      const changes = applyLuminaSessionNotification(state, messageUsageNotification('a1'));
    ...
    it.each([
      ...
      const changes = applyLuminaSessionNotification(state, messageUsageNotification(messageId));
    ...
    it('skips client-made approval rows when falling back', () => {
      ...
      const changes = applyLuminaSessionNotification(state, messageUsageNotification('missing'));
  ...
  describe('usage_update', () => {
    it('still maps usage updates into token state', () => {
      const changes = applyLuminaSessionNotification(
        makeState(),
        luminaUpdate({
