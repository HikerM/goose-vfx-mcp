...
import { buildLocalServeUrls, findLuminaBinaryPath, startLuminaServe } from './luminaServe';
...
function makeTempDir(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lumina-serve-test-'));
...
describe('findLuminaBinaryPath', () => {
  ...
  it('uses LUMINA_BINARY in development builds', () => {
    const tempDir = makeTempDir();
    const overridePath = makeFile(path.join(tempDir, 'override-lumina'));
    vi.stubEnv('LUMINA_BINARY', overridePath);

    expect(findLuminaBinaryPath({ isPackaged: false })).toBe(overridePath);
  });

  it('rejects LUMINA_BINARY in packaged builds', () => {
    const tempDir = makeTempDir();
    ...
    expect(() => findLuminaBinaryPath({ isPackaged: true, resourcesPath })).toThrow(
      'LUMINA_BINARY is only supported in development builds'
    );
  });
  ...
  it('prefers the staged binary over target builds in development builds', () => {
    ...
    const resolvedPath = findLuminaBinaryPath({ isPackaged: false });
  ...
  it('uses the bundled lumina binary in packaged builds', () => {
    const tempDir = makeTempDir();
    const resourcesPath = path.join(tempDir, 'resources');
    const bundledPath = makeFile(path.join(resourcesPath, 'bin', binaryName));

    expect(findLuminaBinaryPath({ isPackaged: true, resourcesPath })).toBe(bundledPath);
  });
});
...
describe('startLuminaServe', () => {
  ...
  it.skipIf(process.platform === 'win32')('uses the injected readiness fetch', async () => {
    const tempDir = makeTempDir();
    const luminaPath = makeExecutable(
      path.join(tempDir, 'lumina'),
      '#!/usr/bin/env sh\nwhile true; do sleep 1; done\n'
    );
    vi.stubEnv('LUMINA_BINARY', luminaPath);
    ...
    const result = await startLuminaServe({
  ...
  it.skipIf(process.platform === 'win32')('captures the TLS fingerprint from stdout', async () => {
    ...
    const luminaPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'printf "LUMINAD_CERT_FINGERPRINT=AA:BB:CC\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
  ...
  it.skipIf(process.platform === 'win32')('uses TLS URLs and args when TLS is enabled', async () => {
    ...
    const luminaPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'printf "%s\\n" "$@" > "$TEST_ARGS_PATH"',
        'printf "LUMINAD_CERT_FINGERPRINT=DD:EE:FF\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
  ...
  it.skipIf(process.platform === 'win32')('waits for TLS fingerprint after readiness succeeds', async () => {
    ...
    const luminaPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'sleep 0.1',
        'printf "LUMINAD_CERT_FINGERPRINT=11:22:33\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
