...
export const ModelAndProviderProvider: React.FC<ModelAndProviderProviderProps> = ({ children }) => {
  ...
  const getFallbackModelAndProvider = useCallback(async () => {
    const provider = window.appConfig.get('LUMINA_DEFAULT_PROVIDER') as string;
    const model = window.appConfig.get('LUMINA_DEFAULT_MODEL') as string;
    if (provider && model) {
      try {
  ...
  const getCurrentModelAndProviderForDisplay = useCallback(async () => {
    ...
    const luminaModel = modelProvider.model;
    const luminaProvider = modelProvider.provider;

    // lookup display name
    let metadata: ProviderMetadata;

    try {
      metadata = await getProviderMetadata(String(luminaProvider));
    } catch {
      return { model: luminaModel, provider: luminaProvider };
    }
    const providerDisplayName = metadata.display_name;

    return { model: luminaModel, provider: providerDisplayName };
