...
import { all_lumina_modes, ModeSelectionItem } from './ModeSelectionItem';
...
export const ModeSection = () => {
  ...
  const handleModeChange = async (newMode: string) => {
    try {
      await upsert('LUMINA_MODE', newMode, false);
      setCurrentMode(newMode);
    } catch (error) {
      console.error('Error updating lumina mode:', error);
      throw new Error(`Failed to store new lumina mode: ${newMode}`);
    }
  };

  useEffect(() => {
    const mode = config.LUMINA_MODE as string | undefined;
    if (mode) {
      setCurrentMode(mode);
    }
  }, [config.LUMINA_MODE]);
  ...
  const fetchMaxTurns = useCallback(async () => {
    try {
      const turns = (await read('LUMINA_MAX_TURNS', false)) as number;
      if (turns) {
        setMaxTurns(turns);
      }
    } catch (error) {
  ...
  const handleMaxTurnsChange = async (value: number) => {
    try {
      await upsert('LUMINA_MAX_TURNS', value, false);
      setMaxTurns(value);
    } catch (error) {
