...
if [ -n "${LUMINA_PATH_ROOT:-}" ]; then
    RESOLVED_LUMINA_CONFIG_DIR="${LUMINA_PATH_ROOT}/config"
elif [ -n "${LUMINA_CONFIG_DIR:-}" ]; then
    log "LUMINA_CONFIG_DIR is deprecated for desktop shims; prefer LUMINA_PATH_ROOT."
    RESOLVED_LUMINA_CONFIG_DIR="${LUMINA_CONFIG_DIR}"
else
    RESOLVED_LUMINA_CONFIG_DIR="${HOME}/.config/lumina"
fi
MCP_HERMIT_DIR="${RESOLVED_LUMINA_CONFIG_DIR}/mcp-hermit"
mkdir -p "${RESOLVED_LUMINA_CONFIG_DIR}"
HERMIT_SETUP_LOCK_DIR="${RESOLVED_LUMINA_CONFIG_DIR}/.mcp-hermit-setup.lock"
...
CLEANUP_MARKER="${RESOLVED_LUMINA_CONFIG_DIR}/.mcp-hermit-cleanup-v1"
...
log "Checking for LUMINA_NPM_REGISTRY and LUMINA_NPM_CERT environment variables for custom npm registry setup..."
# Check if LUMINA_NPM_REGISTRY is set and accessible
if [ -n "${LUMINA_NPM_REGISTRY:-}" ] && curl -s --head --fail "${LUMINA_NPM_REGISTRY}" > /dev/null; then
    log "Checking custom lumina registry availability: ${LUMINA_NPM_REGISTRY}"
    log "${LUMINA_NPM_REGISTRY} is accessible. Using it for npm registry."
    export NPM_CONFIG_REGISTRY="${LUMINA_NPM_REGISTRY}"

    # Check if LUMINA_NPM_CERT is set and accessible
    if [ -n "${LUMINA_NPM_CERT:-}" ] && curl -s --head --fail "${LUMINA_NPM_CERT}" > /dev/null; then
        log "Downloading certificate from: ${LUMINA_NPM_CERT}"
        curl -sSL -o "${MCP_HERMIT_DIR}/cert.pem" "${LUMINA_NPM_CERT}"
        if [ $? -eq 0 ]; then
        ...
        log "LUMINA_NPM_CERT is either not set or not accessible. Skipping certificate setup."
    fi

else
    log "LUMINA_NPM_REGISTRY is either not set or not accessible. Falling back to default npm registry."
