...
function Assert-Runtime([string]$Node) {
  Assert-RegularTree $env:GOOSE_PATH_ROOT
...
public static class GooseCommandLine {
...
function Read-Arguments([string]$CommandLine) {
  ...
  try {
    $count = 0; $argv = [GooseCommandLine]::CommandLineToArgvW($ptr, [ref]$count)
    ...
    finally { [GooseCommandLine]::LocalFree($argv) | Out-Null }
...
if ($index -ge 0) {
  ...
  $forwarded = if ($tail) { @(Read-Arguments "goose.exe $tail" | Select-Object -Skip 1) } else { @() }
  if ($tail -and @($forwarded).Count -eq 0) { throw "Unable to restore npx arguments from parent command line: $tail" }
}

$root = if ([string]::IsNullOrWhiteSpace($env:GOOSE_PATH_ROOT)) { 'D:\Goose' } else { $env:GOOSE_PATH_ROOT }
$root = [IO.Path]::GetFullPath($root).TrimEnd('\')
if ($root -notmatch '^D:\\[^.].*$' -or $root.StartsWith('\\') -or $root -match '[*?"<>|]') { throw 'GOOSE_PATH_ROOT must be a local D: drive folder' }
$env:GOOSE_PATH_ROOT = $root
$nodeDir = Join-Path $root 'runtime\node'
$env:GOOSE_NODE_DIR = $nodeDir
