...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `lumina://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('lumina://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: lumina://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('lumina://recipe?config=', '');
