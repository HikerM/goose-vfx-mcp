...
describe.skipIf(process.platform !== 'win32')('real Windows shim deployment', () => {
  it('publishes the authored npx pair under D:\\Lumina\\bin', async () => {
    ...
    expect(result?.shimDirectory.toLowerCase()).toBe('d:\\lumina\\bin');
    expect(result?.env.PATH?.toLowerCase()).toContain('d:\\lumina\\bin');
  });
});
