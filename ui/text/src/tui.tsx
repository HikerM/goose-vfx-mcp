...
import { resolveLuminaBinary } from "@hikerm/lumina-sdk/node";
      ...
      {queued && (
        <Box>
          <Text color={GOLD} dimColor italic>
            message queued — will send when lumina finishes
          </Text>
        </Box>
      )}
    </Box>
...
const SplashScreen = React.memo(function SplashScreen({
  ...
  const frame = LUMINA_FRAMES[animFrame % LUMINA_FRAMES.length]!;
  const statusColor =
...
function App({
  ...
  const [luminaFrame, setLuminaFrame] = useState(0);
  ...
  const clientRef = useRef<LuminaClient | null>(null);
  ...
  // the splash lumina while the banner is up, or the spinner while loading.
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      if (bannerVisible) setLuminaFrame((f) => (f + 1) % LUMINA_FRAMES.length);
    }, 300);
    ...
    (async () => {
      try {
        ...
        await client.initialize({
          ...
          clientInfo: { name: "lumina-text", version: "0.1.0" },
          clientCapabilities: {},
        });
        if (cancelled) return;
        ...
        try {
          const resp = await client.lumina.defaultsRead_unstable({});
          hasProvider =
...
async function main() {
  ...
  if (cli.flags.server) {
    ...
    const binary = resolveLuminaBinary();
    ...
    serverProcess.on("error", (err) => {
      console.error(`Failed to start lumina acp: ${err.message}`);
      process.exit(1);
    });
