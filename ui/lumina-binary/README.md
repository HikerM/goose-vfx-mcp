# Native Binary Packages for lumina

This directory contains the npm package scaffolding for distributing the
`lumina` Rust binary as platform-specific npm packages.
...
The built binaries are placed into `ui/lumina-binary/lumina-binary-{platform}/bin/`.
...
This will publish all native packages along with `@hikerm/lumina-sdk` and `@hikerm/lumina`.

## Usage

These packages are installed as optional dependencies by `@hikerm/lumina` (the TUI).
