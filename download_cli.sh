...
##############################################################################
# lumina CLI Install Script
#
# This script downloads the latest stable 'lumina' CLI binary from GitHub releases
...
#   LUMINA_BIN_DIR  - Directory to which lumina will be installed (default: $HOME/.local/bin)
#   LUMINA_VERSION  - Optional: specific version to install (e.g., "v1.0.25"). Overrides CANARY. Can be in the format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z
#   LUMINA_PROVIDER - Optional: provider for lumina
#   LUMINA_MODEL    - Optional: model for lumina
#   LUMINA_LINUX_VARIANT - Optional: Linux package variant to install (`standard`, `vulkan`, or `musl`)
#   LUMINA_WINDOWS_VARIANT - Optional: Windows package variant to install (`standard` or `cuda`)
#   CANARY         - Optional: if set to "true", downloads from canary release instead of stable
#   CONFIGURE      - Optional: if set to "false", disables running lumina configure interactively
...
if ! command -v curl >/dev/null 2>&1; then
  echo "Error: 'curl' is required to download lumina. Please install curl and try again."
  exit 1
fi

# Check for tar or unzip (depending on OS)
if ! command -v tar >/dev/null 2>&1 && ! command -v unzip >/dev/null 2>&1; then
  echo "Error: Either 'tar' or 'unzip' is required to extract lumina. Please install one and try again."
  exit 1
fi
...
if [[ "${WINDIR:-}" ]] || [[ "${windir:-}" ]] || [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
    ...
    DEFAULT_BIN_DIR="$USERPROFILE/lumina"
else
    # Linux, macOS, and WSL all use the same bin directory
    DEFAULT_BIN_DIR="$HOME/.local/bin"
fi

LUMINA_BIN_DIR="${LUMINA_BIN_DIR:-$DEFAULT_BIN_DIR}"
RELEASE="${CANARY:-false}"
CONFIGURE="${CONFIGURE:-true}"
LUMINA_LINUX_VARIANT="${LUMINA_LINUX_VARIANT:-}"
LUMINA_WINDOWS_VARIANT="${LUMINA_WINDOWS_VARIANT:-standard}"
if [ -n "${LUMINA_VERSION:-}" ]; then
  # Validate the version format
  if [[ ! "$LUMINA_VERSION" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+(-.*)?$ ]]; then
    echo "[error]: invalid version '$LUMINA_VERSION'."
    echo "  expected: semver format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z"
    exit 1
  fi
  LUMINA_VERSION=$(echo "$LUMINA_VERSION" | sed 's/^v\{0,1\}/v/') # Ensure the version string is prefixed with 'v' if not already present
  RELEASE_TAG="$LUMINA_VERSION"
else
  # If LUMINA_VERSION is not set, fall back to existing behavior for backwards compatibility
...
case "$OS" in
  ...
  mingw*|msys*|cygwin*)
    ...
    ;;
  *)
    echo "Error: Unsupported OS '$OS'. lumina currently supports Linux, macOS, and Windows."
    exit 1
    ;;
esac
...
if [ "$OS" = "linux" ] && [ -z "$LUMINA_LINUX_VARIANT" ]; then
  if detect_linux_musl; then
    LUMINA_LINUX_VARIANT="musl"
  else
    LUMINA_LINUX_VARIANT="standard"
  fi
elif [ -z "$LUMINA_LINUX_VARIANT" ]; then
  LUMINA_LINUX_VARIANT="standard"
fi
...
if [ "$OS" = "darwin" ]; then
  FILE="lumina-$ARCH-apple-darwin.tar.bz2"
  EXTRACT_CMD="tar"
elif [ "$OS" = "windows" ]; then
  case "$LUMINA_WINDOWS_VARIANT" in
    standard|cuda) ;;
    *)
      echo "Error: Unsupported LUMINA_WINDOWS_VARIANT '$LUMINA_WINDOWS_VARIANT'. Expected 'standard' or 'cuda'."
      exit 1
      ;;
  esac
  ...
  FILE="lumina-$ARCH-pc-windows-msvc.zip"
  if [ "$LUMINA_WINDOWS_VARIANT" = "cuda" ]; then
    FILE="lumina-$ARCH-pc-windows-msvc-cuda.zip"
  fi
  EXTRACT_CMD="unzip"
  OUT_FILE="lumina.exe"
else
  case "$LUMINA_LINUX_VARIANT" in
    standard|vulkan|musl) ;;
    *)
      echo "Error: Unsupported LUMINA_LINUX_VARIANT '$LUMINA_LINUX_VARIANT'. Expected 'standard', 'vulkan', or 'musl'."
      exit 1
      ;;
  esac
  FILE="lumina-$ARCH-unknown-linux-gnu.tar.bz2"
  if [ "$LUMINA_LINUX_VARIANT" = "vulkan" ]; then
    FILE="lumina-$ARCH-unknown-linux-gnu-vulkan.tar.bz2"
  elif [ "$LUMINA_LINUX_VARIANT" = "musl" ]; then
    FILE="lumina-$ARCH-unknown-linux-musl.tar.bz2"
  fi
  EXTRACT_CMD="tar"
fi
...
# --- 4) Download & extract 'lumina' binary ---
...
if ! curl -sLf "$DOWNLOAD_URL" --output "$FILE"; then
  ...
  if ! [ -n "${LUMINA_VERSION:-}" ] && [ "${CANARY:-false}" != "true" ]; then
    LATEST_TAG=$(curl -s https://api.github.com/repos/HikerM/lumina/releases/latest | \
...
TMP_DIR="/tmp/lumina_install_$RANDOM"
...
# Windows releases may contain files in a 'lumina-package' subdirectory
EXTRACT_DIR="$TMP_DIR"
if [ "$OS" = "windows" ] && [ -d "$TMP_DIR/lumina-package" ]; then
  echo "Found lumina-package subdirectory, using that as extraction directory"
  EXTRACT_DIR="$TMP_DIR/lumina-package"
fi

# Make binary executable
if [ "$OS" = "windows" ]; then
  chmod +x "$EXTRACT_DIR/lumina.exe"
else
  chmod +x "$EXTRACT_DIR/lumina"
fi

# --- 5) Install to $LUMINA_BIN_DIR ---
if [ ! -d "$LUMINA_BIN_DIR" ]; then
  echo "Creating directory: $LUMINA_BIN_DIR"
  mkdir -p "$LUMINA_BIN_DIR"
fi

echo "Moving lumina to $LUMINA_BIN_DIR/$OUT_FILE"
if [ "$OS" = "windows" ]; then
  mv "$EXTRACT_DIR/lumina.exe" "$LUMINA_BIN_DIR/$OUT_FILE"
else
  ...
  if [ -f "$LUMINA_BIN_DIR/$OUT_FILE" ]; then
    mv "$LUMINA_BIN_DIR/$OUT_FILE" "$LUMINA_BIN_DIR/$OUT_FILE.old"
    if ! mv "$EXTRACT_DIR/lumina" "$LUMINA_BIN_DIR/$OUT_FILE"; then
      echo "Error: failed to install new binary, restoring previous version"
      mv "$LUMINA_BIN_DIR/$OUT_FILE.old" "$LUMINA_BIN_DIR/$OUT_FILE"
      exit 1
    fi
    rm -f "$LUMINA_BIN_DIR/$OUT_FILE.old"
  else
    mv "$EXTRACT_DIR/lumina" "$LUMINA_BIN_DIR/$OUT_FILE"
  fi
fi
...
if [ "$CONFIGURE" = true ]; then
  # --- 6) Configure lumina (Optional) ---
  echo ""
  echo "Configuring lumina"
  echo ""
  if [ -t 0 ]; then
    "$LUMINA_BIN_DIR/$OUT_FILE" configure
  elif [ -r /dev/tty ]; then
    "$LUMINA_BIN_DIR/$OUT_FILE" configure < /dev/tty
  else
    echo "Non-interactive shell detected (e.g. 'curl ... | bash')."
    echo "Skipping 'lumina configure' — please run it manually after installation:"
    echo "    $LUMINA_BIN_DIR/$OUT_FILE configure"
  fi
else
  echo "Skipping 'lumina configure', you may need to run this manually later"
fi



# --- 7) Check PATH and give instructions if needed ---
if [[ ":$PATH:" != *":$LUMINA_BIN_DIR:"* ]]; then
  echo ""
  echo "Warning: lumina installed, but $LUMINA_BIN_DIR is not in your PATH."

  if [ "$OS" = "windows" ]; then
    echo "To add lumina to your PATH in PowerShell:"
    echo ""
    ...
    echo "    lumina configure"
    echo "or rerun this install script after updating your PATH."
  else
    SHELL_NAME=$(basename "$SHELL")

    echo ""
    echo "The \$LUMINA_BIN_DIR is not in your PATH."
    ...
    if [ "$CONFIGURE" = true ]; then
      ...
      1)
        ...
        echo "Adding \$LUMINA_BIN_DIR to $RC_FILE..."
        echo "export PATH=\"$LUMINA_BIN_DIR:\$PATH\"" >> "$RC_FILE"
      ...
      2)
        ...
        echo "    export PATH=\"$LUMINA_BIN_DIR:\$PATH\""
        echo "Then reload your shell (e.g. 'source ~/.${SHELL_NAME}rc') to apply changes."
        ;;
      *)
        echo "Invalid choice. Please add \$LUMINA_BIN_DIR to your PATH manually."
        ;;
      esac
    else
      echo ""
      echo "Configure disabled. Please add \$LUMINA_BIN_DIR to your PATH manually."
    fi
