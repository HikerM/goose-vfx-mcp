...
LUMINA_PORT = "3001"
LUMINA_URL = f"http://{LUMINA_HOST}:{LUMINA_PORT}"
...
async def setup_agent() -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{LUMINA_URL}/agent/update_provider",
            json={"provider": "databricks", "model": "lumina"},
...
def submit_tool_result(tool_id: str, result: List[Dict[str, Any]]) -> None:
    """Submit the tool execution result back to Lumina.
...
async def chat_loop() -> None:
    """Main chat loop that handles the conversation with Lumina."""
