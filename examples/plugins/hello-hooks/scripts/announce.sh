#!/usr/bin/env bash
# Tiny hook handler that loudly announces every event lumina fires at it.
#
# Lumina pipes the event payload as JSON on stdin and sets PLUGIN_ROOT in the
...
# Both stderr (so it shows up alongside lumina tracing) and a log file.
