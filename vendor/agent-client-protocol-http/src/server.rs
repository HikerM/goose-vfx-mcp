...
pub(crate) struct ServerState {
    ...
    cors: CorsOptions,
    pub(crate) raw_json_validator: Option<RawJsonValidator>,
}

pub(crate) type RawJsonValidator = Arc<dyn Fn(&[u8]) -> bool + Send + Sync>;
...
impl AcpHttpServer {
    ...
    /// Reject incoming JSON before it is deserialized by the HTTP or WebSocket transport.
    ///
    /// Returning `false` rejects the complete HTTP body or WebSocket text frame without
    /// exposing the input in the error response.
    #[must_use]
    pub fn with_raw_json_validator(
        mut self,
        validator: impl Fn(&[u8]) -> bool + Send + Sync + 'static,
    ) -> Self {
        self.raw_json_validator = Some(Arc::new(validator));
        self
    }
    ...
    pub fn into_router(self) -> Router {
        ...
        let state = ServerState::new(registry.clone(), cors.clone(), self.raw_json_validator);

        let mut router = Router::new()
            .route(
                &path,
                post(crate::http_server::handle_post).with_state(state.clone()),
            )
...
async fn handle_get(
    ...
    match ws_upgrade {
        Ok(ws) => {
            ...
            crate::websocket_server::handle_ws_upgrade(
                state.registry,
                state.raw_json_validator,
                ws,
            )
        }
