...
impl std::fmt::Debug for ResponseRoute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::Connection => "ResponseRoute::Connection",
            Self::Session(_) => "ResponseRoute::Session",
        })
    }
}
...
impl OutboundTransport {
    ...
    async fn route_outbound(&self, msg: RawJsonRpcMessage) {
        let serialized = match serde_json::to_string(&msg) {
            Ok(s) => s,
            Err(_) => {
                error!("ACP outbound JSON-RPC serialization failed");
                return;
            }
        };
    ...
    async fn route_outbound(&self, msg: &RawJsonRpcMessage, serialized: String) {
        ...
        match route {
            ResponseRoute::Connection => {
                trace!(scope = "connection", "ACP outbound message routed");
                self.connection_stream.push(serialized).await;
            }
            ResponseRoute::Session(sid) => {
                trace!(scope = "session", "ACP outbound message routed");
...
impl ConnectionRegistry {
    ...
    async fn create_connection_with_transport(
        ...
        let outbound = async move {
            while let Some(msg) = agent_rx.next().await {
                match msg {
                    ...
                    Err(_) => {
                        error!("ACP agent emitted an error");
        ...
        let agent_handle = tokio::spawn(async move {
            ...
            debug!("ACP connection task ended");
...
mod tests {
    ...
    #[test]
    fn response_route_debug_omits_session_id() {
        const SECRET: &str = "acp-response-route-secret";

        let debug = format!("{:?}", ResponseRoute::Session(SECRET.to_string()));

        assert_eq!(debug, "ResponseRoute::Session");
