            ...
            async |dispatch, connection| {
                match dispatch {
                    Dispatch::Request(message, responder) => {
                        tracing::debug!(event = "acp_protocol_request_handler");
                        if Req::matches_method(&message.method) {
                            match Req::parse_message(&message.method, &message.params) {
                                Ok(req) => {
                                    tracing::trace!(event = "acp_protocol_request_handler_parsed");
                                ...
                                Err(err) => {
                                    tracing::trace!(
                                        event = "acp_protocol_request_handler_parse_failed"
                                    );
                                    Err(err)
                                }
                            }
                        } else {
                            tracing::trace!(event = "acp_protocol_request_handler_method_mismatch");
                            Ok(Handled::No {
            ...
            async |dispatch, connection| {
                match dispatch {
                    Dispatch::Notification(message) => {
                        tracing::debug!(event = "acp_protocol_notification_handler");
                        if Notif::matches_method(&message.method) {
                            match Notif::parse_message(&message.method, &message.params) {
                                Ok(notif) => {
                                    tracing::trace!(
                                        event = "acp_protocol_notification_handler_parsed"
                                    );
                                    let result =
                                ...
                                Err(err) => {
                                    tracing::trace!(
                                        event = "acp_protocol_notification_handler_parse_failed"
                                    );
                                    Err(err)
                                }
                            }
                            ...
                            tracing::trace!(
                                event = "acp_protocol_notification_handler_method_mismatch"
                            );
                            Ok(Handled::No {
