...
pub(crate) fn send_raw_message(
    ...
    tracing::debug!(event = "acp_protocol_send_queued");
...
pub(super) async fn outgoing_protocol_actor(
    ...
    while let Some(message) = outgoing_rx.next().await {
        tracing::debug!(event = "acp_protocol_outgoing_message");
        ...
        let json_rpc_message = match message {
            OutgoingMessage::Request {
                    ...
                    .and_then(|untyped| untyped.into_raw_jsonrpc_message(Some(id.clone())))
                {
                    Ok(request) => request,
                    Err(error) => {
                        tracing::warn!(event = "acp_protocol_request_conversion_failed");
                        #[cfg(feature = "unstable_cancel_request")]
            ...
            OutgoingMessage::Notification { untyped } => {
                let messages = match protocol_compat.outgoing_notification(untyped) {
                    ...
                    Err(_) => {
                        tracing::warn!(event = "acp_protocol_notification_conversion_failed");
                        continue;
                    }
                };
                ...
                for untyped in messages {
                    let message = match untyped.into_raw_jsonrpc_message(None) {
                        ...
                        Err(_) => {
                            tracing::warn!(
                                event = "acp_protocol_notification_serialization_failed"
                            );
                            continue;
                        }
                    };
            ...
            OutgoingMessage::Response {
                ...
                Ok(value) => {
                    tracing::debug!(event = "acp_protocol_response_success");
                    RawJsonRpcMessage::response(id, Ok(value))
                }
                Err(error) => {
                    tracing::warn!(event = "acp_protocol_response_error");
...
fn complete_request_with_error(
    ...
    if response_tx
        ...
        .is_err()
    {
        tracing::debug!(event = "acp_protocol_outgoing_receiver_dropped");
    }
}
