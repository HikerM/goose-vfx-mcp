...
where
    ...
    Peer: Role,
{
    tracing::trace!(event = "acp_protocol_role_dispatch");
    ...
    if let Dispatch::Response(_, router) = &dispatch {
        tracing::trace!(event = "acp_protocol_role_response");
    ...
    match counterpart.remote_style(peer) {
        RemoteStyle::Counterpart => {
            ...
            tracing::trace!(event = "acp_protocol_role_counterpart");
        ...
        RemoteStyle::Predecessor => {
            ...
            tracing::trace!(event = "acp_protocol_role_predecessor");
        ...
        RemoteStyle::Successor => {
            ...
            if method != METHOD_SUCCESSOR_MESSAGE {
                tracing::trace!(event = "acp_protocol_role_successor_mismatch");
                return Ok(Handled::No {
                    message: dispatch,
                    retry: false,
                });
            ...
            tracing::trace!(event = "acp_protocol_role_successor_unwrap");
            ...
            tracing::trace!(event = "acp_protocol_role_successor_unwrapped");
            match handle_dispatch(successor_dispatch, connection).await? {
                Handled::Yes => {
                    tracing::trace!(event = "acp_protocol_role_successor_accepted");
                    Ok(Handled::Yes)
                }
                ...
                Handled::No {
                    ...
                    tracing::trace!(event = "acp_protocol_role_successor_declined");
                    Ok(Handled::No {
