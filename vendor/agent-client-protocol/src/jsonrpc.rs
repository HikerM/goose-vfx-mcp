    ...
    fn register(&self, id: &RequestId) -> ResponderCancellation {
        let generation = {
            ...
            if inner
                ...
                .is_some()
            {
                tracing::debug!(event = "acp_protocol_request_id_reused");
            }
            generation
        };
...
pub fn is_cancel_request_notification<N: JsonRpcNotification>(notification: &N) -> bool {
    ...
    match notification.to_untyped_message() {
        ...
        Err(_) => {
            tracing::debug!(event = "acp_protocol_cancellation_inspection_failed");
            false
        }
    }
}
...
impl<Counterpart: Role> ConnectionTo<Counterpart> {
        ...
        match message {
            ...
            Dispatch::Notification(notification) => {
                ...
                if is_cancel_request_notification(&notification) {
                    tracing::debug!(event = "acp_protocol_cancellation_not_forwarded");
                    return Ok(());
                }
    ...
    {
        let remote_style = self.counterpart.remote_style(peer);
        tracing::debug!(event = "acp_protocol_notification_send");
        let transformed = remote_style.transform_outgoing_message(notification)?;
        tracing::debug!(event = "acp_protocol_notification_transformed");
        send_raw_message(
            &self.message_tx,
...
impl<T: JsonRpcResponse> Responder<T> {
    ...
    pub fn respond_with_result(
        ...
        tracing::debug!(event = "acp_protocol_responder_send");
        (self.send_fn)(response.map_err(crate::util::safe_protocol_error))
    }
    ...
    pub fn respond_with_error(self, error: crate::Error) -> Result<(), crate::Error> {
        tracing::debug!(event = "acp_protocol_responder_error");
        ...
        Self {
            ...
            send_fn: Box::new(move |response: Result<serde_json::Value, crate::Error>| {
                if sender
                    ...
                    .is_err()
                {
                    tracing::debug!(event = "acp_protocol_response_receiver_dropped");
                }
                Ok(())
            }),
        }
...
impl<T: JsonRpcResponse> ResponseRouter<T> {
    ...
    pub fn respond_with_result(
        ...
        tracing::debug!(event = "acp_protocol_response_routed");
    ...
    pub fn respond_with_error(self, error: crate::Error) -> Result<(), crate::Error> {
        tracing::debug!(event = "acp_protocol_response_error_routed");
        self.respond_with_result(Err(crate::util::safe_protocol_error(error)))
    }
}
...
impl Dispatch {
    ...
    #[tracing::instrument(skip(self), fields(category = "acp_protocol"), level = "trace")]
    pub(crate) fn into_typed_dispatch<Req: JsonRpcRequest, Notif: JsonRpcNotification>(
        ...
        tracing::debug!(event = "acp_protocol_typed_dispatch");
        match self {
            Dispatch::Request(message, responder) => {
                if Req::matches_method(&message.method) {
                    match Req::parse_message(&message.method, &message.params) {
                        Ok(req) => {
                            tracing::trace!(event = "acp_protocol_request_parsed");
                            Ok(Ok(Dispatch::Request(req, responder.cast())))
                        }
                        Err(err) => {
                            tracing::trace!(event = "acp_protocol_request_parse_failed");
                            Err(err)
                        }
                    }
                } else {
                    tracing::trace!(event = "acp_protocol_request_method_mismatch");
            ...
            Dispatch::Notification(message) => {
                if Notif::matches_method(&message.method) {
                    match Notif::parse_message(&message.method, &message.params) {
                        Ok(notif) => {
                            tracing::trace!(event = "acp_protocol_notification_parsed");
                            Ok(Ok(Dispatch::Notification(notif)))
                        }
                        Err(err) => {
                            tracing::trace!(event = "acp_protocol_notification_parse_failed");
                            Err(err)
                        }
                    }
                } else {
                    tracing::trace!(event = "acp_protocol_notification_method_mismatch");
                    ...
                    let typed_result = match result {
                        Ok(value) => {
                            match <Req::Response as JsonRpcResponse>::from_value(method, value) {
                                Ok(parsed) => {
                                    tracing::trace!(event = "acp_protocol_response_parsed");
                                    Ok(parsed)
                                }
                                Err(err) => {
                                    tracing::trace!(event = "acp_protocol_response_parse_failed");
                                    return Err(err);
                                }
                            }
                        }
                        Err(err) => {
                            tracing::trace!(event = "acp_protocol_response_error_passthrough");
                            Err(err)
                        }
                    };
                    Ok(Ok(Dispatch::Response(typed_result, cx.cast())))
                } else {
                    tracing::trace!(event = "acp_protocol_response_method_mismatch");
...
impl Drop for SentRequestCancellation {
    fn drop(&mut self) {
        if let Err(error) = self.send() {
            tracing::debug!(event = "acp_protocol_auto_cancel_failed");
        }
    }
}
...
async fn await_response_forwarding_cancellation(
    ...
    let forward_cancellation = || {
        if let Err(error) = cancellation.send() {
            tracing::debug!(event = "acp_protocol_cancellation_forward_failed");
        }
    };
