    ...
    M: serde::de::DeserializeOwned,
{
    let json = serde_json::to_value(params)
        .map_err(|_| crate::Error::parse_error().data("json_cast_serialization_failed"))?;
    let m = serde_json::from_value(json)
        .map_err(|_| crate::Error::parse_error().data("json_cast_deserialization_failed"))?;
    Ok(m)
}
...
{
    let json = serde_json::to_value(params)
        .map_err(|_| crate::Error::internal_error().data("params_serialization_failed"))?;
    let m = serde_json::from_value(json)
        .map_err(|_| crate::Error::invalid_params().data("invalid_request_params"))?;
    Ok(m)
}

/// Creates an internal error with the given message
pub fn internal_error(_message: impl ToString) -> crate::Error {
    crate::Error::internal_error().data("internal_protocol_error")
}

/// Creates a parse error with the given message
pub fn parse_error(_message: impl ToString) -> crate::Error {
    crate::Error::parse_error().data("invalid_protocol_message")
}

pub(crate) fn safe_protocol_error(error: crate::Error) -> crate::Error {
    crate::Error::new(i32::from(error.code), "ACP protocol error").data("acp_protocol_error")
}
...
pub(crate) async fn instrument_with_connection_name<R>(
    ...
    if name.is_some() {
        instrumented_with_connection_name("acp_transport".to_string(), task).await
    } else {
        task.await
    }
}
...
mod tests {
    ...
    #[test]
    fn parameter_deserialization_error_never_echoes_raw_params() {
        const SECRET: &str = "acp-params-secret";
        let error = json_cast_params::<_, ExpectedParams>(serde_json::json!({
            "pageSize": SECRET,
            "session": SECRET,
        ...
        assert_eq!(
            error.data,
            Some(serde_json::json!("invalid_request_params"))
        );
