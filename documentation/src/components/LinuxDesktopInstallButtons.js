...
const FALLBACK_URL = "https://github.com/HikerM/lumina/releases/latest";
...
const LinuxDesktopInstallButtons = () => {
  ...
  useEffect(() => {
    const fetchLatestRelease = async () => {
      try {
        ...
        const cached = localStorage.getItem('lumina-release-cache');
        const cacheTime = localStorage.getItem('lumina-release-cache-time');
        const now = Date.now();
        ...
        const response = await fetch('https://api.github.com/repos/HikerM/lumina/releases/latest');
        ...
        localStorage.setItem('lumina-release-cache', JSON.stringify(newUrls));
        localStorage.setItem('lumina-release-cache-time', now.toString());
      } catch (error) {
  ...
  return (
    <div>
      <p>Click one of the buttons below to download lumina Desktop for Linux:</p>
