---
title: Advanced Cognee Usage with lumina
description: Advanced patterns for using Cognee knowledge graph with lumina for enhanced memory and automation
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Advanced Cognee Usage with lumina

This tutorial covers advanced usage patterns for the Cognee extension with lumina, including automated memory management, knowledge graph optimization, and various integration strategies.

## Overview

While the basic [Cognee MCP setup](../mcp/cognee-mcp.md) gets you started, this tutorial explores how to make lumina autonomously use the knowledge graph and optimize your workflow.
...
Create `~/.config/lumina/cognee-instructions.md`:
...
Start lumina with instructions:
```bash
lumina run -i ~/.config/lumina/cognee-instructions.md -s
```
...
For faster startup with higher token usage, add to your `.luminahints` file:
...
3. Lower token usage than luminahints
...
```bash
# Start lumina with Cognee
lumina session

# In lumina, analyze your codebase
> lumina, please codify this repository and then help me understand the architecture
```
...
> lumina, please cognify the contents of these research papers: paper1.pdf, paper2.pdf, paper3.pdf
...
```bash
# Check status
> lumina, what's the status of the cognify pipeline?

# Selective pruning (if needed)
> lumina, can you help me identify outdated information in the knowledge graph?
```
...
4. **High token usage**: Use instruction files instead of luminahints
...
# In lumina session:
> lumina, run cognify_status and codify_status
```
...
1. **Use nodesets** for organizing different types of information:
   ```bash
   # Developer rules
   > lumina, add these coding standards to the 'developer_rules' nodeset
   
   # Project-specific info  
   > lumina, cognify this project documentation with nodeset 'project_alpha'
   ```
...
```bash
# Setup
> lumina, codify this repository and remember that I prefer: functional programming patterns, comprehensive tests, and clear documentation
...
```bash
# Before meeting
> lumina, cognify the agenda and participant backgrounds from these documents
...
```bash
# Literature review
> lumina, cognify these 10 research papers and create a knowledge graph of the relationships between their methodologies

# Synthesis
> What are the emerging patterns in the research and what gaps exist?
```

This advanced usage guide should help you maximize the potential of Cognee with lumina for sophisticated knowledge management and automation workflows.
