...
description: Create interactive UI applications that render inside lumina Desktop
---

import { PanelLeft } from 'lucide-react';

# Building MCP Apps for lumina

MCP Apps let MCP servers return interactive UIs that render directly inside the lumina chat interface, rather than responding with text alone. This allows users to express intent through interaction, which is useful for workflows that require input, iteration, or visual feedback.

:::warning Experimental
MCP Apps support in lumina is experimental and based on a draft specification. The implementation is minimal and may change, and does not yet support advanced capabilities or persistent app windows.
:::
...
- lumina Desktop 1.19.1+ installed
:::
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "show_demo_app") {
    return {
      ...
      // This metadata tells lumina to render the MCP App
      _meta: {
        ui: {
...
<body class="light">
  <div class="container">
    <h1>🎮 MCP App Demo</h1>
    <p class="subtitle">An interactive UI running inside lumina</p>
    ...
    <div class="message-section">
      <h3>💬 Send a message to lumina</h3>
    ...
    <div class="info-section">
      ...
      It communicates with lumina via JSON-RPC messages through the sandbox bridge.
      <br><br>
      • Counter uses local state<br>
      • "Send" calls <code>ui/message</code> to append text to chat<br>
      • Theme syncs with lumina's theme setting
    </div>
  </div>
...
1. Restart lumina to load the new extension
2. Prompt lumina: "Show me the demo app"
3. lumina will call the `show_demo_app` tool
4. Your interactive app will render in the chat!

Try:
- Clicking the counter buttons
- Typing a message and clicking "Send"
- Switching lumina between light/dark mode
...
└──────────────────┬───────────────────┘
                   │ postMessage
┌──────────────────▼───────────────────┐
│          lumina Desktop               │  Renders UI, routes messages
└──────────────────┬───────────────────┘
                   │ MCP Protocol
┌──────────────────▼───────────────────┐
...
Your server returns a `ui://` resource URI, lumina fetches the HTML and renders it in an iframe. The app communicates back via `postMessage`—requesting theme info, sending messages to chat, or resizing itself.
