...
When working with [Large Language Models (LLMs)](/docs/getting-started/providers), there are limits to how much conversation history they can process at once. lumina provides smart context management features to help handle context and conversation limits so you can maintain productive sessions. Here are some key concepts:
...
- **Context Management**: How lumina handles conversations approaching these limits
- **Turn**: One complete prompt-response interaction between lumina and the LLM

## How lumina Manages Context
lumina uses a two-tiered approach to context management:
...
This layered approach lets lumina handle token and context limits gracefully.

## Automatic Compaction
lumina automatically compacts (summarizes) older parts of your conversation when approaching token limits, allowing you to maintain long-running sessions without manual intervention.
Auto-compaction is triggered by default when you reach 80% of the token limit in lumina Desktop and the lumina CLI.

Control the auto-compaction behavior with the `LUMINA_AUTO_COMPACT_THRESHOLD` [environment variable](/docs/guides/environment-variables.md#session-management).
...
When you reach the auto-compaction threshold:
  1. lumina will automatically start compacting the conversation to make room.
  2. Once complete, you'll see a confirmation message that the conversation was compacted and summarized.
  3. Continue the session. Your previous conversation remains visible, but only the compacted conversion is included in the active context for lumina.

:::tip Customize Compaction
You can customize how lumina summarizes conversations during compaction by editing the `compaction.md` [prompt template](/docs/guides/context-engineering/prompt-templates).
:::

:::tip Tool Output Summarization
To help maintain efficient context usage, lumina summarizes older tool call outputs in the background while keeping recent calls in full detail. By default, this happens when you have more than 10 tool calls in a session. For advanced tuning, see [`LUMINA_TOOL_CALL_CUTOFF`](/docs/guides/environment-variables#session-management).
:::
...
<Tabs groupId="interface">
  ...
  4. Continue the session. Your previous conversation remains visible, but only the compacted conversion is included in the active context for lumina.
...
<TabItem value="cli" label="lumina CLI" default>
...
When auto-compaction is disabled, or if a conversation still exceeds the context limit, lumina offers different ways to handle it:
...
lumina Desktop exclusively uses summarization by compacting the conversation to manage context, preserving key information while reducing size.
...
- **Headless mode** (`lumina run`): Automatically summarizes (equivalent to `summarize`)

You can configure how lumina handles context limits by setting the `LUMINA_CONTEXT_STRATEGY` environment variable:

```bash
# Set automatic strategy (choose one)
export LUMINA_CONTEXT_STRATEGY=summarize  # Automatically summarize (recommended)
export LUMINA_CONTEXT_STRATEGY=truncate   # Automatically remove oldest messages
export LUMINA_CONTEXT_STRATEGY=clear      # Automatically clear session
...
**With default settings (no `LUMINA_CONTEXT_STRATEGY` set)**, you'll see this prompt to choose a management option:
...
Context maxed out
--------------------------------------------------
lumina summarized messages for you.
```

**With `LUMINA_CONTEXT_STRATEGY` configured**, lumina will automatically apply your chosen strategy:

```sh
# Example with LUMINA_CONTEXT_STRATEGY=summarize
Context maxed out - automatically summarized messages.
--------------------------------------------------
lumina automatically summarized messages for you.

# Example with LUMINA_CONTEXT_STRATEGY=truncate
Context maxed out - automatically truncated messages.
--------------------------------------------------
lumina tried its best to truncate messages for you.

# Example with LUMINA_CONTEXT_STRATEGY=clear
...
## Maximum Turns
The `Max Turns` limit is the maximum number of consecutive turns that lumina can take without user input (default: 1000). When the limit is reached, lumina stops and prompts: "I've reached the maximum number of actions I can do without user input. Would you like me to continue?" If the user answers in the affirmative, lumina continues until the limit is reached and then prompts again.
...
This setting is stored as the `LUMINA_MAX_TURNS` environment variable in your [config.yaml file](/docs/guides/config-files). You can configure it using the Desktop app or CLI.
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="lumina CLI">
      ...
      │  10
        // highlight-end
      │
      └  Set maximum turns to 10 - lumina will ask for input after 10 consecutive actions
      ```

      :::tip
      In addition to the persistent `Max Turns` setting, you can provide a runtime override for a specific session or task via the `lumina session --max-turns` and `lumina run --max-turns` [CLI commands](/docs/guides/lumina-cli-commands).
      :::
...
- **100+ turns**: More suitable for complex, multi-step automation where you trust lumina to work independently, like "migrate this entire project from React 16 to React 18" or "implement comprehensive test coverage for this service"

Remember that even simple-seeming tasks often require multiple turns. For example, asking lumina to "fix the failing tests" might involve analyzing test output (1 turn), identifying the root cause (1 turn), making code changes (1 turn), and verifying the fix (1 turn).

## Token Usage
After sending your first message, lumina Desktop and lumina CLI display token usage.
...
Context limits are automatically detected based on your model name, but lumina provides settings to override the default limits:

| Model | Description | Best For | Setting |
|-------|-------------|----------|---------|
| **Main** | Set context limit for the main model (also serves as fallback for other models) | LiteLLM proxies, custom models with non-standard names | `LUMINA_CONTEXT_LIMIT` |
| **Planner** | Set context for [planner models](/docs/guides/context-engineering/creating-plans) | Large planning tasks requiring extensive context | `LUMINA_PLANNER_CONTEXT_LIMIT` |
...
- **LiteLLM Proxy Models**: When using LiteLLM with custom model names that don't match lumina's patterns
...
lumina resolves context limits with the following precedence (highest to lowest):

1. Explicit context_limit in model configuration (if set programmatically)
2. Specific environment variable (e.g., `LUMINA_PLANNER_CONTEXT_LIMIT`)
3. Global environment variable (`LUMINA_CONTEXT_LIMIT`)
...
<Tabs groupId="interface">
  <TabItem value="ui" label="lumina Desktop" default>
     ...
     Model context limit overrides are not yet available in the lumina Desktop app.
  ...
  <TabItem value="cli" label="lumina CLI">
    ...
    ```bash
    export LUMINA_CONTEXT_LIMIT=1000
    lumina session
    ```
...
export LUMINA_PROVIDER="openai"
export LUMINA_MODEL="my-custom-gpt4-proxy"
export LUMINA_CONTEXT_LIMIT=200000  # Override the 32k default
```
...
export LUMINA_PLANNER_MODEL="claude-opus-custom"
export LUMINA_PLANNER_CONTEXT_LIMIT=500000
```
...
export LUMINA_PLANNER_MODEL="gpt-4-custom"
export LUMINA_PLANNER_CONTEXT_LIMIT=1000000
```

## Credit Balance Monitoring

lumina monitors your API provider balance and warns you when credits are running low or exhausted. When this happens, you'll see an **Insufficient Credits** notification.
...
:::tip
lumina detects low balance conditions automatically, so you won't lose your conversation context—just add credits and pick up where you left off.
:::
...
These costs are estimates only, and not connected to your actual provider bill. The cost shown is an approximation based on token counts and public pricing data.
    ...
    <TabItem value="cli" label="lumina CLI">
    Show estimated cost in the lumina CLI by setting the `LUMINA_CLI_SHOW_COST` [environment variable](/docs/guides/environment-variables.md#session-management) or including it in the [configuration file](/docs/guides/config-files.md).
