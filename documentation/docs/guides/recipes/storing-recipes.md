...
This guide covers storing, organizing, and finding lumina recipes when you need to access them again later.

:::info Desktop UI vs CLI
- **lumina Desktop** has a visual Recipe Library for browsing and managing saved recipes
- **lumina CLI** stores recipes as files that you find using file paths or environment variables
:::
...
|------|----------|-------------|----------|
| **Global** | `~/.config/lumina/recipes/` | All projects and sessions | Personal workflows, general-purpose recipes |
| **Local** | `YOUR_WORKING_DIRECTORY/.lumina/recipes/` | Only when working in that project | Project-specific workflows, team recipes |
...
<Tabs groupId="interface">
  <TabItem value="desktop" label="lumina Desktop" default>
...
:::
  ...
  <TabItem value="cli" label="lumina CLI">
    ...
    * Local project recipes: `/recipe .lumina/recipes/my-recipe.yaml`
...
<Tabs groupId="interface">
  ...
  lumina Desktop accepts `.yaml`, `.yml`, and `.json` files, but **the CLI only supports `.yaml` and `.json`**. For full compatibility across both interfaces, avoid `.yml` extensions.
  ...
  <TabItem value="cli" label="lumina CLI">
    Recipe import is only available in lumina Desktop.
  </TabItem>
</Tabs>
...
Use the `lumina recipe list` command to find all available recipes from multiple sources:
...
lumina searches for recipes in the following locations (in order):

1. **Current directory**: `.` (looks for `*.yaml` and `*.json` files)
2. **Custom paths**: Directories specified in [`LUMINA_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) environment variable
3. **Global recipe library**: `~/.config/lumina/recipes/` (or equivalent on your OS)
4. **Local project recipes**: `./.lumina/recipes/`
5. **GitHub repository**: If [`LUMINA_RECIPE_GITHUB_REPO`](/docs/guides/environment-variables#recipe-configuration) environment variable is configured
...
$ lumina recipe list
Available recipes:
lumina-self-test - A comprehensive meta-testing recipe - local: ./lumina-self-test.yaml
hello-world - A sample recipe demonstrating basic usage - local: ~/.config/lumina/recipes/hello-world.yaml
job-finder - Find software engineering positions - local: ~/.config/lumina/recipes/job-finder.yaml
```
...
Available recipes:
  lumina-self-test - A comprehensive meta-testing recipe - local: ./lumina-self-test.yaml
    Title: lumina Self-Testing Integration Suite
    Path: ./lumina-self-test.yaml
  hello-world - A sample recipe demonstrating basic usage - local: ~/.config/lumina/recipes/hello-world.yaml
    Title: Hello World Recipe
    Path: /Users/username/.config/lumina/recipes/hello-world.yaml
```
...
:::tip
The `lumina recipe list` command is the recommended way to find recipes as it automatically searches all configured sources and provides consistent formatting.
:::
...
Once you've located your recipe file, [run the recipe](/docs/guides/recipes/session-recipes#run-a-recipe) or [open it in lumina Desktop](/docs/guides/lumina-cli-commands#recipe).

:::tip Format Compatibility
The CLI can run recipes saved from lumina Desktop without any conversion. Both CLI-created and Desktop-saved recipes work with all recipe commands.
:::
