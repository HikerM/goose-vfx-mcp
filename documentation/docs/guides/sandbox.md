---
title: macOS Sandbox for lumina Desktop
sidebar_label: Sandbox for lumina Desktop
description: Optional sandboxing for lumina Desktop to control file access, filter network traffic, and enforce security policies on macOS
---

lumina Desktop includes an optional macOS sandbox that you can enable when you need stricter control and visibility over what lumina can access on your system. Use it to:

- **Restrict file system access** — Block writes to SSH keys, shell configs, and lumina configuration files
...
lumina runs with full tool access, but the sandbox uses two layers of protection:
...
To enable the sandbox, launch lumina Desktop from the terminal with the environment variable set. For example:

```bash
export LUMINA_SANDBOX=true
open -a Lumina
```
...
3. Launch the `luminad` backend for lumina Desktop inside `sandbox-exec`, forcing all traffic through the proxy

The sandbox remains active until you quit lumina Desktop. To disable it, quit the app and relaunch normally (or set `LUMINA_SANDBOX=false` when opening from the terminal).
...
|----------|---------|-------------|
| `LUMINA_SANDBOX` | `false` | Set to `true` or `1` to enable the sandbox. See [Quick Start](#quick-start) for launch instructions. |

----

### File System

The [seatbelt sandbox profile](https://github.com/HikerM/lumina/blob/main/ui/desktop/src/sandbox/index.ts) blocks write operations to these sensitive files:

- `~/.ssh/` - Prevent SSH key tampering
- `~/.bashrc`, `~/.zshrc`, `~/.bash_profile`, `~/.zprofile` - Prevent shell config injection
- `~/.config/lumina/sandbox/` - Protect sandbox config from the sandboxed process
- `~/.config/lumina/config.yaml` - Protect lumina config
...
| Variable | Default | Description |
|----------|---------|-------------|
| `LUMINA_SANDBOX_PROTECT_FILES` | `true` | Write-protect sensitive files listed above. Set to `false` to disable |
...
- **Localhost** — Allows the `luminad` process to reach the egress proxy and its own server port
...
| Variable | Default | Description |
|----------|---------|-------------|
| `LUMINA_SANDBOX_BLOCK_RAW_SOCKETS` | `true` | Block `SOCK_RAW`. Set to `false` to disable |
| `LUMINA_SANDBOX_BLOCK_TUNNELING` | `true` | Block `nc`/`netcat`/`socat`/`telnet`. Set to `false` to disable |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `LUMINA_SANDBOX_ALLOW_IP` | `false` | Set to `true` to allow connections to raw IP addresses |
| `LUMINA_SANDBOX_BLOCK_LOOPBACK` | `false` | Set to `true` to block loopback relay through the proxy |
| `LUMINA_SANDBOX_ALLOW_SSH` | `true` | Set to `false` to block all SSH traffic |
| `LUMINA_SANDBOX_GIT_HOSTS` | built-in list | Comma-separated list of allowed SSH git hosts (e.g. `github.com,gitlab.com`) |
| `LUMINA_SANDBOX_SSH_ALL_HOSTS` | `false` | Set to `true` to allow SSH to any host (not just git hosts) |

#### Managing the Domain Blocklist

The file `~/.config/lumina/sandbox/blocked.txt` controls which domains are blocked by the proxy. It's created automatically on first run from a bundled template.
...
export LUMINA_SANDBOX_GIT_HOSTS="github.com,gitlab.com,your-gitea.internal.com"

# Or allow SSH to all hosts
export LUMINA_SANDBOX_SSH_ALL_HOSTS=true
```
...
| `LUMINA_SANDBOX_LD_FAILOVER` | — | Failover mode if LD is unreachable: `allow`, `deny`, or `blocklist` |
...
```bash
export LUMINA_SANDBOX=true
export LUMINA_SANDBOX_ALLOW_IP=true
```
...
```bash
export LUMINA_SANDBOX=true
export LUMINA_SANDBOX_ALLOW_SSH=false
```
...
```bash
export LUMINA_SANDBOX=true
export LUMINA_SANDBOX_PROTECT_FILES=false
export LUMINA_SANDBOX_BLOCK_RAW_SOCKETS=false
export LUMINA_SANDBOX_BLOCK_TUNNELING=false
export LUMINA_SANDBOX_ALLOW_IP=true
export LUMINA_SANDBOX_SSH_ALL_HOSTS=true
```
...
export LUMINA_SANDBOX_LD_FAILOVER=blocklist  # fall back to local blocklist if LD is down
```

## Troubleshooting

- **Error: "LUMINA_SANDBOX=true but sandbox-exec is not available (macOS only)"**
...
- **Extensions or tools can't reach the network**
  Check if the destination domain is in `~/.config/lumina/sandbox/blocked.txt`, or if you need to enable `LUMINA_SANDBOX_ALLOW_IP=true` for IP-based endpoints.

- **git clone over SSH fails**
  The target host may not be in the default Git hosts allowlist. Add it with `LUMINA_SANDBOX_GIT_HOSTS=your-host.com` or set `LUMINA_SANDBOX_SSH_ALL_HOSTS=true`.
