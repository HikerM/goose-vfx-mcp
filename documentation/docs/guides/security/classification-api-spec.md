...
This API specification defines the API that lumina uses for ML-based [prompt injection detection](/docs/guides/security/prompt-injection-detection).
...
**lumina's Usage:**
- lumina looks for the label with the highest score
- If the top label is `"INJECTION"` (or `"LABEL_1"`), the score is used as the injection confidence
- If the top label is `"SAFE"` (or `"LABEL_0"`), lumina uses `1.0 - score` as the injection confidence
