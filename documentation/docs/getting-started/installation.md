...
<Tabs>
  <TabItem value="mac" label="macOS" default>
    Choose to install the Desktop and/or CLI version of lumina:

    <Tabs groupId="interface">
      <TabItem value="ui" label="lumina Desktop" default>
        Install lumina Desktop directly from the browser or with [Homebrew](https://brew.sh/).
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          2. Run the executable file to launch the lumina Desktop application.

          :::tip Updating lumina
          It's best to periodically [update lumina](/docs/guides/updating-lumina).
          :::
        </div>
        <h3>Option 2: Install via Homebrew</h3>
        Homebrew downloads the [same app](https://github.com/Homebrew/homebrew-cask/blob/master/Casks/b/block-lumina.rb) but can take care of updates too.
        ```bash
        brew install --cask block-lumina
        ```
        ---
        <div style={{ marginTop: '1rem' }}>
          :::info Permissions
          If you're on an Apple Mac M3 and the lumina Desktop app shows no window on launch, check and update the following:

          Ensure the `~/.config` directory has read and write access.

          lumina needs this access to create the log directory and file. Once permissions are granted, the app should load correctly. For steps on how to do this, refer to the  [Known Issues Guide](/docs/troubleshooting/known-issues#macos-permission-issues)
          :::
        </div>
      </TabItem>
      <TabItem value="cli" label="lumina CLI">
        Install lumina directly from the browser or with [Homebrew](https://brew.sh/).

        <h3 style={{ marginTop: '1rem' }}>Option 1: Install via Download script</h3>
        Run the following command to install the latest version of lumina on macOS:

        ```sh
        curl -fsSL https://github.com/HikerM/lumina/releases/download/stable/download_cli.sh | bash
        ```
        This script will fetch the latest version of lumina and set it up on your system.
        ...
        :::tip Updating lumina
        It's best to keep lumina updated. To update lumina, run:
        ```sh
        lumina update
        ```
        :::
        ...
        <h3>Option 2: Install via Homebrew</h3>
        Homebrew downloads the [a precompiled CLI tool](https://github.com/Homebrew/homebrew-core/blob/master/Formula/b/block-lumina-cli.rb) and can take care of updates.
        ```bash
        brew install block-lumina-cli
        ```
      </TabItem>
    </Tabs>
  </TabItem>
  ...
  <TabItem value="linux" label="Linux">
    ...
    <Tabs groupId="interface">
      <TabItem value="ui" label="lumina Desktop" default>
        Install lumina Desktop directly from the browser.
      ...
      <TabItem value="cli" label="lumina CLI">
        Run the following command to install the lumina CLI on Linux:
  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="lumina CLI">
        To install lumina natively on Windows, you need one of the following environments:
        ...
        **PowerShell Installation: Standard**
        ...
        ```powershell
        Invoke-WebRequest -Uri "https://raw.githubusercontent.com/HikerM/lumina/main/download_cli.ps1" -OutFile "download_cli.ps1";
        ```
        Then run the script to install lumina:
        ```powershell
        .\download_cli.ps1
        ```

        :::info Windows PATH Setup
        If you see a warning that lumina is not in your PATH, you need to add lumina to your PATH:
        ...
        After updating your PATH, you can run `lumina` commands from any directory.
        :::

        <details>
        <summary>Install via Windows Subsystem for Linux (WSL)</summary>

          We recommend running the lumina CLI natively on Windows, but you can use WSL if you prefer a Linux-like environment.
          ...
          3. Run the lumina installation script:
          ```bash
          curl -fsSL https://github.com/HikerM/lumina/releases/download/stable/download_cli.sh | bash
          ```
          :::tip
          ...
          If needed, add lumina to your path:
...
## Set LLM Provider
lumina works with [supported LLM providers][providers] that give lumina the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.

<Tabs groupId="interface">
  <TabItem value="ui" label="lumina Desktop" default>
    On the welcome screen the first time you open lumina, choose how to configure a provider:
  ...
  <TabItem value="cli" label="lumina CLI">
    ...
    [lumina opens the browser and prints details]
  ...
  Then run `lumina configure` again. lumina will detect the environment variable and display:
...
<Tabs groupId="interface">
    <TabItem value="ui" label="lumina Desktop" default>
        Starting a session in the lumina Desktop is straightforward. After choosing your provider, you'll see the session interface ready for use.

        Type your questions, tasks, or instructions directly into the input field, and lumina will get to work immediately.
    </TabItem>
    <TabItem value="cli" label="lumina CLI">
...
## Pin a lumina version in CI/CD
In CI/CD (and other automated, non-interactive environments), pin a specific version with `LUMINA_VERSION` to make installs reproducible and avoid 404s when downloading the lumina CLI binary assets if the `stable` release tag doesn’t include them.
...
If you're packaging lumina for a Linux distribution or creating a custom build, you can generate Unix manpages from the CLI command definitions:

```bash
just generate-manpages
```

This creates ROFF-formatted manpages in `target/man/` (e.g., `lumina.1`, `lumina-session.1`) that can be installed to `/usr/share/man/man1/` to provide offline documentation via the `man` command.

Manpage generation requires the lumina source repository and is intended for distribution packagers preparing packages for Fedora, Debian, and other Linux distributions. See the [generate_manpages.rs source](https://github.com/HikerM/lumina/blob/main/crates/lumina-cli/src/bin/generate_manpages.rs) for implementation details.

## Additional Resources

You can also configure Extensions to extend lumina's functionality, including adding new ones or toggling them on and off. For detailed instructions, visit the [Using Extensions Guide][using-extensions].
