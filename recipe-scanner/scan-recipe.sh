...
OUTPUT_DIR="/output"
WORKSPACE="/workspace"
LUMINA_BIN="/usr/local/bin/lumina"
BASE_RECIPE="/docker/base_recipe.yaml"

# Globals used for meta
ANALYSIS_METHOD="lumina_ai"
MARKERS_FOUND=false
    ...
    cat > "$OUTPUT_DIR/scan_status.json" << EOF
{
  ...
  "lumina_exit_code": ${SCAN_EXIT_CODE:-0},
  "debug_info": {
    "line": ${line_no},
    "exit_code": ${exit_code},
    ...
    "environment": {
      ...
      "lumina_exists": $([ -f "$LUMINA_BIN" ] && echo "true" || echo "false"),
...
🔍 Lumina Recipe Security Scanner - ERROR REPORT
==============================================
...
- Lumina binary exists: $([ -f "$LUMINA_BIN" ] && echo "✅ YES" || echo "❌ NO")
...
4. Lumina configuration
EOF

    # Ensure lumina_output.log exists
    touch "$OUTPUT_DIR/lumina_output.log" 2>/dev/null || true
...
if python3 /usr/local/bin/decode-training-data.py; then
    ...
    TRAINING_INSTRUCTIONS="/tmp/lumina_training_instructions.md"
...
# Install Lumina CLI if needed
if [ ! -f "$LUMINA_BIN" ]; then
    echo "⬇️ Installing Lumina CLI..."

    if curl -fsSL --connect-timeout 30 --max-time 300 \
       https://github.com/HikerM/lumina/releases/download/stable/download_cli.sh | bash; then
        for path in "$HOME/.local/bin/lumina" "/usr/local/bin/lumina" "$(which lumina 2>/dev/null || true)"; do
            if [ -n "$path" ] && [ -f "$path" ] && [ -x "$path" ]; then
                cp "$path" "$LUMINA_BIN"
                chmod +x "$LUMINA_BIN"
                echo "✅ Lumina CLI installed from $path"
                break
            fi
        done
    fi
    ...
    if [ ! -f "$LUMINA_BIN" ]; then
        ...
        temp_dir=$(mktemp -d)
        if curl -fsSL --connect-timeout 30 --max-time 300 \
           "https://github.com/HikerM/lumina/releases/download/stable/lumina-x86_64-unknown-linux-gnu.tar.bz2" \
           -o "$temp_dir/lumina.tar.bz2"; then
            tar -xjf "$temp_dir/lumina.tar.bz2" -C "$temp_dir"
            lumina_binary=$(find "$temp_dir" -name "lumina" -type f -executable | head -1)
            if [ -n "$lumina_binary" ]; then
                cp "$lumina_binary" "$LUMINA_BIN"
                chmod +x "$LUMINA_BIN"
                echo "✅ Lumina CLI installed via direct download"
            fi
        fi
        rm -rf "$temp_dir"
    fi
    ...
    if [ ! -f "$LUMINA_BIN" ]; then
        echo "❌ Failed to install Lumina CLI"
        exit 1
    fi
fi

# Verify Lumina installation
echo "🔧 Verifying Lumina installation..."
if ! "$LUMINA_BIN" --version >/dev/null 2>&1; then
    echo "❌ Lumina CLI not working"
    "$LUMINA_BIN" --version || true
    exit 1
fi

echo "✅ Lumina CLI ready: $($LUMINA_BIN --version)"

# Set up Lumina environment
echo "🔧 Configuring Lumina environment..."

USER_ID="$(id -u)"
LUMINA_TMP="/tmp/lumina_${USER_ID}"
mkdir -p "$LUMINA_TMP"/{logs,state,cache,config} 2>/dev/null || true
chmod -R 755 "$LUMINA_TMP" 2>/dev/null || true

export XDG_STATE_HOME="$LUMINA_TMP/state"
export XDG_CACHE_HOME="$LUMINA_TMP/cache"
export LUMINA_TELEMETRY_ENABLED=false
export LUMINA_PROJECT_TRACKER_ENABLED=false
export RUST_LOG=error

if [ -f "$HOME/.config/lumina/config.yaml" ]; then
    cp "$HOME/.config/lumina/config.yaml" "$LUMINA_TMP/config/config.yaml" 2>/dev/null || true
    export LUMINA_CONFIG_DIR="$LUMINA_TMP/config"
fi

echo "✅ Lumina environment configured"

# Quick health check (decoupled from analysis)
echo "🔍 Running Lumina health check..."
if timeout 30 "$LUMINA_BIN" run --no-session -t "Hello, are you working?" >> "$OUTPUT_DIR/lumina_output.log" 2>&1; then
    echo "✅ Lumina health check passed"
else
    echo "⚠️ Lumina health check failed - continuing anyway"
fi
...
python3 - "$RECIPE_FILE" > "$PY_UNICODE_REPORT" 2>>"$OUTPUT_DIR/lumina_output.log" <<'PY' || true
import sys, json
path = sys.argv[1]
...
if [ -s "$PY_UNICODE_REPORT" ] && jq -e '.findings | length > 0' "$PY_UNICODE_REPORT" >/dev/null 2>&1; then
  ...
  jq -n \
    ...
    --argjson lumina_exit_code 0 \
    --arg analysis_method "${ANALYSIS_METHOD}" \
    '{status: $status, reason: $reason, risk_score: $risk_score, risk_level: $risk_level, message: $message, scan_successful: $scan_successful, analysis_method: $analysis_method, lumina_exit_code: $lumina_exit_code}' \
  ...
  jq -n \
    ...
    '{path_taken:$path_taken, markers_found:$markers_found, retry_attempted:$retry_attempted, heuristic_used:$heuristic_used, unicode_found:$unicode_found, benign_hint:$benign_hint, lumina_exit_code:$lumina_exit_code, timestamp:$timestamp, unicode_scan:$unicode_findings}' \
    > "$OUTPUT_DIR/analysis_meta.json"

  # Reports
  TIMESTAMP=$(date -u -Iseconds)
  cat > "$OUTPUT_DIR/security-report.md" << EOF
# Lumina Recipe Security Analysis
...
$(jq -r '.[]? | "- " + (.category // "unicode") + ": " + (.snippet // "")' <<< "$EVIDENCE" 2>/dev/null || echo "See lumina_result.json")
...
EOF
  ...
  cat > "$OUTPUT_DIR/summary.txt" << EOF
🔍 Lumina Recipe Security Analysis Summary (Unicode Stealth)
=========================================================
...
if [ "${BENIGN_HINT}" = true ]; then
  ...
  jq -n \
    ...
    --argjson lumina_exit_code 0 \
    --arg timestamp "$(date -u -Iseconds)" \
    '{path_taken:$path_taken, markers_found:$markers_found, retry_attempted:$retry_attempted, heuristic_used:$heuristic_used, unicode_found:$unicode_found, benign_hint:$benign_hint, lumina_exit_code:$lumina_exit_code, timestamp:$timestamp}' \
    > "$OUTPUT_DIR/analysis_meta.json"
...
EOF
  ...
  cat > "$OUTPUT_DIR/summary.txt" << EOF
🔍 Lumina Recipe Security Analysis Summary (Deterministic Benign)
==============================================================
📅 Analysis Date: $(date -u)
📋 Recipe: $(basename "$RECIPE_FILE")
🤖 Analysis Method: Deterministic benign fallback
...
if timeout 60 "$LUMINA_BIN" run \
    --recipe "$BASE_RECIPE" \
    --no-session \
    --render-recipe \
    --params recipe_path="$RECIPE_FILE" \
    --params strict_mode="false" \
    > "$OUTPUT_DIR/rendered_base_recipe.yaml" 2>> "$OUTPUT_DIR/lumina_output.log"; then
  echo "✅ Rendered base recipe saved to $OUTPUT_DIR/rendered_base_recipe.yaml"
else
  echo "⚠️ Failed to render base recipe (non-fatal)" >> "$OUTPUT_DIR/lumina_output.log"
fi
...
timeout 600 "$LUMINA_BIN" run \
    --recipe "$BASE_RECIPE" \
    --no-session \
    --quiet \
    --params recipe_path="$RECIPE_FILE" \
    >> "$OUTPUT_DIR/lumina_output.log" 2>&1 || SCAN_EXIT_CODE=$?

echo "📊 Security analysis completed with exit code: $SCAN_EXIT_CODE"

# Parsing helpers
extract_marked_json() {
  if grep -q 'BEGIN_LUMINA_JSON' "$OUTPUT_DIR/lumina_output.log" && grep -q 'END_LUMINA_JSON' "$OUTPUT_DIR/lumina_output.log"; then
    MARKERS_FOUND=true
    tac "$OUTPUT_DIR/lumina_output.log" | awk '
        /END_LUMINA_JSON/ && !found { found=1; next }
        found && /BEGIN_LUMINA_JSON/ { exit }
        found { print }
    ' | tac > "$OUTPUT_DIR/lumina_result.marked.txt" 2>/dev/null || true
    # strip code fences and blank lines
    sed -e 's/^```[a-zA-Z]*$//g' -e 's/^```$//g' "$OUTPUT_DIR/lumina_result.marked.txt" | sed '/^\s*$/d' > "$OUTPUT_DIR/lumina_result.json" || true
  fi
}

heuristic_json() {
  PY_OUT="$OUTPUT_DIR/lumina_result.heuristic.json"
  python3 - "$OUTPUT_DIR/lumina_output.log" > "$PY_OUT" 2>>"$OUTPUT_DIR/lumina_output.log" <<'PY' || true
import sys, json
path = sys.argv[1]
...
PY
  if [ -s "$PY_OUT" ] && jq . "$PY_OUT" >/dev/null 2>&1; then
    mv -f "$PY_OUT" "$OUTPUT_DIR/lumina_result.json" || true
    HEURISTIC_USED=true
  fi
}
...
# Try markers
extract_marked_json
if [ -f "$OUTPUT_DIR/lumina_result.json" ] && jq . "$OUTPUT_DIR/lumina_result.json" >/dev/null 2>&1; then
  JSON_VALID=true
else
  # Heuristic attempt 1
  heuristic_json
...
if [ "$JSON_VALID" = false ]; then
  ...
  echo "🔁 Retrying once with strict JSON-only instruction..." | tee -a "$OUTPUT_DIR/lumina_output.log"
  timeout 120 "$LUMINA_BIN" run \
      --recipe "$BASE_RECIPE" \
      --no-session \
      --params recipe_path="$RECIPE_FILE" \
      --params strict_mode="true" \
      >> "$OUTPUT_DIR/lumina_output.log" 2>&1 || true
...
if [ "$JSON_VALID" = true ]; then
  echo "✅ Found valid JSON result from Lumina"
  SCORE=$(jq -r '.score // 0' "$OUTPUT_DIR/lumina_result.json")
  RECOMMENDATION=$(jq -r '.recommendation // "UNKNOWN"' "$OUTPUT_DIR/lumina_result.json")
  SUMMARY=$(jq -r '.summary // "No summary provided"' "$OUTPUT_DIR/lumina_result.json")
  if ! [[ "${SCORE}" =~ ^[0-9]+$ ]]; then
    echo "⚠️ SCORE not numeric ('$SCORE'); entering fallback scoring" | tee -a "$OUTPUT_DIR/lumina_output.log"
    JSON_VALID=false
  else
    SCAN_SUCCESSFUL=true
    # Only set to lumina_ai if we did not set a more specific method above
    if [ "$ANALYSIS_METHOD" = "lumina_ai" ]; then
      ANALYSIS_METHOD="lumina_ai"
    fi
  fi
fi
...
if [ "$JSON_VALID" != true ]; then
  echo "🧮 Applying enhanced static fallback scoring" >> "$OUTPUT_DIR/lumina_output.log"
  RAW=$(tr '\n' ' ' < "$RECIPE_FILE" | tr -s ' ')
  SCORE=15
  ...
  # Check for external script downloads - analyze the actual content
    ...
    echo "🔍 Static analysis: found external URLs, analyzing content..." >> "$OUTPUT_DIR/lumina_output.log"
    echo "🔍 Detected URLs: $EXTERNAL_URLS" >> "$OUTPUT_DIR/lumina_output.log"
    ...
    for URL in $EXTERNAL_URLS; do
      ...
      echo "🔍 Downloading: $URL" >> "$OUTPUT_DIR/lumina_output.log"
      
      # Enhanced curl with better error handling and user agent
      if curl -sSfL --max-time 30 --connect-timeout 10 --user-agent "Mozilla/5.0 (Security Scanner)" "$URL" -o "$SCRIPT_FILE" 2>>"$OUTPUT_DIR/lumina_output.log"; then
        echo "✅ Downloaded external file for analysis: $URL ($(wc -c < "$SCRIPT_FILE") bytes)" >> "$OUTPUT_DIR/lumina_output.log"
        SCRIPT_ANALYZED=true
        
        # Show first few lines for debugging
        echo "📄 First 5 lines of downloaded content:" >> "$OUTPUT_DIR/lumina_output.log"
        head -5 "$SCRIPT_FILE" >> "$OUTPUT_DIR/lumina_output.log" 2>/dev/null || echo "Could not read file" >> "$OUTPUT_DIR/lumina_output.log"
        ...
        echo "🔍 Detailed analysis of $URL:" >> "$OUTPUT_DIR/lumina_output.log"
        echo "  - SSH patterns: $SSH_MATCHES" >> "$OUTPUT_DIR/lumina_output.log"
        echo "  - POST/exfil patterns: $POST_MATCHES" >> "$OUTPUT_DIR/lumina_output.log"
        echo "  - Credential patterns: $CRED_MATCHES" >> "$OUTPUT_DIR/lumina_output.log"
        echo "  - Network patterns: $NET_MATCHES" >> "$OUTPUT_DIR/lumina_output.log"
        echo "  - Cleanup patterns: $CLEANUP_MATCHES" >> "$OUTPUT_DIR/lumina_output.log"
        ...
        if [ "$SSH_MATCHES" -gt 0 ]; then
          if [ 95 -gt $HIGHEST_SCORE ]; then
            ...
            echo "🚨 CRITICAL THREAT DETECTED: SSH key access in external script: $URL" >> "$OUTPUT_DIR/lumina_output.log"
          fi
        fi
        ...
        if [ "$POST_MATCHES" -gt 0 ]; then
          if [ 90 -gt $HIGHEST_SCORE ]; then
            ...
            echo "🚨 CRITICAL THREAT DETECTED: Data exfiltration in external script: $URL" >> "$OUTPUT_DIR/lumina_output.log"
          fi
        fi
        ...
        if [ "$CRED_MATCHES" -gt 0 ] && [ "$NET_MATCHES" -gt 0 ]; then
          if [ 85 -gt $HIGHEST_SCORE ]; then
            ...
            echo "🚨 CRITICAL THREAT DETECTED: Credential theft in external script: $URL" >> "$OUTPUT_DIR/lumina_output.log"
          fi
        fi
        ...
        if [ "$CLEANUP_MATCHES" -gt 0 ]; then
          if [ 75 -gt $HIGHEST_SCORE ]; then
            ...
            echo "⚠️ HIGH RISK DETECTED: Stealth cleanup operations in external script: $URL" >> "$OUTPUT_DIR/lumina_output.log"
          fi
        fi
        ...
        if [ "$PKG_MATCHES" -gt 0 ] && [ 40 -gt $HIGHEST_SCORE ]; then
          ...
          echo "📦 MEDIUM RISK DETECTED: Package installation in external script: $URL" >> "$OUTPUT_DIR/lumina_output.log"
        fi
        
      else
        CURL_EXIT_CODE=$?
        echo "⚠️ Failed to download: $URL (curl exit code: $CURL_EXIT_CODE)" >> "$OUTPUT_DIR/lumina_output.log"
        echo "🔍 Trying alternative download method..." >> "$OUTPUT_DIR/lumina_output.log"
        
        # Try with wget as fallback
        if command -v wget >/dev/null 2>&1 && wget --timeout=30 --tries=2 -q "$URL" -O "$SCRIPT_FILE" 2>>"$OUTPUT_DIR/lumina_output.log"; then
          echo "✅ Downloaded via wget: $URL" >> "$OUTPUT_DIR/lumina_output.log"
          SCRIPT_ANALYZED=true
          # Repeat analysis logic here if needed
        else
          echo "❌ All download methods failed for: $URL" >> "$OUTPUT_DIR/lumina_output.log"
...
TAIL_OUT="$OUTPUT_DIR/lumina_output_tail.txt"
tail -n 300 "$OUTPUT_DIR/lumina_output.log" > "$TAIL_OUT" 2>/dev/null || true
...
jq -n \
  ...
  --argjson lumina_exit_code ${SCAN_EXIT_CODE:-0} \
  --arg analysis_method "${ANALYSIS_METHOD}" \
  '{status: $status, reason: $reason, risk_score: $risk_score, risk_level: $risk_level, message: $message, scan_successful: $scan_successful, analysis_method: $analysis_method, lumina_exit_code: $lumina_exit_code}' \
  > "$OUTPUT_DIR/scan_status.json"
...
- Lumina Exit Code: $SCAN_EXIT_CODE
- Timestamp: $TIMESTAMP

## Evidence

$(jq -r '.evidence[]? | "- " + (.category // "evidence") + ": " + (.snippet // "")' "$OUTPUT_DIR/lumina_result.json" 2>/dev/null || echo "See lumina_result.json for detailed evidence")

## Artifacts

- scan_status.json - Machine-readable scan status
- lumina_result.json - Complete analysis results
- lumina_output.log - Full analysis execution log
...
cat > "$OUTPUT_DIR/summary.txt" << EOF
🔍 Lumina Recipe Security Analysis Summary
========================================
...
📋 Available Reports:
  ...
  • lumina_result.json - Analysis results
  • lumina_output.log - Log
