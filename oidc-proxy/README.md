...
| `OIDC_AUDIENCE` | The audience your workflow requests (e.g. `lumina-oidc-proxy`) |
...
steps:
  - name: Get OIDC token
    ...
    with:
      script: |
        const token = await core.getIDToken('lumina-oidc-proxy');
  ...
  - name: Call API through proxy
    ...
    run: lumina run --recipe my-recipe.yaml
```
