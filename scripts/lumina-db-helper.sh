...
BACKUP_DIR="${HOME}/.local/share/lumina/lumina-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/lumina/migrations"
RUST_SESSION_MANAGER="crates/lumina/src/session/session_manager.rs"
...
get_lumina_db_path() {
    if [[ -n "${LUMINA_PATH_ROOT:-}" ]]; then
        echo "${LUMINA_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/lumina/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/lumina/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== Lumina Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run Lumina yet."
        echo "Once you run Lumina, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the lumina repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}Lumina Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/lumina/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
