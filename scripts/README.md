# Lumina Benchmark Scripts

This directory contains scripts for running and analyzing Lumina benchmarks.

## run-benchmarks.sh

This script runs Lumina benchmarks across multiple provider:model pairs and analyzes the results.

### Prerequisites

- Lumina CLI must be built or installed
...
3. For each provider:model pair:
   - Sets the `LUMINA_PROVIDER` and `LUMINA_MODEL` environment variables
