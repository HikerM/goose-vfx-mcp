...
REPO="${LUMINA_GITHUB_REPO:-HikerM/lumina}"
DEST="$HOME/Downloads"
TMPDIR=$(mktemp -d)
...
if ! curl -fSL --connect-timeout 10 -o "$TMPDIR/lumina.zip" "$DOWNLOAD_URL" 2>/dev/null; then
    ...
    gh api "repos/$REPO/actions/artifacts/$ARTIFACT_ID/zip" > "$TMPDIR/lumina.zip"
fi
echo "Done."

unzip -o -q "$TMPDIR/lumina.zip" -d "$TMPDIR/extracted"
