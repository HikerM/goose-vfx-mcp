#!/usr/bin/env bash
# run-benchmarks.sh - Script to run lumina benchmarks across multiple provider:model pairs
...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting LUMINA_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets LUMINA_TOOLSHIM_MODEL)"
...
LUMINA_CMD="lumina"
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/lumina" ]; then
    LUMINA_CMD="./target/debug/lumina"
    echo "Using debug binary: $LUMINA_CMD"
  else
    echo "Warning: Debug binary not found at ./target/debug/lumina. Falling back to system-installed lumina."
  fi
else
  if [ -f "./target/release/lumina" ]; then
    LUMINA_CMD="./target/release/lumina"
    echo "Using release binary: $LUMINA_CMD"
  else
    echo "Warning: Release binary not found at ./target/release/lumina. Falling back to system-installed lumina."
  fi
fi
...
for ((i=0; i<$COUNT; i++)); do
  ...
  export LUMINA_PROVIDER="$provider"
  export LUMINA_MODEL="$model"
  
  # Set toolshim environment variables if enabled
  if [ "$TOOLSHIM" = true ]; then
    export LUMINA_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export LUMINA_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $LUMINA_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
