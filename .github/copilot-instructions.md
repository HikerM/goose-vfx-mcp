...
- Core crates: `lumina` (agent logic and ACP server), `lumina-cli` (CLI), `lumina-mcp` (MCP servers)
- Error handling: Use `anyhow::Result`, not `unwrap()` in production code
- Async runtime: tokio
- MCP protocol implementations require extra scrutiny
- Naming convention: In `documentation/docs` and `documentation/blog`, always refer to the project as "lumina" (lowercase), never "Lumina" (even at the start of sentences)
