...
(notably `n_cache_tokens`, `n_output_tokens`, `cost_usd` on some lumina
...
harbor's fallbacks (reading lumina's `complete` event from `agent/lumina.txt`
...
- `$TRIAL_DIR/agent/<harness>.txt` — raw stream-json or log. The filename
  matches the harness (commonly `lumina.txt` or `pi.txt`; other harnesses
