"""Harbor agent that runs a caller-provided Lumina binary inside the task container."""
...
from harbor.agents.installed.lumina import Lumina
...
CONTAINER_LUMINA_PATH_ROOT = "/installed-agent/lumina-profile"
CONTAINER_CONFIG_PATH = f"{CONTAINER_LUMINA_PATH_ROOT}/config/config.yaml"
CONTAINER_RECIPE_PATH = "/installed-agent/harbor-recipe.yaml"
CONTAINER_CA_BUNDLE_PATH = "/installed-agent/ca-certificates.crt"

FATAL_LUMINA_NOTIFICATIONS = ("creditsExhausted",)


class LuminaBinaryAgent(Lumina):
    """Run a caller-provided Lumina binary in the benchmark environment.

    Differs from harbor's vanilla ``Lumina``:
    ...
    def __init__(
        ...
        *args,
        ...
        install_lumina_runtime_deps: bool = False,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.lumina_binary = Path(lumina_binary).expanduser().resolve()
        self.config_yaml = config_yaml
        self.extension_entries = extension_entries
        self.install_lumina_runtime_deps = install_lumina_runtime_deps
        self.ca_bundle_env_path: str | None = None

    @staticmethod
    def name() -> str:
        return "lumina-binary"

    def get_version_command(self) -> str | None:
        return "/installed-agent/lumina --version"
    ...
    def _run_env(self) -> dict[str, str]:
        ...
        env = {
            "LUMINA_MODEL": model,
            "LUMINA_PROVIDER": provider,
            "LUMINA_TELEMETRY_ENABLED": "false",
            "LUMINA_TELEMETRY_OFF": "true",
            "CONFIGURE": "false",
            "LUMINA_PATH_ROOT": CONTAINER_LUMINA_PATH_ROOT,
    ...
    async def _install_lumina_runtime_deps(self, environment: BaseEnvironment) -> None:
        await self.exec_as_root(
            environment,
            command=(
                "command -v apt-get >/dev/null 2>&1 || "
                "(echo 'install_lumina_runtime_deps requires apt-get in the task container' >&2; exit 1); "
                "apt-get update && "
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        if not self.lumina_binary.is_file():
            raise FileNotFoundError(f"Lumina binary does not exist: {self.lumina_binary}")

        await environment.upload_file(self.lumina_binary, "/installed-agent/lumina")
        await self.exec_as_root(environment, command="chmod 755 /installed-agent/lumina")
        if self.install_lumina_runtime_deps:
            await self._install_lumina_runtime_deps(environment)
        await self._ensure_ca_bundle(environment)

        config_dir = f"{CONTAINER_LUMINA_PATH_ROOT}/config"
        await self.exec_as_root(
        ...
        await self._chown_to_agent_user(environment, CONTAINER_LUMINA_PATH_ROOT, recursive=True)

        await self.exec_as_agent(
            environment,
            command=(
                "mkdir -p ~/.local/bin && "
                "ln -sf /installed-agent/lumina ~/.local/bin/lumina && "
                "~/.local/bin/lumina --version"
            ),
            env={
                "LUMINA_DISABLE_KEYRING": "true",
                "LUMINA_TELEMETRY_ENABLED": "false",
    ...
    async def run(
        ...
        await self.exec_as_agent(
            ...
            command=(
                ...
                f"lumina run --recipe {shlex.quote(CONTAINER_RECIPE_PATH)} "
                "--output-format stream-json "
                + ((cli_flags + " ") if cli_flags else "")
                + "2>&1 | stdbuf -oL tee /logs/agent/lumina.txt"
            ),
            env=env,
        )
        self._raise_on_fatal_lumina_notification()

    def _raise_on_fatal_lumina_notification(self) -> None:
        log_path = self.logs_dir / "lumina.txt"
        if not log_path.is_file():
            return
        log_text = log_path.read_text(errors="replace")
        for notification in FATAL_LUMINA_NOTIFICATIONS:
            if f'"notificationType":"{notification}"' in log_text:
                raise NonZeroAgentExitCodeError(
                    f"Lumina exited without running the task: {notification}. "
    ...
    def populate_context_post_run(self, context: AgentContext) -> None:
        ...
        txt_path = self.logs_dir / "lumina.txt"
