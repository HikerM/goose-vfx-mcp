...
const MODELSCOPE_REQUEST_TIMEOUT: Duration = Duration::from_secs(8);
const MODELSCOPE_CONNECT_TIMEOUT: Duration = Duration::from_secs(5);
const MODELSCOPE_METADATA_ATTEMPTS: usize = 2;
...
fn quant_info(quant: &str) -> QuantInfo {
    QUANT_TABLE
        .iter()
        .find(|(name, _, _)| name.eq_ignore_ascii_case(quant))
...
fn modelscope_metadata_client() -> Result<reqwest::Client> {
    static CLIENT: OnceLock<reqwest::Client> = OnceLock::new();
    if let Some(client) = CLIENT.get() {
        return Ok(client.clone());
    }
    let client = reqwest::Client::builder()
        .connect_timeout(MODELSCOPE_CONNECT_TIMEOUT)
        .timeout(MODELSCOPE_REQUEST_TIMEOUT)
        .build()?;
...
fn is_model_file(filename: &str, model_stem_lower: &str) -> bool {
    ...
    !is_auxiliary
        && (basename.starts_with(model_stem_lower) || parse_quantization(filename) != "unknown")
}
...
pub async fn search_local_models(query: &str, limit: usize) -> Result<Vec<HfModelInfo>> {
    ...
    let mut results = search_gguf_models(query, limit).await?;
    results.sort_by(|a, b| {
...
pub async fn search_gguf_models(query: &str, limit: usize) -> Result<Vec<HfModelInfo>> {
    let queries = modelscope_search_queries(query);
    let mut results = Vec::new();
    let mut completed_request = false;
    for query in queries {
        match search_modelscope_models_page(&query, limit).await {
            Ok(models) => {
                completed_request = true;
                results.extend(models);
            }
            Err(_) => tracing::warn!(event = "modelscope_catalog_search_attempt_failed"),
        }
    ...
    if !completed_request {
        bail!("ModelScope model search is temporarily unavailable");
    }
...
fn modelscope_search_queries(query: &str) -> Vec<String> {
    let normalized = query.split_whitespace().collect::<Vec<_>>().join(" ");
    let contains_gguf = normalized.to_ascii_lowercase().contains("gguf");
    let contains_gguf_token = normalized
        .split_whitespace()
        .any(|token| token.eq_ignore_ascii_case("gguf"));
    if contains_gguf {
        let fallback = normalized
            .split_whitespace()
            .filter(|token| !token.eq_ignore_ascii_case("gguf"))
            .collect::<Vec<_>>()
            .join(" ");
        if contains_gguf_token && !fallback.is_empty() && fallback != normalized {
            return vec![normalized, fallback];
        }
        return vec![normalized];
    }
    vec![format!("{normalized} GGUF"), normalized]
}

async fn search_modelscope_models_page(query: &str, limit: usize) -> Result<Vec<HfModelInfo>> {
    ...
    let token = modelscope_token()?;
    let client = modelscope_metadata_client()?;
    let mut last_failure = "request did not complete".to_string();
    for attempt in 1..=MODELSCOPE_METADATA_ATTEMPTS {
        let response = apply_modelscope_auth(
            client
                .get(url.clone())
                .header("User-Agent", "lumina-ai-agent"),
        ...
        match response {
            Ok(response) if response.status().is_success() => {
                match response.json::<ModelScopeSearchResponse>().await {
                    Ok(payload) if payload.success => return Ok(modelscope_models(payload)),
                    Ok(_) => last_failure = "catalog response reported failure".to_string(),
                    Err(error) => last_failure = format!("invalid catalog response: {error}"),
                }
            }
            Ok(response)
            ...
            Ok(response) => bail!("ModelScope search returned HTTP {}", response.status()),
            Err(error) => last_failure = error.to_string(),
        }

        if attempt < MODELSCOPE_METADATA_ATTEMPTS {
            tokio::time::sleep(Duration::from_millis(250 * attempt as u64)).await;
        }
    }

    bail!(
        "ModelScope search is temporarily unavailable after {} attempts: {}",
        MODELSCOPE_METADATA_ATTEMPTS,
        last_failure
    )
}

fn modelscope_models(payload: ModelScopeSearchResponse) -> Vec<HfModelInfo> {
    payload
        .data
...
mod tests {
    ...
    fn test_is_model_file() {
        ...
        assert!(is_model_file("model-Q4_K_M.gguf", stem));
        assert!(is_model_file(
            "BF16/gemma-3-27b-it-BF16-00001-of-00002.gguf",
            stem
        ));
        assert!(!is_model_file("mmproj-BF16.gguf", stem));
        assert!(!is_model_file("model-projector-Q4_K_M.gguf", stem));
        assert!(!is_model_file("vision-encoder-Q4_K_M.gguf", stem));
    }

    #[test]
    fn modelscope_search_queries_prioritize_gguf_and_normalize_whitespace() {
        assert_eq!(
            modelscope_search_queries("  qwen   3 4b "),
            vec!["qwen 3 4b GGUF", "qwen 3 4b"]
        );
        assert_eq!(
            modelscope_search_queries("Qwen/Qwen3-4B-GGUF"),
            vec!["Qwen/Qwen3-4B-GGUF"]
        );
    }

    #[test]
    fn model_search_rank_matches_separator_insensitive_queries() {
        let exact = HfModelInfo {
            repo_id: "Qwen/Qwen3-4B-GGUF".to_string(),
            author: "Qwen".to_string(),
            model_name: "Qwen3-4B-GGUF".to_string(),
            downloads: 1,
            gguf_files: Vec::new(),
            variants: Vec::new(),
        };
        let unrelated = HfModelInfo {
            repo_id: "someone/Qwen2-72B-GGUF".to_string(),
            author: "someone".to_string(),
            model_name: "Qwen2-72B-GGUF".to_string(),
            downloads: 1_000_000,
            gguf_files: Vec::new(),
            variants: Vec::new(),
        };
        ...
        assert!(
            model_search_rank("qwen 3 4b", &exact) < model_search_rank("qwen 3 4b", &unrelated)
        );
    }

    #[test]
    fn conversational_search_prefers_instruction_models() {
        let base = HfModelInfo {
            repo_id: "Qwen/Qwen2.5-Coder-1.5B-GGUF".to_string(),
            author: "Qwen".to_string(),
            model_name: "Qwen2.5-Coder-1.5B-GGUF".to_string(),
            downloads: 10_000,
            gguf_files: Vec::new(),
            variants: Vec::new(),
        };
        let instruct = HfModelInfo {
            repo_id: "Qwen/Qwen2.5-Coder-1.5B-Instruct-GGUF".to_string(),
            author: "Qwen".to_string(),
            model_name: "Qwen2.5-Coder-1.5B-Instruct-GGUF".to_string(),
            downloads: 1,
            gguf_files: Vec::new(),
            variants: Vec::new(),
        };
        ...
        assert!(
            model_search_rank("Qwen2.5 Coder 1.5B GGUF", &instruct)
                < model_search_rank("Qwen2.5 Coder 1.5B GGUF", &base)
        );
        assert!(
            model_search_rank("Qwen2.5 Coder 1.5B base GGUF", &base)
                < model_search_rank("Qwen2.5 Coder 1.5B base GGUF", &instruct)
        );
    }

    #[test]
    fn quantization_metadata_is_case_insensitive() {
        let upper = quant_info("Q4_K_M");
        let lower = quant_info("q4_k_m");

        assert_eq!(lower.quality_rank, upper.quality_rank);
        assert_eq!(lower.description, upper.description);
    }
...
fn model_search_rank(query: &str, model: &HfModelInfo) -> (u8, u8, u8) {
    let query = query.trim().to_lowercase();
    let repo_id = model.repo_id.to_lowercase();
    let model_name = model.model_name.to_lowercase();
    let compact_query = normalize_model_search_text(&query);
    let compact_repo_id = normalize_model_search_text(&repo_id);
    let compact_model_name = normalize_model_search_text(&model_name);
    let tokens = query
        .split(|character: char| !character.is_alphanumeric())
        .filter(|token| !token.is_empty())
        .collect::<Vec<_>>();

    let lexical_rank = if repo_id == query {
        0
    } else if model_name == query || compact_model_name == compact_query {
        1
    } else if repo_id.ends_with(&format!("/{query}")) {
        2
    } else if !tokens.is_empty() && tokens.iter().all(|token| repo_id.contains(token)) {
        3
    } else if !compact_query.is_empty() && compact_repo_id.contains(&compact_query) {
        4
    ...
    let query_requests_base = tokens
        .iter()
        .any(|token| matches!(*token, "base" | "pretrain" | "pretrained"));
    let query_requests_chat = tokens
        .iter()
        .any(|token| matches!(*token, "chat" | "instruct" | "instruction" | "assistant"));
    let chat_ready = is_chat_ready_model(&model_name);
    let intent_rank = if query_requests_base {
        u8::from(chat_ready)
    } else if query_requests_chat || !looks_like_repo_id(&query) {
        u8::from(!chat_ready)
    } else {
        0
    };

    (1, intent_rank, lexical_rank)
}

fn is_chat_ready_model(model_name: &str) -> bool {
    ...
    normalized.iter().any(|token| {
        matches!(
            *token,
            "chat" | "instruct" | "instruction" | "assistant" | "it"
        )
    })
}
...
async fn resolve_gguf_model(repo_id: &str, quantization: &str) -> Result<ResolvedLocalModel> {
    ...
    Ok(ResolvedLocalModel::Gguf {
        ...
        local_paths,
        mmproj_path,
        storage: LocalModelStorage::LuminaManaged,
    })
}
