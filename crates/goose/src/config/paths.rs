...
pub(crate) const WINDOWS_DEFAULT_GOOSE_PATH_ROOT: &str = r"D:\Goose";
...
impl Paths {
    ...
    pub fn ensure_windows_governed_root() -> Result<(), WindowsGoosePathRootError> {
        #[cfg(windows)]
        {
            let inspector = FsWindowsPathRootInspector;
            let configured_root = std::env::var("GOOSE_PATH_ROOT").ok();
            if let Some(path) = resolve_goose_path_root_for_platform(
                GovernedRootPlatform::Windows,
                configured_root.as_deref(),
                &inspector,
            )? {
                unsafe { std::env::set_var("GOOSE_PATH_ROOT", path) };
            }
        }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum WindowsGoosePathRootErrorKind {
    InvalidPath,
    DriveUnavailable,
    NotWritable,
    ReparseRisk,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct WindowsGoosePathRootError {
    kind: WindowsGoosePathRootErrorKind,
    message: &'static str,
}

impl WindowsGoosePathRootError {
    pub const fn kind(self) -> WindowsGoosePathRootErrorKind {
        self.kind
    }

    #[cfg(test)]
    pub const fn new_for_test(kind: WindowsGoosePathRootErrorKind, message: &'static str) -> Self {
        Self { kind, message }
    }
}

impl std::fmt::Display for WindowsGoosePathRootError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(self.message)
    }
}

impl std::error::Error for WindowsGoosePathRootError {}

fn invalid_windows_goose_path_root() -> WindowsGoosePathRootError {
...
fn unavailable_windows_goose_path_root() -> WindowsGoosePathRootError {
    WindowsGoosePathRootError {
...
fn unwritable_windows_goose_path_root() -> WindowsGoosePathRootError {
    WindowsGoosePathRootError {
...
fn reparse_windows_goose_path_root() -> WindowsGoosePathRootError {
    WindowsGoosePathRootError {
    ...
    match platform {
        ...
        GovernedRootPlatform::Windows => {
            let candidate = configured_root
                .map(str::trim)
                .filter(|value| !value.is_empty())
                .unwrap_or(WINDOWS_DEFAULT_GOOSE_PATH_ROOT);
            let normalized = validate_windows_goose_path_root(candidate)?;
...
fn validate_windows_goose_path_root(value: &str) -> Result<String, WindowsGoosePathRootError> {
    if value.starts_with(r"\\") || value.starts_with("//") {
        return Err(invalid_windows_goose_path_root());
    }
...
trait WindowsPathRootInspector {
    fn ensure_existing_chain_safe(
        &self,
        normalized_root: &str,
    ) -> Result<(), WindowsGoosePathRootError>;
...
impl WindowsPathRootInspector for FsWindowsPathRootInspector {
    ...
    fn ensure_writable_directory(
        ...
        std::fs::create_dir_all(path).map_err(|_| unwritable_windows_goose_path_root())?;
        ...
        OpenOptions::new()
            ...
            .create_new(true)
            .open(&probe)
            .map_err(|_| unwritable_windows_goose_path_root())?;
        std::fs::remove_file(&probe).map_err(|_| unwritable_windows_goose_path_root())?;
        Ok(())
    }
}

#[cfg(windows)]
fn ensure_existing_directory_is_safe(path: &Path) -> Result<(), WindowsGoosePathRootError> {
    let metadata =
        std::fs::symlink_metadata(path).map_err(|_| unavailable_windows_goose_path_root())?;
    if !metadata.is_dir() {
        return Err(unavailable_windows_goose_path_root());
    }
    if is_reparse_point(&metadata) {
        return Err(reparse_windows_goose_path_root());
    }
    Ok(())
}
...
fn inspect_windows_directory_chain(
    ...
    match lookup(&current) {
        ...
        WindowsDirectoryStatus::Reparse => return Err(reparse_windows_goose_path_root()),
        WindowsDirectoryStatus::Missing | WindowsDirectoryStatus::Unavailable => {
            return Err(unavailable_windows_goose_path_root())
        }
        WindowsDirectoryStatus::NotDirectory => return Err(unwritable_windows_goose_path_root()),
    }
    ...
    for segment in normalized_root[3..].split('\\') {
        ...
        match lookup(&current) {
            ...
            WindowsDirectoryStatus::NotDirectory => {
                return Err(unwritable_windows_goose_path_root())
            }
            WindowsDirectoryStatus::Missing if allow_missing_tail => break,
            WindowsDirectoryStatus::Missing => return Err(unwritable_windows_goose_path_root()),
...
mod tests {
    ...
    struct FakeWindowsPathRootInspector {
        existing_chain_error: Option<WindowsGoosePathRootError>,
        writable_error: Option<WindowsGoosePathRootError>,
    ...
    fn windows_defaults_to_governed_d_root_when_unset() {
        let resolved = resolve_goose_path_root_for_platform(
            GovernedRootPlatform::Windows,
            None,
        ...
        assert_eq!(resolved, PathBuf::from(WINDOWS_DEFAULT_GOOSE_PATH_ROOT));
    }
    ...
    fn windows_accepts_explicit_d_subdirectories() {
        let resolved = resolve_goose_path_root_for_platform(
            GovernedRootPlatform::Windows,
            Some(r"d:/Goose Team/storage"),
            &FakeWindowsPathRootInspector::default(),
        )
        .unwrap()
        .unwrap();

        assert_eq!(resolved, PathBuf::from(r"D:\Goose Team\storage"));
    }
    ...
    fn windows_checks_existing_chain_before_writable_probe() {
        ...
        let _ = resolve_goose_path_root_for_platform(
            GovernedRootPlatform::Windows,
            Some(r"D:\Goose\storage"),
            &inspector,
        )
        .unwrap();
    ...
    fn windows_rejects_c_relative_unc_and_drive_root_paths() {
        for candidate in [r"C:\Goose", r"Goose", r"\\server\share\goose", r"D:\"] {
            let error = resolve_goose_path_root_for_platform(
                GovernedRootPlatform::Windows,
                Some(candidate),
                &FakeWindowsPathRootInspector::default(),
            )
            ...
            assert_eq!(error.kind(), WindowsGoosePathRootErrorKind::InvalidPath);
        }
    }
    ...
    fn windows_rejects_unavailable_d_drive_and_unwritable_targets() {
        let unavailable = FakeWindowsPathRootInspector {
            existing_chain_error: Some(unavailable_windows_goose_path_root()),
            writable_error: None,
            ..Default::default()
        };
        let unavailable_error =
            resolve_goose_path_root_for_platform(GovernedRootPlatform::Windows, None, &unavailable)
                .unwrap_err();
        assert_eq!(
            unavailable_error.kind(),
            WindowsGoosePathRootErrorKind::DriveUnavailable
        );

        let unwritable = FakeWindowsPathRootInspector {
            existing_chain_error: None,
            writable_error: Some(unwritable_windows_goose_path_root()),
            ..Default::default()
        };
        let unwritable_error = resolve_goose_path_root_for_platform(
            GovernedRootPlatform::Windows,
            Some(r"D:\Goose\storage"),
            &unwritable,
        )
        ...
        assert_eq!(
            ...
            WindowsGoosePathRootErrorKind::NotWritable
        );
    }
    ...
    fn windows_rejects_reparse_risks() {
        let inspector = FakeWindowsPathRootInspector {
            existing_chain_error: Some(reparse_windows_goose_path_root()),
            writable_error: None,
            ..Default::default()
        };
        ...
        assert_eq!(error.kind(), WindowsGoosePathRootErrorKind::ReparseRisk);
    }

    #[test]
    fn windows_post_create_chain_rejects_missing_or_reparse_ancestors() {
        let missing =
            inspect_windows_directory_chain(r"D:\Goose\storage", false, |path| {
                match path.to_string_lossy().as_ref() {
                    r"D:\" => WindowsDirectoryStatus::Directory,
                    r"D:\Goose" => WindowsDirectoryStatus::Missing,
                    _ => WindowsDirectoryStatus::Directory,
                }
            })
            .unwrap_err();
        assert_eq!(missing.kind(), WindowsGoosePathRootErrorKind::NotWritable);

        let reparse =
            inspect_windows_directory_chain(r"D:\Goose\storage", false, |path| {
                match path.to_string_lossy().as_ref() {
                    r"D:\" => WindowsDirectoryStatus::Directory,
                    r"D:\Goose" => WindowsDirectoryStatus::Reparse,
                    _ => WindowsDirectoryStatus::Directory,
                }
            })
            .unwrap_err();
        assert_eq!(reparse.kind(), WindowsGoosePathRootErrorKind::ReparseRisk);
    }

    #[test]
    fn non_windows_keeps_existing_root_and_does_not_default() {
        let configured = resolve_goose_path_root_for_platform(
            GovernedRootPlatform::Other,
            Some("/tmp/goose"),
            &FakeWindowsPathRootInspector::default(),
        )
        .unwrap();
        let unset = resolve_goose_path_root_for_platform(
            GovernedRootPlatform::Other,
            None,
            &FakeWindowsPathRootInspector::default(),
        )
        ...
        assert_eq!(configured, Some(PathBuf::from("/tmp/goose")));
