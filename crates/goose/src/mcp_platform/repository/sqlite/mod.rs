...
fn normalize_in_memory_integrity_signer(
    ...
    let digest = Sha256::digest(b"goose.mcp-platform.in-memory-path-binding-v1");
...
fn checkpoint_root(
    ...
    integrity::digest(&[
        b"goose.mcp-platform.commit-root",
        b"v1",
...
fn checkpoint_payload(
    ...
    match format {
        IntegrityAnchorFormat::LegacyV1 => integrity::canonical_fields(&[
            b"goose.mcp-platform.anchored-commit",
            b"v1",
...
async fn security_state_digest(
    ...
    hash_component(&mut hasher, b"goose.mcp-platform.security-state-v1");
    for row in schema {
...
fn move_file_without_replace_with_state(
    ...
    std::fs::remove_file(source).map_err(|error| MoveFileWithoutReplaceFailure {
        error,
        final_created: true,
    })
}
...
fn database_path_binding_from_resolved_path(path: &Path) -> McpPlatformResult<String> {
    ...
    #[cfg(unix)]
    {
        ...
        hash_component(&mut hasher, b"goose.mcp-platform.path-binding.unix-v1");
    ...
    #[cfg(windows)]
    {
        let normalized = path
            ...
            .replace('/', "\\");
        hash_component(&mut hasher, b"goose.mcp-platform.path-binding.windows-v1");
    ...
    #[cfg(not(any(unix, windows)))]
    {
        ...
        hash_component(&mut hasher, b"goose.mcp-platform.path-binding.portable-v1");
        hash_component(&mut hasher, normalized.as_bytes());
    }
    Ok(crate::utils::bytes_to_hex(hasher.finalize()))
}
