...
fn refresh_plan_digest(plan: &mut serde_json::Value) {
    ...
    plan["plan_digest"] =
        serde_json::Value::String(goose::utils::bytes_to_hex(Sha256::digest(canonical)));
}
...
async fn projection_json_tampering_stales_plan_confirm_consume_hydrate_and_runtime() {
    ...
    let tampered_digest =
        goose::mcp_platform::projection_config_digest(&tampered_projection).unwrap();
    ...
    let injected_json =
        goose::mcp_platform::encode_projection_config(&injected_projection).unwrap();
    let injected_digest =
        goose::mcp_platform::projection_config_digest(&injected_projection).unwrap();
    sqlx::query(
...
async fn durable_projection_rollback_validates_plan_and_digest_before_restore_effects() {
    ...
    let restored_digest =
        goose::mcp_platform::projection_config_digest(&restored_projection).unwrap();
    ...
    let current_config = CoreTransportProjectionAdapter
        ...
        .unwrap();
    let current_digest =
        goose::mcp_platform::projection_config_digest(&current_projection).unwrap();
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect_with(SqliteConnectOptions::new().filename(&database_path))
        .await
