...
/// The fixed, Goose-owned names a runtime writer may address.
///
...
impl RuntimeStorageWriterLease {
    ...
    pub(in crate::mcp_platform) fn remove_goose_owned_contents(&mut self) -> McpPlatformResult<()> {
        self.require_state(RuntimeWriterLeaseState::Fresh)?;
        self.witness.remove_goose_owned_contents()?;
        self.state = RuntimeWriterLeaseState::Retired;
        Ok(())
    }
