...
async fn batch2_2_remote_generation_and_tuple_tamper_fail_closed() {
    ...
    let private_reference = repository
        .mint_intake_private_reference(
            goose::mcp_platform::intake::IntakePrivateReferenceKind::ManualHttpsEndpoint,
        )
        .unwrap();
    repository
        .save_intake_configuration_ref(goose::mcp_platform::intake::SaveIntakeConfigurationRef {
    ...
    let candidate = repository
        .save_intake_candidate(goose::mcp_platform::intake::SaveIntakeCandidate {
            candidate_id: "candidate_b22_remote_tamper",
            submission_binding: &"3".repeat(64),
            source_facet: goose::mcp_platform::intake::IntakeSourceFacet::ManualHttpsCandidate,
            transport: goose::mcp_platform::intake::IntakeTransport::StreamableHttp,
            initial_state: goose::mcp_platform::intake::IntakeLifecycleState::AwaitingConsent,
    ...
    let policy_revision =
        goose::mcp_platform::intake_remote_inspection::trusted_remote_inspection_policy_revision(
            purpose,
        )
        .unwrap();
    ...
    let proof = authority.verify_no_live_proof(
        ...
        reservation.generation,
        "intent_tamper",
        goose::mcp_platform::intake_remote_inspection::RemoteInspectionNoLiveProofKind::Tombstoned,
        14,
    );
...
fn batch2_1_manual_https_prevalidation_accepts_and_rejects_samples() {
    let valid = goose::mcp_platform::intake_remote_inspection::canonicalize_manual_https_target(
    ...
    let rejected = goose::mcp_platform::intake_remote_inspection::canonicalize_manual_https_target(
        "https://127.0.0.1/mcp",
    )
    .unwrap_err();
    assert_eq!(
        rejected.subcode,
        goose::mcp_platform::intake_remote_inspection::RemoteInspectionSafeSubcode::IpLiteralRejected
    );
    ...
    for target in [
        ...
        let rejected =
            goose::mcp_platform::intake_remote_inspection::canonicalize_manual_https_target(target)
                .unwrap_err();
        assert_eq!(
            rejected.subcode,
            goose::mcp_platform::intake_remote_inspection::RemoteInspectionSafeSubcode::InvalidUrl
        );
    }
}
...
async fn batch2_1_remote_attempts_enforce_uniqueness_and_finalize_fail_closed() {
    ...
    assert_eq!(
        stale.state,
        goose::mcp_platform::intake_remote_inspection::RemoteInspectionAttemptState::Finalized
    );
