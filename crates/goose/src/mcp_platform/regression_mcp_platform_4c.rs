...
use goose::mcp_platform::lifecycle::{CoreTransportProjectionAdapter, TransportProjectionAdapter};
use goose::mcp_platform::manifest::Transport;
use goose::mcp_platform::remote_https_fixture::RemoteHttpsFixture;
use goose::mcp_platform::remote_https_test_policy::{
...
impl goose::mcp_platform::IdGenerator for Ids {
...
async fn https_install_reaches_real_fixture_mcp_tool_call() {
    ...
    let expected_url_id =
        goose::utils::bytes_to_hex(Sha256::digest(fixture.manifest_url.as_bytes()));
    ...
    assert_eq!(prepared.preview.manifest_id, "goose.fixture");
    ...
    assert_eq!(review.manifest.id, "goose.fixture");
    ...
    let managed = managed
        .items
        .iter()
        .find(|item| item.mcp_id == "goose.fixture")
        .expect("successful install missing managed projection");
    assert_eq!(managed.active_version.as_deref(), Some("1.0.0"));
    assert_eq!(
        managed.installation,
        goose::mcp_platform::InstallationState::Installed
    );
    assert_eq!(
        managed.registration,
        goose::mcp_platform::RegistrationState::Registered
    );
    ...
    assert_eq!(
        ...
        goose::mcp_platform::projection_config_digest(&projection.projection).unwrap()
    );
