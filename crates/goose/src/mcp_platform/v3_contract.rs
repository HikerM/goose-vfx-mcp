...
pub(crate) const MANIFEST_DOCUMENT_TYPE: &str = "goose.mcp.package-manifest";
pub(crate) const AUTHORIZATION_DOCUMENT_TYPE: &str = "goose.mcp.install-authorization";
pub(crate) const ADAPTER_CONTRACT_DOCUMENT_TYPE: &str = "goose.mcp.adapter-capability-contract";
...
mod tests {
    ...
    fn legacy_document_types_are_not_accepted() {
        assert!(parse_untrusted_document(
            &manifest_json().replace(MANIFEST_DOCUMENT_TYPE, "goose.mcp.package-manifest-v2")
        )
        .is_err());
        assert!(parse_untrusted_document(
            "{\"document_type\":\"goose.mcp.package-manifest\",\"schema_version\":2}"
        )
        .is_err());
    }
}
