...
fn effect_fence_mutex_name(identity: &EffectFenceRepositoryIdentity) -> Vec<u16> {
    ...
    for value in [
        "goose.mcp-platform.effect-fence.v41",
        EFFECT_FENCE_SCOPE,
    ...
    format!("Local\\Goose.EffectFence.{digest}")
        .encode_utf16()
...
mod tests {
    ...
    fn effect_fence_child_protocol() {
        let Some(mode) = std::env::var_os("GOOSE_EFFECT_FENCE_CHILD_MODE") else {
            return;
        };
    ...
    fn assert_child_mutex_result(
        ...
        let mut child = match Command::new(executable)
            ...
            .env("GOOSE_EFFECT_FENCE_CHILD_MODE", mode)
            .spawn()
        {
            Ok(child) => child,
            Err(error) => {
