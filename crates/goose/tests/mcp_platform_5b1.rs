...
struct StaticProvenanceVerifier {
    provenance: goose::session::ManagedExtensionProvenance,
    available: bool,
    calls: Arc<AtomicUsize>,
}

#[async_trait]
impl goose::session::SessionExtensionProvenanceVerifier for StaticProvenanceVerifier {
    async fn verified_provenance(&self) -> Result<goose::session::ManagedExtensionProvenance> {
...
fn frontend_extension(name: &str, instructions: &str) -> goose::agents::ExtensionConfig {
...
async fn agent_direct_extension_path_rejects_managed_extension_name_before_live_sink() {
    ...
    let mut provenance = goose::session::ManagedExtensionProvenance::default();
    provenance.names = HashSet::from([managed.name()]);
    let session_manager = Arc::new(
        goose::session::SessionManager::new_with_provenance_verifier(
    ...
    let session = session_manager
        ...
        .await
        .unwrap();
    let agent = Arc::new(goose::agents::Agent::with_config(
        goose::agents::AgentConfig::new(
            session_manager,
            Arc::new(goose::config::permission::PermissionManager::new(
                temp_dir.path().to_path_buf(),
            )),
            None,
            goose::config::GooseMode::default(),
...
fn public_profile_views_remain_constructible_without_internal_hash_fields() {
    let profile = goose::mcp_platform::McpProfile {
        profile_id: "profile".to_string(),
        name: "Example".to_string(),
        description: "public view".to_string(),
        revision: 1,
        archived: false,
        entries: vec![goose::mcp_platform::ProfileEntry {
    ...
    let confirmation = goose::mcp_platform::ProfileApplyConfirmationView {
        confirmation_token: "profile_apply_confirmation_token".to_string(),
    };
    let token = goose::mcp_platform::ProfileApplicationTokenView {
...
async fn unavailable_provenance_keeps_plain_session_sink_free_and_fails_closed_on_add() {
    ...
    let session_manager = Arc::new(
        goose::session::SessionManager::new_with_provenance_verifier(
            temp_dir.path().to_path_buf(),
            Arc::new(StaticProvenanceVerifier {
                provenance: goose::session::ManagedExtensionProvenance::default(),
                available: false,
                calls: Arc::new(AtomicUsize::new(0)),
            }),
        ),
...
fn extension_data_with_profile_marker(
    extension: &goose::agents::ExtensionConfig,
    marker_name: &str,
) -> goose::session::ExtensionData {
    let mut extension_data = goose::session::ExtensionData::new();
    goose::session::EnabledExtensionsState::new(vec![extension.clone()])
...
async fn public_session_load_verifies_real_persisted_managed_record_and_rejects_tampering() {
    ...
    let control_seed_manager = goose::session::SessionManager::new(control_root.clone());
    let control_seed = control_seed_manager
        ...
        .await
        .unwrap();
    let tampered_seed_manager = goose::session::SessionManager::new(tampered_root.clone());
    ...
    let control_session_manager = Arc::new(
        goose::session::SessionManager::new_with_provenance_verifier(
            ...
            Arc::new(StaticProvenanceVerifier {
                provenance: {
                    let mut p = goose::session::ManagedExtensionProvenance::default();
    ...
    let control = control_session_manager
        ...
        .await
        .unwrap();
    let control_agent = Arc::new(goose::agents::Agent::with_config(
        goose::agents::AgentConfig::new(
    ...
    let tampered = tampered_session_manager
        ...
        .await
        .unwrap();
    let tampered_agent = Arc::new(goose::agents::Agent::with_config(
        goose::agents::AgentConfig::new(
