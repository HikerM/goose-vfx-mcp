...
) -> anyhow::Result<goose::custom_requests::McpTaskRef> {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        service
            .runner_tick()
            .await
            .map_err(|error| anyhow::anyhow!("ACP HTTPS E2E task runner tick failed: {error:?}"))?;
        ...
        match value.status {
            ...
            | McpTaskStatus::RecoveryRequired => {
                ...
                anyhow::bail!(
                    "install task did not succeed: status={:?}, error={:?}, redacted={diagnostic:?}",
...
async fn acp_https_install_profile_model_tool_call_round_trip_inner() -> anyhow::Result<()> {
    ...
    let first_exchange = include_str!("acp_test_data/openai_tool_call.txt");
    ...
    assert!(plan
        ...
        .any(|confirmation| matches!(
            confirmation,
            goose::custom_requests::McpHttpsProvisionConfirmation::Policy
        )));
    assert!(plan
        .permissions
        .iter()
    ...
    let connection_test: goose::custom_requests::McpProfileConnectionTestResponse = connection
        .cx()
    ...
    assert!(connection_test.stages.iter().any(|stage| {
        stage.phase == McpConnectionTestPhase::ToolVisibility
            && stage.code == goose::custom_requests::McpConnectionTestCode::ToolVisibilityValidated
    }));
        ...
        matches!(
            ...
            agent_client_protocol::schema::v1::SessionUpdate::ToolCallUpdate(update)
                if update.fields.status == Some(ToolCallStatus::Completed)
                    && update.fields.title.as_deref() == Some("mcp-fixture__get_code")
                    && update.fields.raw_output.as_ref()
                        == Some(&serde_json::Value::String(FIXTURE_CODE.to_owned()))
        )
    });
    assert!(
        completed_tool_call,
