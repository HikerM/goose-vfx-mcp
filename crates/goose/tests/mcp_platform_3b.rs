...
use goose::mcp_platform::{
    parse_manifest, validate_archive_entries, validate_archive_path, ArchiveEntryKind,
    ArchiveEntryMetadata, ArchiveInstaller, ArchiveLimits, ArtifactFetchEffect, ArtifactFetcher,
    ArtifactNetworkClient, ArtifactNetworkResponse, ArtifactSignatureStatus,
    ...
    ManagedInstallEffect, ManagedInstallOutcome, ManifestProof, ManifestRecord, McpPlatformError,
    McpPlatformErrorCode, McpPlatformResult, McpPlatformService, McpPlatformServiceOptions,
    PlanCreateInput, PlanIntent, PlanOperation, PolicyContext, ProductionArtifactFetcher,
    ProductionDistributionEffectAdapter, ProjectionSink, ProjectionSnapshot, RegistrationEffect,
    RegistrationEffectAdapter, RegistrationEffectEvidence, RequestContext, RuntimeCapabilities,
    RuntimeCapabilitySnapshot, SetDefaultEnabledInput, Sha256Verifier, SqliteMcpPlatformRepository,
    TaskCancelInput, TaskOperation, TaskRetryInput, TaskRunner, TaskStatus, TrustTier,
...
fn production_boundaries_have_no_shell_or_generic_execution_escape_hatch() {
    for source in [
        include_str!("../src/mcp_platform/managed_distribution.rs"),
        include_str!("../src/mcp_platform/task_runner.rs"),
        include_str!("../src/acp/server/mcp_platform.rs"),
    ] {
    ...
    let handler = include_str!("../src/acp/server/mcp_platform.rs");
    for forbidden in [
        "sqlx::",
...
impl ManagedHarness {
    async fn new() -> Self {
        ...
        let distribution = Arc::new(FakeDistribution::default());
        let service = Arc::new(McpPlatformService::new_with_distribution_ports(
            repository.clone(),
            clock.clone(),
            Arc::new(TestIds::default()),
    ...
    async fn save_version(&self, version: &str, digest_char: char) -> String {
        ...
        value["distribution"]["artifacts"][0]["url"] = json!(format!("https://registry.npmjs.org/@example/issue-tracker-mcp/-/issue-tracker-mcp-{version}.tgz"));
    ...
    for phase in ["materialize", "pointer", "live_config"] {
        ...
        match phase {
            ...
            "pointer" => harness
                ...
                .block_activate_once
                .store(true, Ordering::SeqCst),
            "live_config" => harness.sink.block_once.store(true, Ordering::SeqCst),
            _ => unreachable!(),
        }
        ...
        match phase {
            ...
            "live_config" => harness.sink.entered.notified().await,
            _ => unreachable!(),
        }
...
async fn lifecycle_and_projection_mutations_are_mutually_exclusive_and_recovery_unblocks() {
    ...
    harness.sink.entered.notified().await;
    ...
    let mutation = harness
        .repository
        .begin_projection_mutation(&managed, current.managed.revision, false, 200)
        .await
        .unwrap();
    harness
        .repository
        .mark_projection_mutation_recovery_required(mutation.mutation_id, 201)
        .await
        .unwrap();
