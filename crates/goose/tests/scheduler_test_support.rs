...
static TEST_PROVIDER: Lazy<Mutex<Option<Arc<dyn goose::providers::base::Provider>>>> =
...
pub async fn set_test_provider(p: Arc<dyn goose::providers::base::Provider>) {
    let mut guard = TEST_PROVIDER.lock().await;
    *guard = Some(p);
}

pub async fn get_test_provider() -> Option<Arc<dyn goose::providers::base::Provider>> {
