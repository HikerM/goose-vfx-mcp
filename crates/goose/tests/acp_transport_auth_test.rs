...
use goose::acp::server::{
    AcpProviderFactory, GooseAcpAgent, GooseAcpAgentOptions, UNAVAILABLE_CUSTOM_ROUTE_METHODS,
};
use goose::acp::server_factory::{AcpServer, AcpServerFactoryConfig};
use goose::acp::transport::{create_acp_router, create_router};
use goose::agents::GoosePlatform;
use goose::custom_requests::{
    DeleteSessionRequest, McpGetRequest, McpGetResponse, McpHealthCheckMode, McpHealthRunRequest,
    McpHealthRunResponse, McpInstallConfirmRequest, McpInstallConfirmResponse, McpListRequest,
    McpListResponse, McpPlanCreateRequest, McpPlanCreateResponse, McpPlatformOutcome,
    McpProfileApplyConfirmRequest, McpProfileApplyConfirmResponse,
    McpProfileApplyPlanCreateRequest, McpProfileApplyPlanCreateResponse, McpProfileCreateRequest,
    McpProfileCreateResponse, McpTaskGetRequest, McpTaskGetResponse, McpTaskStatus,
    McpUserDecision, MCP_CATALOG_LIST_METHOD, MCP_PLAN_CREATE_METHOD,
};
use goose::mcp_platform::{parse_manifest, InstallationScope, SqliteMcpPlatformRepository};
...
#[tokio::test]
async fn websocket_uses_goose_raw_json_scanner_before_protocol_parsing() {
    let directory = tempfile::tempdir().unwrap();
...
async fn insert_manifest(data_dir: &std::path::Path) -> (Arc<SqliteMcpPlatformRepository>, String) {
    ...
    .bind(serde_json::to_string(&goose::mcp_platform::ManifestProof::LocalBytes).unwrap())
    .bind(serde_json::to_string(&goose::mcp_platform::TrustTier::Local).unwrap())
    .bind(1_000_i64)
    .execute(&pool)
    .await
    .unwrap();
...
async fn assert_catalog_denied(cx: &ConnectionTo<Agent>) {
    ...
    .await
    .expect_err("unreviewed catalog routes must be rejected before dispatch");
    assert_eq!(
        error.code,
        agent_client_protocol::Error::method_not_found().code
    );
    ...
    for secret in [PAGE_SIZE_SECRET, URL_SECRET, SESSION_SECRET] {
...
async fn wait_for_terminal_task(
    cx: &ConnectionTo<Agent>,
    task_id: &str,
) -> goose::custom_requests::McpTaskRef {
    for _ in 0..100 {
        let response = cx
...
async fn prepare_profile_application(
    ...
    let created = cx
        .send_request(McpPlanCreateRequest {
            intent: goose::custom_requests::McpPlanIntent::Register {
                manifest_digest: digest.to_string(),
                installation_scope: Some(goose::custom_requests::McpInstallationScope::User),
            },
    ...
    assert_eq!(
        ...
        goose::custom_requests::McpHealthState::Healthy
    );
...
fn legacy_v9_fixture_columns(table_name: &str) -> Vec<LegacyFixtureColumnSpec> {
    match table_name {
        ...
        "sessions" => vec![
            ...
            LegacyFixtureColumnSpec::new("goose_mode", "TEXT", true, Some("'auto'"), 0),
        ],
        "messages" => vec![
...
async fn public_raw_agent(data_dir: &Path, config_dir: &Path) -> GooseAcpAgent {
...
async fn serve_router_without_token_denies_mcp_mutations() {
    ...
    assert_catalog_denied(&cx).await;
...
#[tokio::test]
async fn authenticated_http_transport_rejects_unreviewed_plan_creation_without_side_effects() {
    let dir = tempfile::tempdir().unwrap();
    let (repository, digest) = insert_manifest(&dir.path().join("data")).await;
    let (addr, server) = spawn_router(test_router(true, &dir)).await;
    let endpoint = format!("http://{addr}/acp?token={SECRET}");
    ...
    assert_plan_create_denied(&cx, &digest, "authenticated-register-1").await;
    assert!(repository
    ...
    let fresh_digest = parse_manifest(REMOTE.as_bytes())
        ...
        .digest()
        .to_string();
    assert_plan_create_denied(&fresh_cx, &fresh_digest, "authenticated-register-2").await;
    assert!(repository
