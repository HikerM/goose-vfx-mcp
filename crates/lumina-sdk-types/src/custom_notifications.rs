...
/// Lumina-custom session update notification — a parallel to ACP's
/// `session/update` carrying lumina-specific update variants.
#[derive(Debug, Default, Clone, Serialize, Deserialize, JsonSchema, JsonRpcNotification)]
#[notification(method = "_lumina/unstable/session/update")]
#[serde(rename_all = "camelCase")]
pub struct LuminaSessionNotification {
    pub session_id: String,
    pub update: LuminaSessionUpdate,
}

/// Discriminated union of lumina-specific session update payloads.
...
pub enum LuminaSessionUpdate {
    UsageUpdate(SessionUsageUpdate),
    StatusMessage(StatusMessageUpdate),
    MessageUsage(MessageUsageUpdate),
}

impl Default for LuminaSessionUpdate {
    fn default() -> Self {
        LuminaSessionUpdate::UsageUpdate(SessionUsageUpdate::default())
    }
}
...
/// lumina-provider-types); field names and serde casing MUST stay in parity.
#[derive(Debug, Default, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
...
/// Schemas for every lumina-custom outbound notification. To register a new
...
pub fn custom_notification_schemas(generator: &mut SchemaGenerator) -> Vec<CustomMethodSchema> {
    vec![notification_schema::<LuminaSessionNotification>(generator)]
}
...
mod tests {
    ...
    fn status_message_serializes_to_expected_wire_shape() {
        let notification = LuminaSessionNotification {
    ...
    fn message_usage_serializes_to_expected_wire_shape() {
        let notification = LuminaSessionNotification {
            session_id: "s1".to_string(),
            update: LuminaSessionUpdate::MessageUsage(MessageUsageUpdate {
