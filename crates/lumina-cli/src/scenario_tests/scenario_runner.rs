...
use anyhow::Result;
use lumina::agents::{Agent, AgentConfig, LuminaPlatform};
use lumina::config::permission::PermissionManager;
use lumina::config::LuminaMode;
use lumina::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("LUMINA_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    lumina::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        lumina::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn lumina::providers::base::Provider>,
