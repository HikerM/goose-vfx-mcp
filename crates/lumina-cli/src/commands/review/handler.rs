...
use lumina::checks::{discover, DiscoveredReview};
...
/// Entry point for the `lumina review` subcommand.
pub async fn handle_review(opts: ReviewOptions) -> Result<()> {
    ...
    if diff.trim().is_empty() {
        eprintln!("lumina review: no changes to review");
        return Ok(());
    }
    ...
    if !use_orchestrator {
        ...
        if opts.checks_only {
            ...
            if !opts.quiet {
                ...
                eprintln!(
                    "lumina review: emitted {total_emitted} finding(s) from {} check(s) ({suppressed} hidden below severity={:?})",
                    discovered.checks.len(),
                    min_sev
                );
            }
    ...
    if !opts.quiet {
        ...
        if suppressed == 0 {
            eprintln!(
                "lumina review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s))",
                discovered.checks.len(),
                main_findings.len()
            );
        } else {
            eprintln!(
                "lumina review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s); {suppressed} hidden below severity={:?})",
                discovered.checks.len(),
                main_findings.len(),
                min_sev
            );
...
fn print_discovered_summary(d: &DiscoveredReview) {
    if d.checks.is_empty() {
        eprintln!("lumina review: no checks or REVIEW.md rules discovered");
        return;
    }
    eprintln!("lumina review: discovered {} check(s):", d.checks.len());
    for c in &d.checks {
...
/// `discovery_root` so [`lumina::checks::discover`] doesn't double-
