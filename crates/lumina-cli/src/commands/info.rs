...
use lumina::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
...
async fn check_provider(
    ...
    let (provider, model) = match (config.get_lumina_provider(), config.get_lumina_model()) {
    ...
    let model_config = lumina::model_config::model_config_from_user_config(&provider, &model)
        .map_err(|e| ProviderCheckError::InvalidModel(e.to_string()))?;

    let provider_client = lumina::providers::create(&provider, Vec::new())
        .await
        .map_err(|e| {
    ...
    lumina::session_context::with_session_id(
    ...
    if check {
        ...
        match &result {
            ...
            Err(ProviderCheckError::NotConfigured { label, error }) => {
                ...
                print_aligned(
                    "Hint:",
                    &format!("Run '{}'", style("lumina configure").cyan()),
                    label_padding,
                );
            }
        ...
        // checks, health probes) can rely on `lumina info --check` as a
        // pre-flight verifier.
        if result.is_err() {
