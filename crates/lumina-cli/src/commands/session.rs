...
pub async fn handle_session_import(input: String) -> Result<()> {
    ...
    let format = lumina::session::import_formats::detect_format(&json);
    let label = match format {
        lumina::session::import_formats::ImportFormat::Lumina => "lumina",
        lumina::session::import_formats::ImportFormat::ClaudeCode => "Claude Code",
        lumina::session::import_formats::ImportFormat::Codex => "Codex",
        lumina::session::import_formats::ImportFormat::Pi => "Pi",
    };
...
fn export_session_to_markdown(
    messages: Vec<lumina::conversation::message::Message>,
    session_name: &String,
) -> String {
    ...
    for message in &messages {
        ...
        let is_only_tool_response = message.role == rmcp::model::Role::User
            && message.content.iter().all(|content| {
                matches!(
                    content,
                    lumina::conversation::message::MessageContent::ToolResponse(_)
                )
            });
        ...
        if message.content.iter().any(|content| {
            matches!(
                content,
                lumina::conversation::message::MessageContent::ToolRequest(_)
            )
        }) {
