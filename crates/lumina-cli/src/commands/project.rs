...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new(&lumina_bin);
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run lumina. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("lumina Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run lumina
            let mut command = std::process::Command::new(&lumina_bin);
            command.arg("session");
        ...
        "fresh" => {
            ...
            // Build the command to run lumina with a fresh session
            let mut command = std::process::Command::new(&lumina_bin);
            command.arg("session");
