...
pub fn handle_plugin_install(url: &str) -> Result<()> {
    let install = lumina::plugins::install_plugin(url)?;
...
pub fn handle_plugin_update(name: &str) -> Result<()> {
    let install = lumina::plugins::update_plugin(name)?;
...
fn print_plugin_install(install: &lumina::plugins::PluginInstall) {
