...
fn asset_name() -> &'static str {
    ...
    {
        "lumina-aarch64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    {
        "lumina-x86_64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu"))]
    {
        "lumina-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "gnu"))]
    {
        "lumina-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "musl"))]
    {
        "lumina-x86_64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "musl"))]
    {
        "lumina-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", feature = "cuda"))]
    {
        "lumina-x86_64-pc-windows-msvc-cuda.zip"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", not(feature = "cuda")))]
    {
        "lumina-x86_64-pc-windows-msvc.zip"
    }
}
...
fn sha256_hex(data: &[u8]) -> String {
    ...
    hasher.update(data);
    lumina::utils::bytes_to_hex(hasher.finalize())
}
...
fn release_repository() -> Result<(&'static str, &'static str)> {
    let owner = option_env!("LUMINA_RELEASE_OWNER")
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let repo = option_env!("LUMINA_RELEASE_REPO")
        .map(str::trim)
        .filter(|value| !value.is_empty());
    ...
    match (owner, repo) {
        ...
        _ => bail!(
            "Update is not configured in this build. The distributor must set LUMINA_RELEASE_OWNER and LUMINA_RELEASE_REPO at build time."
        ),
    }
}
...
fn verify_bundle(
    ...
    let expected = format!("https://github.com/{owner}/{repo}/.github/workflows/{workflow}@");
    if !identity.contains(&expected) {
        bail!("Workflow identity mismatch: expected {expected}, got {identity}");
    }
...
async fn verify_provenance(archive_data: &[u8], tag: &str, owner: &str, repo: &str) -> Result<()> {
    ...
    let bundles = fetch_attestations(&digest, token.as_deref(), owner, repo)
        .await
        .context(
...
/// Update the lumina binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        let (release_owner, release_repo) = release_repository()?;
        let tag = if canary { "canary" } else { "stable" };
        let asset = asset_name();
        let url = format!(
            "https://github.com/{release_owner}/{release_repo}/releases/download/{tag}/{asset}"
        );
        ...
        verify_provenance(&bytes, tag, release_owner, release_repo).await?;
        ...
        println!("lumina updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running lumina configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run lumina configure")?;
            if !status.success() {
                eprintln!("Warning: lumina configure exited with {status}");
            }
        }
...
///   1. A `lumina-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check lumina-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("lumina-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another lumina process running?",
                    old_exe.display()
                )
            })?;
        }
...
mod tests {
    ...
    fn test_asset_name_valid() {
        ...
        assert!(name.starts_with("lumina-"));
        #[cfg(target_os = "windows")]
    ...
    fn test_binary_name() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(name, "lumina.exe");
        #[cfg(not(target_os = "windows"))]
        assert_eq!(name, "lumina");
    }

    #[test]
    fn test_find_binary_in_package_subdir() {
        let tmp = tempdir().unwrap();
        let pkg = tmp.path().join("lumina-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_lumina");
        let current = tmp.path().join("current_lumina");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("lumina.exe");
        let new_bin = tmp.path().join("new_lumina.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with lumina-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("lumina-package/", options).unwrap();
            writer
                .start_file("lumina-package/lumina.exe", options)
                .unwrap();
            writer.write_all(b"fake lumina binary").unwrap();
            writer
                .start_file("lumina-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "lumina.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake lumina binary");

        // DLL should be in lumina-package too
        assert!(tmp.path().join("lumina-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("lumina")).is_ok());
        assert!(validate_entry_path(Path::new("lumina-package/lumina")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"lumina binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "lumina-package/lumina", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("lumina-package/lumina");
        assert!(extracted.exists());
        assert_eq!(
            fs::read_to_string(extracted).unwrap(),
            "lumina binary content"
        );
    }
