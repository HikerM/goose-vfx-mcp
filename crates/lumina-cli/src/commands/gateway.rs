use anyhow::Result;
use lumina::execution::manager::AgentManager;
use lumina::gateway::manager::GatewayManager;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = lumina::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = lumina::gateway::create_gateway(&mut config)?;
