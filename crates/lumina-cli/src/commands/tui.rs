...
const TUI_NPM_SPEC_ENV: &str = "LUMINA_TUI_NPM_SPEC";
const TUI_REL_PATH: &str = "ui/text/dist/tui.js";
const NPM_BIN_NAME: &str = "lumina-tui";
...
fn resolve_source() -> Result<TuiSource> {
    ...
    let spec = std::env::var(TUI_NPM_SPEC_ENV)
        .ok()
        .map(|value| value.trim().to_owned())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| {
            anyhow!(
                "No bundled Lumina TUI was found. Set {TUI_NPM_SPEC_ENV} to a package owned by your distributor."
            )
        })?;
...
pub fn handle_tui(args: Vec<String>) -> Result<()> {
    ...
    let lumina_binary = std::env::current_exe()
        .context("could not determine current lumina executable to expose as LUMINA_BINARY")?;

    let mut cmd = build_command(&source, &args)?;
    cmd.env("LUMINA_BINARY", &lumina_binary);
...
mod tests {
    ...
    fn find_local_script_ignores_unrelated_directories() {
        ...
        let executable = temp_dir.path().join("install/bin/lumina");
    ...
    fn find_local_script_accepts_executable_ancestor() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let executable = temp_dir.path().join("target/debug/lumina");
