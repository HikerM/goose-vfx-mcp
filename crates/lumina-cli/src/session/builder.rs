...
use console::style;
use lumina::agents::{Agent, Container, ExtensionError};
use lumina::config::resolve_extensions_for_new_session;
use lumina::config::{Config, ExtensionConfig, LuminaMode};
use lumina::model_config::model_config_from_user_config;
use lumina::providers::create;
use lumina::recipe::Recipe;
use lumina::session::session_manager::SessionType;
use lumina::session::EnabledExtensionsState;
...
/// Configuration for building a new Lumina session
///
...
async fn load_extensions(
    ...
    for (id, err) in failed {
        ...
        eprintln!(
            "{}",
            style(format!(
                "  Hint: once the session starts, ask lumina to help debug the '{}' extension",
                label
            ))
            .dim()
        );
...
fn resolve_provider_and_model(
    ...
    saved_model_config: Option<lumina_providers::model::ModelConfig>,
    ...
    let provider_name = session_config
        ...
        .clone()
        .or(saved_provider)
        .or_else(|| recipe_settings.and_then(|s| s.lumina_provider.clone()))
        .or_else(|| config.get_lumina_provider().ok())
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'lumina configure' first.");
            process::exit(1);
        });
    ...
    let model_name = session_config
        ...
        .or_else(|| recipe_settings.and_then(|s| s.lumina_model.clone()))
        .or_else(|| config.get_lumina_model().ok())
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'lumina configure' first.");
            process::exit(1);
        });
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &lumina::session::session_manager::SessionManager,
...
async fn collect_extension_configs(
    ...
    if !session_config.no_profile && !session_config.resume && recipe_extensions.is_none() {
        ...
        all.extend(lumina::plugins::mcp_servers::enabled_plugin_mcp_servers(
...
async fn configure_session_prompts(
    ...
    let system_prompt_file: Option<String> =
        config.get_param("LUMINA_SYSTEM_PROMPT_FILE_PATH").ok();
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    ...
    let session_id =
        resolve_session_id(&session_config, &session_manager, agent.config.lumina_mode).await;
    ...
    let (new_provider, effective_provider_name, effective_model_name, effective_model_config) =
        match create(&resolved.provider_name, extensions_for_provider.clone()).await {
            ...
            {
                let fallback_provider = config.get_lumina_provider().unwrap_or_else(|_| {
                    output::render_error("No provider configured. Run 'lumina configure' first.");
                    process::exit(1);
                });
                let fallback_model = config.get_lumina_model().unwrap_or_else(|_| {
                    output::render_error("No model configured. Run 'lumina configure' first.");
                    process::exit(1);
                });
                eprintln!(
    ...
    agent
        .update_lumina_mode(agent.config.lumina_mode, &session_id)
        .await
        .unwrap_or_else(|e| {
    ...
    let debug_mode = session_config.debug || config.get_param("LUMINA_DEBUG").unwrap_or(false);
...
mod tests {
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: lumina::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
    ...
    async fn test_implicit_resume_ignores_newer_scheduled_sessions() {
        ...
        let lumina_mode = LuminaMode::default();
