...
use lumina::conversation::message::{
    ActionRequiredData, Message, MessageContent, SystemNotificationContent, SystemNotificationType,
    ToolRequest, ToolResponse,
};
use lumina::providers::canonical::maybe_get_canonical_model;
#[cfg(target_os = "windows")]
use lumina::subprocess::SubprocessExt;
    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("LUMINA_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("LUMINA_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("LUMINA_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("LUMINA_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("LUMINA_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("LUMINA_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("LUMINA_CLI_THEME", theme_str) {
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("LUMINA_STATUS_HOOK") {
...
pub fn lumina_mode_message(text: &str) {
    println!("\n{} {}", accent("mode:"), text);
}

fn should_show_thinking() -> bool {
    Config::global()
        .get_param::<bool>("LUMINA_CLI_SHOW_THINKING")
        .unwrap_or(false)
...
pub fn display_session_info(
    ...
    // ASCII art lumina with session info on the right
    println!();
    println!(
        "  {}  {} {} {} {} {}",
        style("  __( O)>").white(),
