...
use lumina::conversation::Conversation;
use std::env;
use std::io::Write;
use std::str::FromStr;
...
use console::Color;
use lumina::agents::AgentEvent;
use lumina::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use lumina::permission::permission_confirmation::PrincipalType;
use lumina::permission::Permission;
use lumina::permission::PermissionConfirmation;
use lumina::providers::base::Provider;
use lumina::providers::base::ProviderUsage;
use lumina::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::LuminaCompleter;
use input::InputResult;
use lumina::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use lumina::agents::types::RetryConfig;
use lumina::agents::{Agent, SessionConfig, COMPACT_TRIGGERS};
use lumina::config::extensions::name_to_key;
...
use lumina::conversation::message::{ActionRequiredData, Message, MessageContent};
...
const LUMINA_PLANNER_CONTEXT_LIMIT: &str = "LUMINA_PLANNER_CONTEXT_LIMIT";
...
impl HistoryManager {
    ...
    fn load(
        &self,
        editor: &mut rustyline::Editor<LuminaCompleter, rustyline::history::DefaultHistory>,
    ) {
...
pub async fn classify_planner_response(
    ...
    model_config: lumina_providers::model::ModelConfig,
    ...
    let (result, _usage) = lumina::session_context::with_session_id(
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        let mut parts = lumina::utils::split_command_args(extension_command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: lumina::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(lumina::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            lumina::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        ...
        self.agent
            .emit_hook(lumina::hooks::HookEvent::SessionEnd, &self.session_id)
            .await;
    ...
    fn create_editor(
        &self,
    ) -> Result<rustyline::Editor<LuminaCompleter, rustyline::history::DefaultHistory>> {
        let builder =
        ...
        let mut editor =
            rustyline::Editor::<LuminaCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;
        let completer = LuminaCompleter::new(self.completion_cache.clone());
    ...
    async fn handle_input(
        ...
        match input {
            ...
            InputResult::LuminaMode(mode) => {
                history.save(editor);
                self.handle_lumina_mode(&mode).await?;
            }
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 lumina configure set lumina_prompt_editor \"vim\"\n  \
    ...
    async fn handle_lumina_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match LuminaMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
                output::render_error(&format!(
                    "Invalid mode '{mode}'. Mode must be one of: {}",
                    LuminaMode::VARIANTS.join(", ")
                ));
                return Ok(());
            }
        };
        self.agent
            .update_lumina_mode(mode, &self.session_id)
            .await?;
        config.set_lumina_mode(mode)?;
        output::lumina_mode_message(&format!("Lumina mode set to '{mode}'"));
        Ok(())
    }
    ...
    async fn handle_model(&self, model: Option<&str>) -> Result<()> {
        ...
        if model.is_none() {
            output::lumina_mode_message(&format!(
        ...
        let configured_effort = Config::global().get_lumina_thinking_effort();
        ...
        let new_provider = lumina::providers::create(&current_provider_name, extensions)
            .await
        ...
        let mode = self.agent.lumina_mode().await;
        self.agent
            .update_lumina_mode(mode, &self.session_id)
            .await?;
        output::lumina_mode_message(&format!(
    ...
    async fn handle_clear(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            ...
            .usage(lumina_providers::conversation::token_usage::Usage::new(
                Some(0),
                Some(0),
                Some(0),
            ))
    ...
    async fn handle_list_skills(&mut self) -> Result<()> {
        ...
        use lumina::custom_requests::SourceType;
        use lumina::skills::list_installed_skills;
    ...
    async fn plan_with_reasoner_model(
        ...
        output::show_thinking();
        let (plan_response, _usage) = lumina::session_context::with_session_id(
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set lumina mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_lumina_mode = config.get_lumina_mode().unwrap_or_default();
                    if curr_lumina_mode != LuminaMode::Auto {
                        config.set_lumina_mode(LuminaMode::Auto).unwrap();
                    }
                    ...
                    // Reset run & lumina mode
                    if curr_lumina_mode != LuminaMode::Auto {
                        config.set_lumina_mode(curr_lumina_mode)?;
                    }
                } else {
                        ...
                        Some(Ok(AgentEvent::Message(message))) => {
                            ...
                            if let Some((id, security_prompt)) = find_tool_confirmation(&message) {
                                let permission = if interactive {
                                    ...
                                    let config = Config::global();
                                    let lumina_mode = config.get_lumina_mode().unwrap_or(LuminaMode::Auto);
                                    if lumina_mode == LuminaMode::Approve || lumina_mode == LuminaMode::SmartApprove {
                                        cancel_token_clone.cancel();
                                        drop(stream);
                                        return Err(anyhow::anyhow!(
                                            "Tool approval required in non-interactive mode with LuminaMode::{lumina_mode}. \
                                             This is an invalid configuration — Approve/SmartApprove modes require an \
                                             interactive terminal. Use LuminaMode::Auto for headless sessions."
                                        ));
                                    }
                                    tracing::warn!(
    ...
    pub async fn get_session(&self) -> Result<lumina::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("LUMINA_CLI_SHOW_COST")
            .unwrap_or(false);
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "Lumina would like to call the above tool, do you allow?".to_string()
    };
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("LUMINA_CLI_MIN_PRIORITY")
                            .ok()
    ...
    for content in &message.content {
        if let MessageContent::ToolRequest(tool_request) = content {
            if let Ok(tool_call) = &tool_request.tool_call {
                tracing::info!(
                    monotonic_counter.lumina.tool_calls = 1,
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                monotonic_counter.lumina.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<lumina_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                lumina_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
) -> Result<(Arc<dyn Provider>, lumina_providers::model::ModelConfig), anyhow::Error> {
    use lumina::providers::create;

    let config = Config::global();

    // Try planner-specific provider first, fall back to default provider
    let provider = if let Ok(provider) = config.get_param::<String>("LUMINA_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: LUMINA_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_lumina_provider()
            .expect("No provider configured. Run 'lumina configure' first")
    };

    // Try planner-specific model first, fall back to default model
    let model = if let Ok(model) = config.get_param::<String>("LUMINA_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: LUMINA_PLANNER_MODEL not found. Using default model...");
        config
            .get_lumina_model()
            .expect("No model configured. Run 'lumina configure' first")
    };

    let planner_context_limit = match env::var(LUMINA_PLANNER_CONTEXT_LIMIT)
        .ok()
        .map(|v| v.parse::<usize>())
    {
        Some(Ok(n)) if n >= 4096 => Some(n),
        Some(Ok(_)) => anyhow::bail!("{} must be at least 4096", LUMINA_PLANNER_CONTEXT_LIMIT),
        Some(Err(e)) => anyhow::bail!("{}: {}", LUMINA_PLANNER_CONTEXT_LIMIT, e),
        None => None,
    };

    let model_config =
        lumina::model_config::model_config_from_user_config(&provider, model.as_str())?
            .with_context_limit(planner_context_limit);
    let extensions = lumina::config::extensions::get_enabled_extensions_with_config(config);
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &lumina_providers::model::ModelConfig,
) -> Result<lumina_providers::model::ModelConfig> {
...
mod tests {
    ...
    use lumina::config::ExtensionConfig;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        let _guard = env_lock::lock_env([
            ("LUMINA_MAX_TOKENS", None::<&str>),
            ("LUMINA_TEMPERATURE", None::<&str>),
            ("LUMINA_CONTEXT_LIMIT", None::<&str>),
            ("LUMINA_TOOLSHIM", None::<&str>),
            ("LUMINA_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);

        let current_model_config = lumina_providers::model::ModelConfig {
        ...
        let expected = lumina_providers::model::ModelConfig::new("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        let _guard = env_lock::lock_env([
            ...
            ("LUMINA_TOOLSHIM", None::<&str>),
            ("LUMINA_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
            ("LUMINA_THINKING_EFFORT", None::<&str>),
        ]);

        let current = lumina_providers::model::ModelConfig::new("gpt-5.4-high")
            .with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(lumina_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            lumina::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
            vec![r"C:\tools\mcp.exe", "--arg", "value"]
        );
        assert_eq!(
            lumina::utils::split_command_args(r#""C:\Program Files\server\mcp.exe" --arg"#)
                .unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(lumina::utils::split_command_args(r#""unmatched"#).is_err());
    }
