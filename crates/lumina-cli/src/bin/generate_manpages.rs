//! Generate manpages for the lumina CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man lumina`).
//!
//! When lumina is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Usage:
//!   cargo run -p lumina-cli --bin generate_manpages
//!   # or
//!   just generate-manpages
//!
//! Output: target/man/lumina.1, target/man/lumina-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main lumina command if we're not it
    if current_name != "lumina" {
        references.push("lumina".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "lumina" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "lumina" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("lumina-{}", subcmd_name);
