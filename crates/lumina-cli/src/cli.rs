...
use lumina::builtin_extension::register_builtin_extensions;
use lumina::config::paths::Paths;
use lumina::config::{Config, LuminaMode};
use lumina::recipe::Recipe;
use lumina::source_roots::SourceRoot;
use lumina_mcp::mcp_server_runner::{serve, McpCommand};
use lumina_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
use lumina::agents::Container;
use lumina::session::session_manager::SessionType;
use lumina::session::SessionManager;
use std::io::Read;
use std::path::PathBuf;
use tracing::warn;

const LUMINA_SERVER_SECRET_KEY_ENV: &str = "LUMINA_SERVER__SECRET_KEY";
...
impl From<ServePlatform> for LuminaPlatform {
    fn from(platform: ServePlatform) -> Self {
        match platform {
            ServePlatform::Cli => LuminaPlatform::LuminaCli,
            ServePlatform::Desktop => LuminaPlatform::LuminaDesktop,
        }
    }
}

#[derive(Parser)]
#[command(name = "lumina", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, lumina must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), lumina::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with lumina by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to lumina directly",
        long_help = "Input text containing commands for lumina. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=lumina-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the LUMINA_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the LUMINA_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum SessionCommand {
    ...
    Import {
        #[arg(
            help = "Path to a lumina session export or a Claude Code, Codex, or Pi .jsonl transcript"
        )]
        input: String,
    },
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the lumina agent
    #[command(about = "List all skills available to the lumina agent")]
    List,
}
...
#[derive(Subcommand)]
enum Command {
    /// Configure lumina settings
    #[command(about = "Configure lumina settings")]
    Configure {},

    /// Display lumina configuration information
    #[command(about = "Display lumina information")]
    Info {
    ...
    #[command(about = "Check that your Lumina setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with lumina")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run lumina as an ACP (Agent Client Protocol) agent
    #[command(about = "Run lumina as an ACP agent server on stdio")]
    Acp {
    ...
    Serve {
        ...
        #[arg(
            ...
            help = "Start the ACP endpoint without requiring LUMINA_SERVER__SECRET_KEY"
        )]
    ...
    /// Update the lumina CLI version
    #[cfg(feature = "update")]
    #[command(about = "Update the lumina CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the lumina CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure lumina during update
        #[arg(short, long, help = "Enforce to re-configure lumina during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated lumina session",
        long_about = "Runs a lumina session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(lumina term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"lumina-term-init.nu\"); ^lumina term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        lumina term run \"list files in this directory\"\n  \
                        @lumina \"create a python script\"  # using alias\n  \
    ...
    /// Launch the lumina terminal UI (TUI)
    #[cfg(feature = "tui")]
    #[command(
        about = "Launch the lumina terminal UI",
        long_about = "Launch the lumina terminal UI (the @hikerm/lumina npm package).\n\
                      \n\
                      Resolution order:\n  \
                      1. LUMINA_TUI_SCRIPT, if set to an existing dist/tui.js\n  \
                      2. A local checkout's ui/text/dist/tui.js (dev workflow)\n  \
                      3. `npx --yes --package <spec> -- lumina-tui` (deployed installs)\n\
                      \n\
                      If no bundled script is present, LUMINA_TUI_NPM_SPEC must name a distributor-owned package.\n\
    ...
    Completion {
        ...
        #[arg(long, default_value = "lumina", help = "Provide a custom binary name")]
        bin_name: String,
    },
    ...
    /// runs the review through lumina.
    #[command(about = "Review the current diff using lumina")]
    Review {
        ...
        /// orchestrator dispatches one `lumina run` subprocess per check
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent lumina session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(lumina term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"lumina-term-init.nu\")\n  \
                        ^lumina term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to lumina):\n  \
                        echo 'eval \"$(lumina term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^lumina term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make lumina the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make lumina the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to lumina. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        lumina term run list files in this directory\n  \
                        @lumina list files  # using alias\n  \
                        @g why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to lumina (multiple words allowed without quotes)
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    use lumina::acp::transport::create_router;
    ...
    let server = Arc::new(AcpServer::new(AcpServerFactoryConfig {
        ...
        lumina_platform: platform.into(),
        additional_source_roots,
    }));
    let env_secret = std::env::var(LUMINA_SERVER_SECRET_KEY_ENV)
        .ok()
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "{LUMINA_SERVER_SECRET_KEY_ENV} must be set to start `lumina serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    if dangerously_unauthenticated && !require_token {
        warn!(
            "{LUMINA_SERVER_SECRET_KEY_ENV} is not set and --dangerously-unauthenticated was passed; the ACP endpoint will accept unauthenticated connections"
        );
    }
    ...
    let tls_cert_path =
        tls_cert_path.or_else(|| config.get_param::<String>("LUMINA_TLS_CERT_PATH").ok());
    let tls_key_path =
        tls_key_path.or_else(|| config.get_param::<String>("LUMINA_TLS_KEY_PATH").ok());
    let tls = tls
        || config.get_param::<bool>("LUMINA_TLS").unwrap_or(false)
    ...
    let serve = async {
        if tls {
            ...
            {
                let tls_setup = lumina::acp::transport::tls::setup_tls(
...
async fn handle_interactive_session(
    ...
    let lumina_mode = Config::global().get_lumina_mode().unwrap_or_default();
    let mut session_id = get_or_create_session_id(identifier, resume, false, lumina_mode).await?;
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use lumina run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    lumina::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
...
#[cfg(feature = "local-inference")]
fn print_download_progress(manager: &lumina::download_manager::DownloadManager) {
    let Some(progress) = manager
        .list_progress()
        .into_iter()
        .find(|progress| progress.status == lumina::download_manager::DownloadStatus::Downloading)
    else {
        return;
    };
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use lumina::providers::local_inference::hf_models;
    use lumina::providers::local_inference::local_model_registry::get_registry;

    lumina::providers::local_inference::configure_huggingface_auth();
    ...
    match command {
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = lumina::download_manager::get_download_manager();
...
async fn handle_default_session() -> Result<()> {
    ...
    let lumina_mode = Config::global().get_lumina_mode().unwrap_or_default();
    let session_id = get_or_create_session_id(None, false, false, lumina_mode).await?;
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(lumina_mcp::BUILTIN_EXTENSIONS.clone());
    ...
    tracing::info!(
        monotonic_counter.lumina.cli_commands = 1,
    ...
    match cli.command {
        ...
        Some(Command::Acp { builtins }) => lumina::acp::server::run(builtins).await,
        Some(Command::Serve {
            host,
            port,
            tls,
        ...
        Some(Command::ValidateExtensions { file }) => {
            use lumina::agents::validate_extensions::validate_bundled_extensions;
...
mod tests {
    ...
    fn completion_command_accepts_nushell_alias() {
        let cli = Cli::try_parse_from(["lumina", "completion", "nushell"]).expect("parse failed");
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "lumina", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern lumina"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("lumina term init nu"));
    ...
    fn skills_command_accepts_list_subcommand() {
        let cli = Cli::try_parse_from(["lumina", "skills", "list"]).expect("parse failed");
    ...
    fn tui_command_accepts_trailing_args() {
        let cli =
            Cli::try_parse_from(["lumina", "tui", "--", "--theme", "dark"]).expect("parse failed");
