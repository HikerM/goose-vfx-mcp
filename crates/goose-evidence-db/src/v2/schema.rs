...
pub const V2_SCHEMA_VERSION: &str = "goose-evidence-db/pre-a0.2-v2.3-foundation";
...
pub fn manifest_hash() -> V2Result<[u8; 32]> {
    ...
    sha256_domain("goose.evidence-db.v2.schema-manifest", &parts)
}

pub fn params_hash() -> V2Result<[u8; 32]> {
    sha256_domain(
        "goose.evidence-db.v2.schema-params",
...
pub fn catalog_hash() -> V2Result<[u8; 32]> {
    sha256_domain(
        "goose.evidence-db.v2.sqlite-schema",
...
pub fn actual_catalog_hash(conn: &Connection) -> V2Result<[u8; 32]> {
    ...
    sha256_domain("goose.evidence-db.v2.sqlite-schema", &[&out])
}
