...
native_lib="$1"
resource_prefix="${2:-$(crates/lumina-sdk/scripts/maven-resource-prefix.sh)}"
...
bindgen="target/release/lumina-uniffi-bindgen"
if [ ! -x "$bindgen" ]; then
  cargo build -p lumina-sdk --features uniffi --release -q
fi

maven_dir="crates/lumina-sdk/maven"
...
rm -rf "$kotlin_dir/io/github/hikerm.lumina" "$resources_dir/$resource_prefix"
mkdir -p "$kotlin_dir" "$resources_dir/$resource_prefix" "$resources_dir/META-INF"
cp LICENSE "$resources_dir/META-INF/LICENSE"

"$bindgen" generate \
  --library "$native_lib" \
  --config crates/lumina-sdk/uniffi.toml \
  --language kotlin \
  --no-format \
  --out-dir "$kotlin_dir" 2>/dev/null

python3 - "$kotlin_dir/io/github/hikerm.lumina/lumina.kt" <<'PY'
import sys
