//! ACP Client Example
//!
//! Spawns `lumina acp` as a child process and sends it a completion request
//! using the Agent Client Protocol over stdio.
//!
//! # Prerequisites
//!
//! You must have lumina built and a provider configured (`lumina configure`).
//!
//! # Usage
//!
//! ```bash
//! cargo run -p lumina-sdk --example acp_client -- "What is 2 + 2?"
//! ```
//!
//! Or with a custom lumina binary path:
//!
//! ```bash
//! cargo run -p lumina-sdk --example acp_client -- --lumina-bin ./target/debug/lumina "Explain Rust's ownership model in one sentence"
//! ```
...
use lumina_sdk::custom_requests::GetExtensionsRequest;
use std::path::PathBuf;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Parse args: [--lumina-bin PATH] PROMPT
    let args: Vec<String> = std::env::args().skip(1).collect();
    let (lumina_bin, prompt) = parse_args(&args)?;

    eprintln!("🚀 Spawning: {} acp", lumina_bin.display());

    let mut child = tokio::process::Command::new(&lumina_bin)
        .arg("acp")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::inherit())
        .spawn()
        .map_err(|e| format!("Failed to spawn '{}': {e}", lumina_bin.display()))?;
...
fn parse_args(args: &[String]) -> Result<(PathBuf, String), String> {
    let mut lumina_bin = PathBuf::from("lumina");
    let mut i = 0;
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--lumina-bin" => {
                i += 1;
                lumina_bin = PathBuf::from(args.get(i).ok_or("--lumina-bin requires a value")?);
            }
            _ => break,
        }
        i += 1;
    ...
    if prompt.is_empty() {
        return Err("Usage: acp_client [--lumina-bin PATH] PROMPT".into());
    }

    Ok((lumina_bin, prompt))
}
