...
pub struct OllamaOptions {
    /// Explicit context window override from `LUMINA_INPUT_LIMIT`.
    ...
    /// `OLLAMA_STREAM_TIMEOUT` > `LUMINA_STREAM_TIMEOUT` > `OLLAMA_TIMEOUT` >
    /// default (120s).
...
/// Configurable via OLLAMA_STREAM_TIMEOUT, LUMINA_STREAM_TIMEOUT, or falls back
                ...
                Err(_) => {
                    Err::<(), anyhow::Error>(anyhow::anyhow!(
                        "Ollama stream stalled: no data received for {}s. \
                         ...
                         timeout via OLLAMA_STREAM_TIMEOUT, LUMINA_STREAM_TIMEOUT, or \
