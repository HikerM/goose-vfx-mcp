...
impl MemoryServer {
    pub fn new() -> Self {
        let instructions = formatdoc! {r#"
             ...
             Storage:
             - Local: .lumina/memory/ (project-specific)
             - Global: ~/.config/lumina/memory/ (user-wide)
        ...
        let global_memory_dir = choose_app_strategy(crate::APP_STRATEGY.clone())
            ...
            .unwrap_or_else(|_| PathBuf::from(".config/lumina/memory"));
    ...
    fn get_memory_file(
        ...
        let base_dir = if is_global {
            ...
            local_base.join(".lumina").join("memory")
        };
...
mod tests {
    ...
    fn test_lazy_directory_creation() {
        ...
        let local_memory_dir = working_dir.join(".lumina").join("memory");
