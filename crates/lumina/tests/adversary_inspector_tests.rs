...
use lumina::conversation::message::{Message, MessageContent, ToolRequest};
use lumina::security::adversary_inspector::AdversaryInspector;
use lumina::tool_inspection::ToolInspector;
...
async fn test_adversary_disabled_without_config_file() {
    ...
    let inspector = AdversaryInspector::with_config_dir(
        provider,
        Arc::new(lumina::session::SessionManager::new(
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        lumina::tool_inspection::InspectionAction::Allow
    ));
