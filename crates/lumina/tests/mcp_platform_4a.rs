...
use lumina::custom_requests::{McpCatalogDetail, McpInstallConfirmRequest};

fn cargo_manifest(lumina_manifest_dir: &str) -> String {
    format!(
        r#"[package]
name = "lumina-profile-public-api-fixture"
version = "0.0.0"
edition = "2021"
...
const PROFILE_INTERNAL_FIXTURE: &str = r#"use lumina::mcp_platform::{
...
fn wire_schemas_are_closed_and_never_accept_raw_execution_or_auth_material() {
    let manual_schema = serde_json::to_string(&schemars::schema_for!(
        lumina::custom_requests::McpManualPlanCreateRequest
    ))
    .unwrap();
    for forbidden in [
        "executable",
    ...
    let task_schema =
        serde_json::to_string(&schemars::schema_for!(lumina::custom_requests::McpTaskRef)).unwrap();
