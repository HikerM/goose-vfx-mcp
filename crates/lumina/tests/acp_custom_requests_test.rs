...
use lumina::acp::server::AcpProviderFactory;
use lumina::providers::base::{MessageStream, Provider};
use lumina_providers::errors::ProviderError;
use lumina_providers::model::ModelConfig;
use lumina_test_support::{EnforceSessionId, IgnoreSessionId};
...
fn write_acp_global_config(contents: &str) -> PathBuf {
    std::env::set_var("LUMINA_PATH_ROOT", ACP_CONFIG_ROOT.path());
    std::env::set_var("LUMINA_DISABLE_KEYRING", "1");
    let config_dir = lumina::config::paths::Paths::config_dir();
    std::fs::create_dir_all(&config_dir).unwrap();
    let mut contents = contents.to_string();
    if !contents.contains("LUMINA_DISABLE_KEYRING") {
        contents.push_str("LUMINA_DISABLE_KEYRING: true\n");
    }
    std::fs::write(
        config_dir.join(lumina::config::base::CONFIG_YAML_NAME),
        contents,
    )
    .unwrap();
    config_dir
...
fn steer_chunk_message_ids(updates: &[SessionUpdate]) -> Vec<String> {
    updates
        ...
        .filter_map(|update| {
            ...
            let lumina = chunk.meta.as_ref()?.get("lumina")?;
            lumina.get("steer")?.as_bool().filter(|b| *b)?;
            lumina.get("messageId")?.as_str().map(ToString::to_string)
        })
        .collect()
}
...
fn test_custom_get_extensions() {
    ...
    run_test(async move {
        ...
        let add_result = send_custom(
            conn.cx(),
            "_lumina/unstable/config/extensions/add",
            serde_json::json!({
                "enabled": true,
                "extension": {
                    "type": "mcp",
        ...
        let stored_inline_token = lumina::config::Config::global()
...
fn test_steer_session_adds_input_to_active_prompt() {
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "expected a chunk marked _meta.lumina.steer with the steer text, got: {steer_chunks:?}"
        );
...
fn test_custom_list_builtin_skill_sources() {
    ...
    run_test(async move {
        ...
        let builtin = sources
            .iter()
            .find(|source| source.get("name") == Some(&serde_json::json!("lumina-doc-guide")))
            .expect("expected lumina-doc-guide builtin skill");
        ...
        assert_eq!(
            builtin.get("path"),
            Some(&serde_json::json!("builtin://skills/lumina-doc-guide"))
        );
    });
}
...
fn test_custom_preferences_read_save_remove() {
    let config_dir = write_acp_global_config(
        "LUMINA_MODEL: gpt-4o\nLUMINA_PROVIDER: openai\nLUMINA_AUTO_COMPACT_THRESHOLD: 0.7\nLUMINA_THINKING_EFFORT: high\nVOICE_AUTO_SUBMIT_PHRASES: send it\n",
    );
    ...
    run_test(async move {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!([
                ...
                { "key": "luminaThinkingEffort", "value": "high" },
        ...
        send_custom(
            conn.cx(),
            "_lumina/unstable/preferences/save",
            serde_json::json!({
                "values": [
                    { "key": "luminaThinkingEffort", "value": "disabled" },
        ...
        send_custom(
            conn.cx(),
            "_lumina/unstable/preferences/remove",
            serde_json::json!({
        ...
        let response = send_custom(
            ...
            "_lumina/unstable/preferences/read",
            serde_json::json!({
                "keys": ["luminaThinkingEffort", "voiceDictationProvider", "voiceDictationPreferredMic"],
            }),
        )
        .await
        ...
        assert_eq!(
            response.get("values"),
            Some(&serde_json::json!([
                { "key": "luminaThinkingEffort", "value": "off" },
...
fn test_custom_preferences_save_rejects_invalid_values() {
    ...
    run_test(async {
        ...
        let invalid_payloads = [
            ...
            serde_json::json!({
                "values": [{ "key": "luminaThinkingEffort", "value": "bogus" }],
            }),
            serde_json::json!({
                "values": [{ "key": "luminaThinkingEffort", "value": ["high"] }],
            }),
            serde_json::json!({
        ...
        for payload in invalid_payloads {
            let result = send_custom(conn.cx(), "_lumina/unstable/preferences/save", payload).await;
...
fn test_custom_defaults_read() {
    let config_dir = write_acp_global_config(
        "LUMINA_MODEL: claude-3-5-haiku-latest\nLUMINA_PROVIDER: anthropic\n",
    );
...
fn test_custom_dictation_secret_save_delete() {
    ...
    run_test(async move {
        ...
        send_custom(
            conn.cx(),
            "_lumina/unstable/dictation/secret/save",
            serde_json::json!({
                "provider": "groq",
                "value": "groq-key",
            }),
...
fn test_raw_config_and_secret_methods_are_removed() {
    ...
    run_test(async {
        ...
        for method in [
            "_lumina/config/read",
            "_lumina/config/upsert",
            "_lumina/config/remove",
            "_lumina/secret/check",
            "_lumina/secret/upsert",
            "_lumina/secret/remove",
        ] {
...
fn test_provider_switching_updates_session_state() {
    ...
    run_test(async {
        ...
        conn.set_config_option(&session_id, "provider", "lumina")
            .await
            .expect("provider reset to lumina should succeed");
    });
}
                ...
                Box::pin(async move {
                    Ok(Arc::new(MockProvider {
                        ...
                        supported_models: vec![
                            "lumina-claude-opus-4-8".to_string(),
