...
use lumina::agents::{Agent, AgentEvent, LuminaPlatform};
use lumina::config::extensions::{set_extension, ExtensionEntry};
...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use lumina::agents::platform_tools::PLATFORM_MANAGE_SCHEDULE_TOOL_NAME;
        use lumina::agents::AgentConfig;
        use lumina::config::permission::PermissionManager;
        ...
        async fn test_schedule_management_tool_list() {
            ...
            assert!(tool
                ...
                .clone()
                .unwrap_or_default()
                .contains("Manage lumina's internal scheduled recipe execution"));
        }
    ...
    mod retry_tests {
        use super::*;
        use lumina::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use lumina::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use async_trait::async_trait;
        use lumina::agents::SessionConfig;
        use lumina::config::LuminaMode;
        use lumina::conversation::message::{Message, MessageContent};
        use lumina::providers::base::{
        ...
        impl lumina::providers::base::ProviderDescriptor for MockToolProvider {
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _extensions: Vec<lumina::config::ExtensionConfig>,
                _tls_config: Option<lumina::providers::api_client::TlsConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let lumina::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    lumina::permission::PermissionConfirmation {
                                        principal_type: lumina::permission::permission_confirmation::PrincipalType::Tool,
                                        permission: lumina::permission::Permission::AllowOnce,
                                    }
                                ).await;
                            }
                        }
    ...
    mod unparseable_tool_call_tests {
        use super::*;
        use async_trait::async_trait;
        use lumina::agents::{AgentConfig, SessionConfig};
        use lumina::config::permission::PermissionManager;
        use lumina::config::LuminaMode;
        use lumina::conversation::message::{Message, MessageContent};
        use lumina::providers::base::{
        ...
        impl lumina::providers::base::ProviderDescriptor for UnparseableToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod tool_pair_summarization_tests {
        ...
        impl lumina::providers::base::ProviderDescriptor for SummarizationTestProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_batch_summarization_preserves_all_summaries() -> Result<()> {
            ...
            Config::global()
                .set_param("LUMINA_TOOL_CALL_CUTOFF", 2)
                .unwrap();
            ...
            Config::global().delete("LUMINA_TOOL_CALL_CUTOFF").unwrap();
    ...
    mod extension_manager_tests {
        use super::*;
        use lumina::agents::extension::ExtensionConfig;
        use lumina::agents::platform_extensions::{
        ...
        async fn setup_agent_with_extension_manager() -> (Agent, String) {
            ...
            let todo_extension_entry = ExtensionEntry {
                ...
                config: ExtensionConfig::Platform {
                    name: "todo".to_string(),
                    description:
                        "Enable a todo list for Lumina so it can keep track of what it is doing"
                            .to_string(),
    ...
    mod streaming_persistence_tests {
        ...
        use lumina::config::LuminaMode;
        use lumina::conversation::message::Message;
        use lumina::providers::base::{MessageStream, Provider, ProviderDef, ProviderMetadata};
        use lumina::session::session_manager::SessionType;
        use lumina::session::SessionManager;
        use lumina_providers::conversation::token_usage::{ProviderUsage, Usage};
        use lumina_providers::errors::ProviderError;
        ...
        impl lumina::providers::base::ProviderDescriptor for MultiStepProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod thinking_preservation_tests {
        ...
        impl lumina::providers::base::ProviderDescriptor for ThinkingStreamProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        fn assert_formatter_adds_reasoning_to_tool_calls(messages: &[Message], provider: &str) {
            use lumina_providers::formats::openai::{
                format_messages_with_options, OpenAiFormatOptions,
            };
            use lumina_providers::images::ImageFormat;
        ...
        impl lumina::providers::base::ProviderDescriptor for CombinedThinkingToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl lumina::providers::base::ProviderDescriptor for MultiToolThinkingProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_signed_thinking_not_duplicated_for_anthropic() -> Result<()> {
            use lumina_providers::formats::anthropic::format_messages as anthropic_format;
    ...
    mod goal_checking_tests {
        ...
        impl lumina::providers::base::ProviderDescriptor for GoalTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod cumulative_token_tests {
        ...
        use lumina::config::LuminaMode;
        use lumina::conversation::message::Message;
        use lumina::providers::base::{stream_from_single_message, MessageStream, Provider};
        use lumina::session::session_manager::SessionType;
        use lumina::session::SessionManager;
        use lumina_providers::conversation::token_usage::{ProviderUsage, Usage};
        use lumina_providers::errors::ProviderError;
    ...
    mod frontend_extension_tests {
        use super::*;
        use lumina::agents::{AgentConfig, ExtensionConfig};
        use lumina::config::permission::PermissionManager;
        use lumina::config::LuminaMode;
        use lumina::session::session_manager::SessionType;
        use lumina::session::{
    ...
    mod empty_turn_tests {
        use super::*;
        use async_trait::async_trait;
        use lumina::agents::{AgentEvent, SessionConfig};
        use lumina::config::LuminaMode;
        use lumina::conversation::message::{Message, MessageContent};
        use lumina::providers::base::{
            stream_from_single_message, MessageStream, Provider, ProviderDef, ProviderMetadata,
        ...
        impl lumina::providers::base::ProviderDescriptor for EmptyThenTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_empty_turn_with_final_output_tool_nudges() -> Result<()> {
            use lumina::agents::final_output_tool::FINAL_OUTPUT_CONTINUATION_MESSAGE;
            use lumina::recipe::Response;
