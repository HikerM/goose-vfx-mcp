...
//!   lumina local-models download bartowski/Llama-3.2-1B-Instruct-GGUF:Q4_K_M
//!
//! Run with the default model:
//!   cargo test -p lumina --test local_inference_integration -- --ignored
//!
//! Run with a specific model:
//!   TEST_MODEL="bartowski/Qwen_Qwen3-32B-GGUF:Q4_K_M" cargo test -p lumina --test local_inference_integration -- --ignored
//!
//! Run vision tests (requires a vision-capable model like gemma-4):
//!   TEST_VISION_MODEL="unsloth/gemma-4-E4B-it-GGUF:Q4_K_M" cargo test -p lumina --test local_inference_integration test_local_inference_vision -- --ignored
...
async fn test_local_inference_stream_produces_output() {
    ...
    let provider = lumina::providers::create("local", Vec::new())
        .await
        .expect("provider creation should succeed");

    let system = "You are a helpful assistant. Be brief.";
    let messages = vec![lumina::conversation::message::Message::user().with_text("Say hello.")];
...
async fn test_local_inference_large_prompt() {
    ...
    let padding = "You are Lumina, a highly capable AI assistant.\n".repeat(80);
    let prompt = format!("{padding}\nNow answer this: what is the capital of Moldova?");
    let messages = vec![lumina::conversation::message::Message::user().with_text(&prompt)];
...
///     cargo test -p lumina --test local_inference_integration test_local_inference_vision -- --ignored
#[tokio::test]
#[ignore]
async fn test_local_inference_vision_produces_output() {
    ...
    let messages = vec![lumina::conversation::message::Message::user()
