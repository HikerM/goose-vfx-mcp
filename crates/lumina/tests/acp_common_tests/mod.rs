...
use lumina_test_support::{McpFixture, FAKE_CODE, TEST_IMAGE_B64, TEST_MODEL};
...
pub async fn run_load_mode<C: Connection>() {
    ...
    assert_eq!(
        ...
        SessionModeId::new(<&str>::from(LuminaMode::default()))
    );
    let session_id = session.session_id().0.to_string();
    conn.set_mode(&session_id, <&str>::from(LuminaMode::Approve))
        .await
        .unwrap();
    ...
    assert_eq!(
        modes.unwrap().current_mode_id,
        SessionModeId::new(<&str>::from(LuminaMode::Approve))
    );
...
pub async fn run_load_session_replays_image_attachment<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert!(output.text.contains("Hello Lumina!"));
...
async fn run_mode_set_impl<C: Connection>(via: SetModeVia) {
    ...
    let approve = <&str>::from(LuminaMode::Approve);
    match via {
...
pub async fn run_new_session_uses_current_config_mode<C: Connection>() {
    let temp_dir = tempfile::tempdir().unwrap();
    let config_path = temp_dir.path().join(lumina::config::base::CONFIG_YAML_NAME);
    fs::write(
        &config_path,
        format!("LUMINA_MODEL: {TEST_MODEL}\nLUMINA_PROVIDER: openai\nLUMINA_MODE: approve\n"),
    )
    .unwrap();
    ...
    let global_config_path =
        lumina::config::paths::Paths::config_dir().join(lumina::config::base::CONFIG_YAML_NAME);
    fs::write(
        &global_config_path,
        format!("LUMINA_MODEL: {TEST_MODEL}\nLUMINA_PROVIDER: openai\nLUMINA_MODE: auto\n"),
    )
...
pub async fn run_prompt_image<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert_eq!(output.text, "Hello Lumina!\nThis is a test image.");
    assert_notifications(
        &session.notifications(),
        &[
            Notification::ToolCall,
