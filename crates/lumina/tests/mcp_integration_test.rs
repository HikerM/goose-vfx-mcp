...
impl lumina::providers::base::ProviderDescriptor for MockProvider {
    fn metadata() -> lumina::providers::base::ProviderMetadata {
        lumina::providers::base::ProviderMetadata::empty()
    }
}

impl lumina::providers::base::ProviderDef for MockProvider {
    type Provider = Self;
...
#[async_trait]
impl lumina::providers::base::Provider for MockProvider {
    ...
    async fn stream(
        ...
        _tools: &[Tool],
    ) -> Result<lumina::providers::base::MessageStream, ProviderError> {
        let message = lumina::conversation::message::Message::assistant().with_text("\"So we beat on, boats against the current, borne back ceaselessly into the past.\" — F. Scott Fitzgerald, The Great Gatsby (1925)");
        let usage = ProviderUsage::new("mock".to_string(), Usage::default());
        Ok(lumina::providers::base::stream_from_single_message(
            message, usage,
        ))
    }
}
...
async fn test_replayed_session(
    ...
    let mode = if env::var("LUMINA_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
    ...
    let envs = lumina::agents::extension::Envs::new(env);
    let extension_config = lumina::agents::extension::ExtensionConfig::Stdio {
        name: "test".to_string(),
    ...
    let provider = Arc::new(tokio::sync::Mutex::new(Some(
        Arc::new(MockProvider::new()) as Arc<dyn lumina::providers::base::Provider>
    )));
    let temp_dir = tempfile::tempdir().unwrap();
    let session_manager = Arc::new(lumina::session::SessionManager::new(
        temp_dir.path().to_path_buf(),
    ));
    let extension_manager = Arc::new(lumina::agents::extension_manager::ExtensionManager::new(
        provider,
        session_manager,
        lumina::agents::LuminaPlatform::LuminaDesktop.to_string(),
        lumina::agents::extension_manager::ExtensionManagerCapabilities {
            mcpui: true,
            host_info: None,
        },
        true,
