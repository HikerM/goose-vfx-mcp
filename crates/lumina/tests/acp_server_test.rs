...
use lumina::conversation::message::{Message, MessageMetadata};
use lumina::custom_requests::{
    ExportSessionRequest, GetSessionInfoRequest, GetSessionInfoResponse, ImportSessionRequest,
    SessionImportSource,
};
use lumina::recipe::{Recipe, Settings};
use lumina::recipe_deeplink;
...
fn include_last_message_snippet_meta(
    ...
    lumina.insert("includeLastMessageSnippet".to_string(), value);
...
fn include_empty_sessions_meta(
    value: serde_json::Value,
) -> serde_json::Map<String, serde_json::Value> {
    let mut lumina = serde_json::Map::new();
    lumina.insert("includeEmpty".to_string(), value);
...
#[test]
fn test_list_sessions_can_include_empty_project_tasks() {
    run_test(async {
        let data_root = tempfile::tempdir().unwrap();
        let cwd = tempfile::tempdir().unwrap();
        let session_manager = SessionManager::new(data_root.path().to_path_buf());
        let empty_session = session_manager
            .create_session(
        ...
        let default_response =
            list_sessions_request(&conn, ListSessionsRequest::new().cwd(cwd.path()))
                .await
                .unwrap();
        assert!(default_response.sessions.is_empty());

        let response = list_sessions_request(
            &conn,
            ListSessionsRequest::new()
                .cwd(cwd.path())
                .meta(include_empty_sessions_meta(serde_json::Value::Bool(true))),
        )
        .await
        .unwrap();
        assert_eq!(response.sessions.len(), 1);
        assert_eq!(
            response.sessions[0].session_id.to_string(),
            empty_session.id
        );
    });
}
...
fn test_list_sessions_invalid_params() {
    run_test(async {
        ...
        let error = list_sessions_request(
            &conn,
            ListSessionsRequest::new().meta(include_empty_sessions_meta(
                serde_json::Value::String("true".to_string()),
            )),
        )
        .await
    ...
    run_test(async {
        ...
        let recipe = Recipe::builder()
            ...
            .settings(Settings {
                ...
                lumina_model: Some(recipe_model.to_string()),
                temperature: None,
                max_turns: None,
            })
            .build()
