...
const SOURCE_SCHEMA_FENCE_ID: &str =
    lumina::verified_source_catalog::schema::SOURCE_SCHEMA_FENCE_ID;

type SourceCatalogDao = lumina::verified_source_catalog::SourceCatalogDao;
type SourceCatalogOperation = lumina::verified_source_catalog::SourceCatalogOperation;
type SourceCatalogSafeView = lumina::verified_source_catalog::SourceCatalogSafeView;
type SourceReleaseV1 = lumina::verified_source_catalog::SourceReleaseV1;
type SourceRootKeyV1 = lumina::verified_source_catalog::SourceRootKeyV1;
type SourceSignatureV1 = lumina::verified_source_catalog::SourceSignatureV1;
type SourceSignedEnvelopeV1 = lumina::verified_source_catalog::SourceSignedEnvelopeV1;
type SourceSnapshotV1 = lumina::verified_source_catalog::SourceSnapshotV1;
type SourceState = lumina::verified_source_catalog::SourceState;
type SourceTrustRootV1 = lumina::verified_source_catalog::SourceTrustRootV1;
type SourceUnsignedPayloadV1 = lumina::verified_source_catalog::SourceUnsignedPayloadV1;
type StoredSourceOperation = lumina::verified_source_catalog::StoredSourceOperation;

async fn bootstrap_source_schema(pool: &sqlx::Pool<sqlx::Sqlite>) -> anyhow::Result<()> {
    lumina::verified_source_catalog::bootstrap_source_schema(pool).await
}

async fn ensure_source_schema_compatible(pool: &sqlx::Pool<sqlx::Sqlite>) -> anyhow::Result<()> {
    lumina::verified_source_catalog::ensure_source_schema_compatible(pool).await
}

fn parse_signed_envelope(
    raw: &[u8],
) -> anyhow::Result<lumina::verified_source_catalog::VerifiedSourceDocument> {
    lumina::verified_source_catalog::parse_signed_envelope(raw)
}

#[tokio::test]
async fn same_mcp_version_can_coexist_across_sources() {
    let pool = test_pool().await;
    lumina::verified_source_catalog::bootstrap_source_schema(&pool)
        .await
        .unwrap();
    let dao = SourceCatalogDao::new(pool.clone());

    let first = lumina::verified_source_catalog::parse_signed_envelope(&envelope_json(
        "source-a",
        "source-a-name",
        "1.0.0",
        false,
    ...
    .unwrap();
    let second = lumina::verified_source_catalog::parse_signed_envelope(&envelope_json(
        "source-b",
        "source-b-name",
        "1.0.0",
        false,
...
async fn revocation_is_source_scoped() {
    ...
    let revoked = lumina::verified_source_catalog::parse_signed_envelope(&envelope_json(
        "source-a",
        "source-a-name",
        "1.0.0",
        true,
    ...
    .unwrap();
    let active = lumina::verified_source_catalog::parse_signed_envelope(&envelope_json(
        "source-b",
        "source-b-name",
        "1.0.0",
        false,
    ...
    assert_eq!(
        ...
        lumina::verified_source_catalog::ReleaseState::Revoked
    );
    assert!(!active_release[0].revoked);
    assert_eq!(
        active_release[0].release_state,
        lumina::verified_source_catalog::ReleaseState::Active
    );
}
...
async fn safe_dto_redacts_unsafe_values_and_rejects_unsafe_input() {
    ...
    dao.record_operation(&StoredSourceOperation {
        ...
        operation_state: lumina::verified_source_catalog::dao::OperationState::Applied,
    ...
    let payload = SourceUnsignedPayloadV1 {
        ...
        snapshot: SourceSnapshotV1 {
            ...
            revocations: if revoked {
                vec![lumina::verified_source_catalog::SourceRevocationV1 {
                    release_id: "release-a".to_string(),
