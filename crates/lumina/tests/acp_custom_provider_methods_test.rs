...
use lumina::config::base::CONFIG_YAML_NAME;
use lumina::config::declarative_providers::load_provider;
use lumina::config::paths::Paths;
use lumina::config::{Config, ConfigError, DeclarativeProviderConfig};
use lumina_test_support::EnforceSessionId;
...
fn acp_catalog_and_custom_provider_methods_use_core_provider_store() {
    ...
    let _env = env_lock::lock_env([
        ("LUMINA_PATH_ROOT", Some(root_path.as_str())),
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "custom provider should be saved in Lumina's declarative provider store"
        );
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "created-custom-key",
            "custom provider create should write through Lumina's config store"
        );
        assert!(
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "updated-custom-key",
            "custom provider update should write through Lumina's config store"
        );
        ...
        assert_eq!(
            auth_reenabled_without_key.code,
            agent_client_protocol::ErrorCode::InvalidParams,
            "re-enabling authentication without a key must be rejected as invalid parameters"
        );
        assert!(
            matches!(
                Config::global().get_secret::<String>("CUSTOM_STARK_ACP_PROVIDER_API_KEY"),
        ...
        for valid_id in ["custom_openai", "openai-compat", "a1"] {
            assert!(
                lumina::config::declarative_providers::validate_provider_id(valid_id).is_ok(),
