...
use lumina::acp::server::serve_trusted_in_process;
use lumina::acp::server::{serve, AcpProviderFactory, LuminaAcpAgent, LuminaAcpAgentOptions};
pub use lumina::acp::{map_permission_response, PermissionDecision};
use lumina::agents::LuminaPlatform;
use lumina::builtin_extension::register_builtin_extensions;
use lumina::config::paths::Paths;
use lumina::config::{LuminaMode, PermissionManager};
use lumina::mcp_platform::McpPlatformService;
...
use lumina::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
use lumina::providers::base::Provider;
use lumina::providers::openai::OpenAiProvider;
use lumina::scheduler::{ScheduledJob, SchedulerError};
use lumina::scheduler_trait::SchedulerTrait;
use lumina::session::Session as LuminaSession;
use lumina::session_context::SESSION_ID_HEADER;
use lumina_test_support::{ExpectedSessionId, TEST_MODEL};
...
impl SchedulerTrait for FixtureScheduler {
    ...
    async fn sessions(
        ...
        sched_id: &str,
        _limit: usize,
    ) -> Result<Vec<(String, LuminaSession)>, SchedulerError> {
...
fn write_global_test_config(config_path: &Path, openai_base_url: &str) {
    ...
    let global_config_path = global_config_dir.join(lumina::config::base::CONFIG_YAML_NAME);
...
pub async fn spawn_acp_server_in_process_with_mcp_service(
    ...
    let config_path = data_root.join(lumina::config::base::CONFIG_YAML_NAME);
    if !config_path.exists() {
        fs::write(
            &config_path,
            format!(
                "LUMINA_MODEL: {current_model}\nLUMINA_PROVIDER: openai\nLUMINA_MODE: {}\n",
                lumina_mode
            ),
        )
        .unwrap();
    ...
    let options = LuminaAcpAgentOptions {
        provider_factory,
    ...
    let (agent, trusted) = if trusted_transport {
        #[cfg(feature = "integration-test-support")]
        {
            ...
            (
                LuminaAcpAgent::new_for_trusted_in_process(options, service_cell)
                    .await
                    .unwrap(),
                true,
            )
        ...
        (LuminaAcpAgent::new(options).await.unwrap(), false)
    };
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    if std::env::var_os("LUMINA_PATH_ROOT").is_none() {
...
pub fn https_fixture_mcp_platform_service(
    ...
    let config = Arc::new(lumina::config::Config::new_with_file_secrets(
        data_root.join(lumina::config::base::CONFIG_YAML_NAME),
        data_root.join("secrets.yaml"),
    )?);
    Ok(
        McpPlatformService::new_with_remote_http_network_policy_and_config(
            repository,
            clock,
            ids,
            options,
