...
use lumina::acp::{AcpProvider, AcpProviderConfig};
...
use lumina_test_support::{ExpectedSessionId, IgnoreSessionId, TEST_MODEL};
...
impl AcpProviderSession {
    ...
    async fn send_message(
        ...
        let mut stream = lumina::session_context::with_session_id(
...
impl Connection for AcpProviderConnection {
    ...
    async fn new(config: TestConnectionConfig, openai: OpenAiFixture) -> Self {
        ...
        let lumina_mode = config.lumina_mode;
        ...
        let provider_config = AcpProviderConfig {
            ...
            mode_mapping: LuminaMode::VARIANTS
                .iter()
                .map(|v| {
                    let mode = LuminaMode::from_str(v).unwrap();
    ...
    async fn new_session(&mut self) -> anyhow::Result<SessionData<AcpProviderSession>> {
        ...
        let lumina_id = format!("test-session-{}", self.session_counter);
        ...
        let session = AcpProviderSession {
            ...
            session_id: agent_client_protocol::schema::v1::SessionId::new(lumina_id),
