...
use lumina_providers::conversation::token_usage::Usage;
...
impl<'a> SessionUpdateBuilder<'a> {
    ...
    pub fn lumina_mode(mut self, mode: LuminaMode) -> Self {
        self.lumina_mode = Some(mode);
        self
    }
...
///
/// ```rust
/// use lumina::session::ManagedExtensionProvenance;
///
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        self.storage
            .create_session(working_dir, name, session_type, lumina_mode)
            .await
    }
    ...
    async fn import_session_guarded(
        ...
        let normalized = super::import_formats::convert_to_lumina_session_json(json)?;
...
impl Default for Session {
    fn default() -> Self {
        Self {
            ...
            provider_name: None,
            model_config: None,
            lumina_mode: LuminaMode::default(),
            archived_at: None,
            project_id: None,
            parent_session_id: None,
...
impl SessionStorage {
    ...
    async fn create_schema(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        crate::project::create_project_tables_in_tx(&mut tx).await?;
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        INSERT INTO sessions (
            ...
            provider_name, model_config_json, lumina_mode
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        "#,
        )
        .bind(&session.id)
        ...
        .bind(session.lumina_mode.to_string())
        .execute(&mut *tx)
        .await?;
    ...
    async fn apply_migration(tx: &mut sqlx::Transaction<'_, Sqlite>, version: i32) -> Result<()> {
        match version {
            ...
            8 => {
                sqlx::query(
                    r#"
                    ALTER TABLE sessions ADD COLUMN lumina_mode TEXT NOT NULL DEFAULT 'auto'
                "#,
                )
                .execute(&mut **tx)
                .await?;
            ...
            16 => {
                let has_lumina_mode = sqlx::query_scalar::<_, i32>(
            ...
            17 => {
                crate::project::create_project_tables_in_tx(tx).await?;
            }
            _ => {
    ...
    async fn create_session(
        ...
        let session = sqlx::query_as(
            r#"
                INSERT INTO sessions (id, name, user_set_name, session_type, working_dir, extension_data, lumina_mode)
                VALUES (
                    ? || '_' || CAST(COALESCE((
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.lumina_mode, "lumina_mode");
        ...
        if let Some(lumina_mode) = builder.lumina_mode {
            q = q.bind(lumina_mode.to_string());
        }
    ...
    async fn list_sessions_matching(&self, query: SessionListQuery<'_>) -> Result<Vec<Session>> {
        ...
        let sql = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.lumina_mode,
    ...
    async fn copy_session(
        ...
        builder = builder.lumina_mode(original_session.lumina_mode);
...
mod tests {
    ...
    use crate::agents::ExtensionConfig;
    use crate::agents::{Agent, AgentConfig, LuminaPlatform};
    ...
    use lumina_providers::conversation::token_usage::{CostSource, ProviderUsage};
    ...
    impl Provider for NamingTestProvider {
        ...
        ) -> std::result::Result<MessageStream, lumina_providers::errors::ProviderError> {
    ...
    #[tokio::test]
    async fn test_lumina_mode_persists(mode: LuminaMode) {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.lumina_mode, mode);
    }

    #[tokio::test]
    async fn test_lumina_mode_update() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sm.update(&session.id)
            .lumina_mode(LuminaMode::Approve)
            .apply()
            .await
            .unwrap();

        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.lumina_mode, LuminaMode::Approve);
    }

    #[tokio::test]
    async fn test_lumina_mode_malformed_defaults_to_auto() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sqlx::query("UPDATE sessions SET lumina_mode = 'garbage' WHERE id = ?")
            .bind(&session.id)
            .execute(pool)
            .await
            .unwrap();
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.lumina_mode, LuminaMode::default());
    }
