//! Importers for non-lumina session formats.
//!
//! Lumina's native session export is a JSON-serialized [`crate::session::Session`].
...
//!
//! The strategy is to convert any supported foreign format into lumina's
...
/// the lumina-native session JSON handed to `SessionManager::import_session`.
...
pub(crate) fn build_session_json(session: ImportedSession) -> Value {
    ...
    obj.insert("lumina_mode".into(), json!("auto"));
...
pub enum ImportFormat {
    /// Native lumina session export — a JSON object representing a `Session`.
    Lumina,
...
///   and a `conversation` (or `messages`) field, it's lumina.
...
pub fn detect_format(content: &str) -> ImportFormat {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(first_line) {
        ...
        // Claude Code lines always include a sessionId; lumina's native JSON is
    ...
    // Lumina's pretty-printed export starts with `{` and *eventually* contains
...
/// Convert any supported foreign format to a lumina-native session JSON string.
pub fn convert_to_lumina_session_json(content: &str) -> Result<String> {
    match detect_format(content) {
        ImportFormat::Lumina => Ok(upgrade_legacy_token_fields(content)),
...
/// Exports from lumina versions before `usage`/`accumulated_usage` existed
