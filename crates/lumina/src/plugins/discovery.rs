...
fn project_settings_path(project_root: &Path, local: bool) -> PathBuf {
    ...
    project_root.join(".config").join("lumina").join(file)
}
...
mod tests {
    ...
    fn test_config(dir: &Path) -> Config {
        Config::new(dir.join("config.yaml"), "lumina-discovery-test").unwrap()
    }
    ...
    fn disabled_in_project_settings_drops_plugin() {
        ...
        write_settings(
            &project.join(".config").join("lumina"),
    ...
    fn project_scope_overrides_user_scope() {
        ...
        write_settings(
            &fake_home.path().join(".config").join("lumina"),
            r#"{"disabledPlugins":["demo"]}"#,
        );
        ...
        let prev = std::env::var("LUMINA_PATH_ROOT").ok();
        unsafe { std::env::set_var("LUMINA_PATH_ROOT", fake_home.path()) };
        let found = discover(project);
        match prev {
            Some(v) => unsafe { std::env::set_var("LUMINA_PATH_ROOT", v) },
            None => unsafe { std::env::remove_var("LUMINA_PATH_ROOT") },
        }
