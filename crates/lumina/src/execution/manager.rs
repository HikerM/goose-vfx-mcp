use crate::agents::mcp_client::LuminaMcpHostInfo;
use crate::agents::{Agent, AgentConfig, ExtensionLoadResult, LuminaPlatform};
...
impl AgentManager {
        ...
        AGENT_MANAGER
            .get_or_try_init(|| async {
                ...
                let max_sessions = config
                    .get_lumina_max_active_agents()
                    .unwrap_or(DEFAULT_MAX_SESSION);
                let default_mode = config.get_lumina_mode().unwrap_or_default();
    ...
    async fn create_agent_locked(
        ...
        let mut mode = self.agent_config.lumina_mode;
        if let Ok(session) = self
            ...
            .await
        {
            mode = session.lumina_mode;
            info!(lumina_mode = %mode, session_id = %session_id, "Session loaded");
        }

        let mut config = self.agent_config.clone();
        config.lumina_mode = mode;
        ...
        if agent.provider().await.is_err() {
            if let Some(provider) = &*self.default_provider.read().await {
                ...
                let model_config = config
                    .get_lumina_provider()
                    .ok()
                    .zip(config.get_lumina_model().ok())
                    ...
                    .unwrap_or_else(|| lumina_providers::model::ModelConfig::new("unknown"));
                agent
...
mod tests {
    ...
    use crate::agents::{AgentConfig, LuminaPlatform};
    use crate::config::permission::PermissionManager;
    ...
    #[test_case(LuminaMode::Approve ; "approve")]
    #[test_case(LuminaMode::Chat ; "chat")]
    #[test_case(LuminaMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: LuminaMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.lumina_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.lumina_mode().await, LuminaMode::Approve);
        assert_eq!(a2.lumina_mode().await, LuminaMode::Auto);
    }
}
