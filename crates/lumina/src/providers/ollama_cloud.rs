...
use lumina_providers::api_client::{ApiClient, AuthMethod, TlsConfig};
...
use lumina_providers::ollama::fetch_ollama_model_names;
use lumina_providers::openai::OpenAiProvider;
use rmcp::model::Tool;
...
fn build_ollama_api_client(
    ...
    let normalized_base_url = lumina_providers::openai::ensure_url_scheme(&config.base_url);
...
impl Provider for OllamaCloudProvider {
    ...
    fn retry_config(&self) -> lumina_providers::retry::RetryConfig {
        ...
        Box::pin(async {
            anyhow::bail!(
                "Ollama Cloud must be configured as a declarative provider. \
                 Run `lumina configure` to set it up."
            )
        })
    }
}
