...
pub fn oauth_client_id() -> Option<&'static str> {
    option_env!("LUMINA_HUGGINGFACE_OAUTH_CLIENT_ID")
...
pub async fn resolve_token_async_with_provider_token(
    ...
    let oauth_token = async {
        let Some(client_id) = oauth_client_id() else {
            return Ok(None);
        };
        refreshed_or_usable_oauth_token_from_path(&oauth_cache_path(), client_id, TOKEN_URL).await
    };
    resolve_token_async_from_sources(provider_token, oauth_token, hf_token_secret).await
}
...
pub async fn configure_oauth() -> Result<()> {
    let client_id = oauth_client_id().ok_or_else(|| {
        anyhow!(
            "Hugging Face OAuth is unavailable because this Lumina build has no owned client ID"
        )
    })?;
...
<html>
  <head>
    <title>lumina - Hugging Face Authorization Successful</title>
...
<html>
  <head>
    <title>lumina - Hugging Face Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex; justify-content: center; align-items: center;
...
async fn perform_loopback_oauth_flow(client_id: &str) -> Result<HuggingFaceTokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize lumina with Hugging Face:\n{}",
            auth_url
        );
    }
...
mod tests {
    ...
    #[test]
    fn oauth_client_id_requires_lumina_owned_configuration() {
        if option_env!("LUMINA_HUGGINGFACE_OAUTH_CLIENT_ID").is_none() {
