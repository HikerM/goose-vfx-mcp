...
use crate::config::{Config, ExtensionConfig, LuminaMode};
use crate::conversation::message::{Message, MessageContent};
use crate::subprocess::configure_subprocess;
use lumina_providers::errors::ProviderError;
use lumina_providers::model::ModelConfig;
use lumina_providers::request_log::{start_log, LoggerHandleExt};
use rmcp::model::Role;
use rmcp::model::Tool;
...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, LuminaMode>>,
}
...
impl CodexProvider {
    ...
    /// Apply permission flags based on LuminaMode
    ...
    async fn execute_command(
        ...
        if std::env::var("LUMINA_CODEX_DEBUG").is_ok() {
        ...
        // Let the signed-in Codex CLI select an account-compatible model by default.
        // Explicit custom model names are forwarded so the provider remains future-proof.
        if model.model_name != CODEX_DEFAULT_MODEL && !model.model_name.trim().is_empty() {
        ...
        Self::apply_permission_flags(&mut cmd, lumina_mode)?;
...
impl lumina_providers::base::ProviderDescriptor for CodexProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            CODEX_PROVIDER_NAME,
            "ChatGPT Codex (Official CLI)",
            "Run models through the official Codex CLI using its supported ChatGPT or API-key authentication. Install the CLI and complete `codex login` first.",
            CODEX_DEFAULT_MODEL,
            CODEX_KNOWN_MODELS.to_vec(),
            CODEX_DOC_URL,
            vec![
                ConfigKey::new("CODEX_COMMAND", true, false, Some("codex"), true),
                ConfigKey::new("CODEX_SKIP_GIT_CHECK", false, false, Some("true"), true),
            ],
        )
    }
}
...
impl ProviderDef for CodexProvider {
    ...
    fn from_env(
        ...
        Box::pin(async move {
            ...
            // Desktop chats may start outside a Git repository.
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(model_config, system, messages, tools, lumina_mode)
            .await?;
...
mod tests {
    ...
    use lumina_providers::base::ProviderDescriptor as _;
    use lumina_test_support::TEST_IMAGE_B64;
    use std::collections::HashMap;
    ...
    fn test_known_models() {
        assert_eq!(CODEX_KNOWN_MODELS, &["default"]);
    }
    ...
    #[test_case(LuminaMode::Auto, &["--yolo"] ; "auto_yolo")]
    #[test_case(LuminaMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(LuminaMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(LuminaMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: LuminaMode, expected: &[&str]) {
