pub use lumina_providers::canonical::catalog::{
...
pub async fn get_providers_by_format(format: ProviderFormat) -> Vec<ProviderCatalogEntry> {
    ...
    lumina_providers::canonical::catalog::get_providers_by_format(format, &native_provider_ids)
}
...
pub async fn get_setup_catalog_entries() -> Vec<ProviderSetupCatalogEntry> {
    ...
    lumina_providers::canonical::catalog::get_setup_catalog_entries(&registry_metadata)
}

pub fn get_provider_setup_category(provider_id: &str) -> Option<ProviderSetupCategory> {
    lumina_providers::canonical::catalog::get_provider_setup_category(provider_id)
}

pub fn get_provider_template(provider_id: &str) -> Option<ProviderTemplate> {
    lumina_providers::canonical::catalog::get_provider_template(provider_id)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn setup_catalog_includes_lumina_and_curated_fields() {
        let entries = get_setup_catalog_entries().await;

        let lumina = entries
            .iter()
            .find(|entry| entry.provider_id == "lumina")
            .expect("setup catalog should include synthetic lumina");
        assert_eq!(lumina.category, ProviderSetupCategory::Agent);
        assert_eq!(lumina.setup_method, ProviderSetupMethod::None);
        assert!(lumina.fields.is_empty());
