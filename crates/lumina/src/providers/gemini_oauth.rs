...
fn google_oauth_client_id() -> Result<String> {
    match std::env::var("LUMINA_GEMINI_OAUTH_CLIENT_ID") {
        ...
        _ => {
            ...
            #[cfg(not(test))]
            return Err(anyhow!(
                "Gemini OAuth is unavailable until the distributor configures LUMINA_GEMINI_OAUTH_CLIENT_ID"
            ));
        }
    }
}
...
fn google_oauth_client_secret() -> Result<String> {
    match std::env::var("LUMINA_GEMINI_OAUTH_CLIENT_SECRET") {
        Ok(value) if !value.trim().is_empty() => Ok(value),
        _ => {
            #[cfg(test)]
            return Ok("lumina-test-secret".to_string());
            #[cfg(not(test))]
            return Err(anyhow!(
                "Gemini OAuth is unavailable until the distributor configures LUMINA_GEMINI_OAUTH_CLIENT_SECRET"
            ));
        }
    }
}
...
<html>
  <head>
    <title>lumina - Google Authorization Successful</title>
    <style>
      body {
...
<html>
  <head>
    <title>lumina - Google Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex;
...
impl lumina_providers::base::ProviderDescriptor for GeminiOAuthProvider {
...
mod tests {
    ...
    fn test_build_authorize_url() {
        ...
        assert!(url.contains(&format!("client_id={}", google_oauth_client_id().unwrap())));
    ...
    fn test_token_cache_roundtrip() {
        ...
        let _guard = env_lock::lock_env([("LUMINA_PATH_ROOT", Some(root_path.as_str()))]);
