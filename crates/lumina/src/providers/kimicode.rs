...
use lumina_providers::formats::anthropic::{AnthropicFormatOptions, ANTHROPIC_PROVIDER_NAME};
...
const KIMI_MSH_PLATFORM: &str = "lumina";
const KIMI_MSH_VERSION: &str = env!("CARGO_PKG_VERSION");
...
fn kimi_code_client_id() -> Result<String> {
    match std::env::var("LUMINA_KIMI_CODE_CLIENT_ID") {
        ...
        _ => {
            ...
            return Err(anyhow::anyhow!(
                "Kimi Code OAuth is unavailable until the distributor configures LUMINA_KIMI_CODE_CLIENT_ID"
            ));
        }
    }
}
...
impl KimiCodeProvider {
    ...
    async fn device_flow_login(&self) -> Result<KimiToken> {
        ...
        let client_id = kimi_code_client_id()?;
...
impl lumina_providers::base::ProviderDescriptor for KimiCodeProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            ...
            KIMI_CODE_DOC_URL,
            // Marker key — the actual token lives in ~/.config/lumina/kimicode/token.json.
        ...
        .with_setup_steps(vec![
            "Run `lumina configure` and select 'Kimi Code'",
            "A browser window will open — log in to kimi.com and enter the displayed code",
            "Once authorized, Lumina will save your token automatically",
        ])
    }
}
        ...
        KimiCodeProvider {
            ...
            token_cache: TokenCache {
                path: std::env::temp_dir()
                    .join(format!("lumina-kimicode-test-{}.json", Uuid::new_v4())),
            },
            cached_token: tokio::sync::Mutex::new(None),
