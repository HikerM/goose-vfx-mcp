...
fn oauth_client_id() -> Result<String> {
    match std::env::var("LUMINA_XAI_OAUTH_CLIENT_ID") {
        Ok(value) if !value.trim().is_empty() => Ok(value),
        _ => {
            #[cfg(test)]
            return Ok("lumina-test-client".to_string());
            #[cfg(not(test))]
            return Err(anyhow!(
                "xAI OAuth is unavailable until the distributor configures LUMINA_XAI_OAUTH_CLIENT_ID"
            ));
        }
    }
}

// xAI rejects redirect_uris that don't match the distributor's registered
// OAuth client. The host:port pair is part of that registration, so the
// loopback server must bind to this exact port.
...
async fn poll_device_code_token(device: &DeviceCodeResponse) -> Result<TokenResponse> {
    ...
    loop {
        ...
        match body.error.as_deref() {
            ...
            Some("expired_token") => {
                return Err(anyhow!(
                    "xAI device code expired - please re-run lumina configure"
                ));
            }
            other => {
                let detail = body
...
<html>
  <head>
    <title>lumina - xAI Authorization Successful</title>
    <style>
      body {
...
<html>
  <head>
    <title>lumina - xAI Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        ...
        if e.kind() == io::ErrorKind::AddrInUse {
            anyhow!(
                "xAI OAuth callback server failed to bind to {}: port {} is already in use \
                 (another local OAuth client may be bound to the same port). \
...
async fn perform_loopback_oauth_flow(auth_state: &XaiAuthState) -> Result<TokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize lumina with xAI:\n{}",
            auth_url
        );
    }
...
async fn perform_device_code_flow() -> Result<TokenData> {
    ...
    eprintln!(
        "\nTo authorize lumina with xAI, open this URL in any browser:\n  {}\nand enter code: {}\n",
...
impl lumina_providers::base::ProviderDescriptor for XaiOAuthProvider {
...
mod tests {
    ...
    fn authorize_url_contains_required_oauth_params() {
        ...
        assert!(url.contains(&format!("client_id={}", oauth_client_id().unwrap())));
        ...
        assert!(url.contains("referrer=lumina"));
    ...
    #[test]
    fn redirect_uri_is_stable_for_distributor_registration() {
        assert_eq!(redirect_uri(), "http://127.0.0.1:56121/callback");
    }

    #[test]
    fn token_cache_path_lives_under_lumina_config_dir() {
