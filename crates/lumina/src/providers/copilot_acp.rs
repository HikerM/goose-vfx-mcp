...
impl lumina_providers::base::ProviderDescriptor for CopilotAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            COPILOT_ACP_PROVIDER_NAME,
            "GitHub Copilot CLI (ACP)",
            "Use lumina with your GitHub Copilot subscription via the Copilot CLI.",
            ACP_CURRENT_MODEL,
            vec![],
            COPILOT_ACP_DOC_URL,
            vec![],
    ...
    fn from_env_with_working_dir(
        ...
        Box::pin(async move {
            ...
            let mode_mapping = HashMap::from([
                (LuminaMode::Auto, vec![MODE_AGENT.to_string()]),
                (LuminaMode::Approve, vec![MODE_AGENT.to_string()]),
                (LuminaMode::SmartApprove, vec![MODE_AGENT.to_string()]),
                (LuminaMode::Chat, vec![MODE_PLAN.to_string()]),
            ]);
