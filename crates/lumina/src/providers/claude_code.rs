...
pub struct ClaudeCodeProvider {
    ...
    #[serde(skip)]
    initial_mode: tokio::sync::Mutex<Option<LuminaMode>>,
}
...
impl ClaudeCodeProvider {
    ...
    fn build_stream_json_command(&self) -> Command {
        ...
        // Allow lumina to run inside a Claude Code session.
    ...
    fn apply_permission_flags(cmd: &mut Command) -> Result<bool, ProviderError> {
        ...
        match lumina_mode {
            ...
            LuminaMode::SmartApprove | LuminaMode::Approve => {
                cmd.arg("--permission-prompt-tool").arg("stdio");
                Ok(true)
            }
            LuminaMode::Chat => Ok(false),
        }
    }
...
impl lumina_providers::base::ProviderDescriptor for ClaudeCodeProvider {
...
impl Provider for ClaudeCodeProvider {
    ...
    async fn update_mode(&self, _session_id: &str, mode: LuminaMode) -> Result<(), ProviderError> {
...
mod tests {
    ...
    use chrono::Utc;
    use lumina_test_support::session::TEST_SESSION_ID;
    use serde_json::json;
    use std::collections::HashMap;
    use std::fs;
