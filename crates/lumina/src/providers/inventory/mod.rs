...
mod tests {
    ...
    #[test]
    fn databricks_v2_inventory_keeps_the_first_id_for_duplicate_names() {
        let models = enrich_model_ids_with_canonical(
            "databricks_v2",
            &[
                "databricks-gpt-5-5".to_string(),
                "tenant-gpt-5-5".to_string(),
            ],
        );

        assert!(
            models.iter().any(|model| model.id == "databricks-gpt-5-5"),
            "expected the provider's first identifier to remain stable, got {models:?}"
        );
    }
