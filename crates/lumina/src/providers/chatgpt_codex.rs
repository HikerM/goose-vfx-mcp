...
use lumina_providers::formats::openai_responses::responses_api_to_streaming_message;
...
fn oauth_client_id() -> Result<String> {
    match std::env::var("LUMINA_CHATGPT_CODEX_CLIENT_ID") {
        ...
        _ => {
            ...
            #[cfg(not(test))]
            return Err(anyhow!(
                "ChatGPT Codex OAuth is unavailable until the distributor configures LUMINA_CHATGPT_CODEX_CLIENT_ID"
            ));
        }
    }
}

pub fn oauth_distribution_configured() -> bool {
    #[cfg(test)]
    {
        true
    }
    #[cfg(not(test))]
    {
        std::env::var("LUMINA_CHATGPT_CODEX_CLIENT_ID")
...
<html>
  <head>
    <title>lumina - ChatGPT Authorization Successful</title>
    <style>
      body {
...
<html>
  <head>
    <title>lumina - ChatGPT Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex;
...
impl lumina_providers::base::ProviderDescriptor for ChatGptCodexProvider {
