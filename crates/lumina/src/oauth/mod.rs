...
pub use persist::LuminaCredentialStore;
...
const CLIENT_METADATA_URL: Option<&str> = option_env!("LUMINA_OAUTH_CLIENT_METADATA_URL");
const DEFAULT_OAUTH_CALLBACK_TIMEOUT_SECS: u64 = 300;
const OAUTH_CALLBACK_TIMEOUT_ENV: &str = "LUMINA_OAUTH_CALLBACK_TIMEOUT_SECONDS";
...
pub async fn oauth_flow(
    ...
    let client_metadata_url = CLIENT_METADATA_URL.ok_or_else(|| {
        anyhow::anyhow!(
            "OAuth is unavailable because this Lumina build has no owned client metadata URL"
        )
    })?;
    let credential_store = LuminaCredentialStore::new(name.clone());
    ...
    let port: u16 = std::env::var("LUMINA_OAUTH_CALLBACK_PORT")
        .ok()
