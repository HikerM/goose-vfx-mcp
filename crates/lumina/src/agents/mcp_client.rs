...
/// MCP client implementation for Lumina
use rmcp::{
    model::{
...
fn resolve_sampling_model_config() -> anyhow::Result<lumina_providers::model::ModelConfig> {
...
impl LuminaClient {
    ...
    fn resolved_client_info(&self) -> Implementation {
        ...
        let version = self
            ...
            .unwrap_or_else(|| {
                std::env::var("LUMINA_MCP_CLIENT_VERSION")
...
impl ClientHandler for LuminaClient {
    async fn list_roots(
        &self,
    ...
    async fn create_message(
        ...
        let system_prompt = params
            .system_prompt
            .as_deref()
            .unwrap_or("You are a general-purpose AI agent called Lumina");
...
#[derive(Debug, Clone)]
pub struct LuminaMcpClientCapabilities {
    pub mcpui: bool,
...
pub struct McpClient {
    client: Mutex<RunningService<RoleClient, LuminaClient>>,
...
impl McpClient {
        ...
        E: std::error::Error + From<std::io::Error> + Send + Sync + 'static,
    {
        ...
        let client: rmcp::service::RunningService<rmcp::RoleClient, LuminaClient> =
...
mod tests {
    use super::*;
    use crate::agents::LuminaPlatform;
    use serde_json::json;
    use test_case::test_case;

    fn new_client(platform: LuminaPlatform) -> LuminaClient {
        let capabilities = match platform {
            LuminaPlatform::LuminaDesktop => LuminaMcpClientCapabilities {
                mcpui: true,
                host_info: None,
            },
            LuminaPlatform::LuminaCli => LuminaMcpClientCapabilities {
                mcpui: false,
                host_info: None,
            },
        };
    ...
    fn test_resolve_session_id(
        ...
        runtime.block_on(async {
            let client = new_client(LuminaPlatform::LuminaCli);
    ...
    fn test_request_injects_session(request_builder: fn(Extensions) -> ClientRequest) {
        ...
        extensions.insert(
            serde_json::from_value::<Meta>(json!({
                "Lumina-Session-Id": "old-session-id",
    ...
    fn test_client_info_advertises_mcp_apps_ui_extension() {
        let client = new_client(LuminaPlatform::LuminaDesktop);
    ...
    fn test_explicit_host_info_passes_through_client_identity() {
        let client = LuminaClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            LuminaPlatform::LuminaDesktop.to_string(),
            LuminaMcpClientCapabilities {
                mcpui: true,
                host_info: Some(LuminaMcpHostInfo {
                    explicit_extensions: true,
                    extensions: ExtensionCapabilities::new(),
                    client_name: Some("lumina2".to_string()),
        ...
        let info = ClientHandler::get_info(&client);
        assert_eq!(info.client_info.name, "lumina2");
    ...
    fn test_explicit_host_extensions_override_platform_fallback() {
        let client = LuminaClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            LuminaPlatform::LuminaCli.to_string(),
            LuminaMcpClientCapabilities {
                mcpui: false,
                host_info: Some(LuminaMcpHostInfo {
                    explicit_extensions: true,
