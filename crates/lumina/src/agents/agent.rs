...
use super::mcp_client::LuminaMcpHostInfo;
...
use crate::config::{get_enabled_extensions, Config, LuminaMode};
...
use lumina_providers::conversation::token_usage::ProviderUsage;
...
const COMPACTION_PROGRESS_TEXT: &str = "Lumina is compacting the conversation...";
...
fn stop_hook_block_cap_warning(plugin: &str, cap: u32) -> Message {
    Message::assistant().with_system_notification(
        SystemNotificationType::InlineMessage,
        format!(
            "Stop hook `{plugin}` blocked the turn from ending more than {cap} consecutive times — overriding and ending turn to avoid an infinite loop. Set LUMINA_STOP_HOOK_BLOCK_CAP to raise this limit."
        ),
    )
}
...
pub struct ReplyContext {
    ...
    pub lumina_mode: LuminaMode,
    pub tool_call_cut_off: usize,
    pub initial_messages: Vec<Message>,
    pub model_config: lumina_providers::model::ModelConfig,
}
...
#[derive(Clone, Debug)]
pub enum LuminaPlatform {
    LuminaDesktop,
    LuminaCli,
}

impl fmt::Display for LuminaPlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            LuminaPlatform::LuminaCli => write!(f, "lumina-cli"),
            LuminaPlatform::LuminaDesktop => write!(f, "lumina-desktop"),
        }
    }
}
...
pub struct AgentConfig {
    ...
    pub mcp_host_info: Option<LuminaMcpHostInfo>,
...
impl AgentConfig {
    pub fn new(
        ...
        lumina_platform: LuminaPlatform,
    ) -> Self {
        Self {
            session_manager,
            permission_manager,
    ...
    pub fn with_mcp_host_info(mut self, mcp_host_info: Option<LuminaMcpHostInfo>) -> Self {
    ...
    fn resolve_use_login_shell_path(&self) -> bool {
        resolve_use_login_shell_path(self.use_login_shell_path, &self.lumina_platform)
    }
}

fn resolve_use_login_shell_path(explicit: Option<bool>, platform: &LuminaPlatform) -> bool {
    explicit.unwrap_or(matches!(platform, LuminaPlatform::LuminaDesktop))
}

/// The main lumina Agent
pub struct Agent {
    pub(super) provider: SharedProvider,
    pub config: AgentConfig,
    pub(super) current_lumina_mode: Mutex<LuminaMode>,
...
impl Agent {
    pub fn new() -> Self {
        ...
        Self::with_config(AgentConfig::new(
            ...
            None,
            config.get_lumina_mode().unwrap_or_default(),
            config.get_lumina_disable_session_naming().unwrap_or(false),
            LuminaPlatform::LuminaCli,
        ))
    }
    ...
    pub fn with_config(config: AgentConfig) -> Self {
        ...
        let lumina_platform = config.lumina_platform.clone();
        let initial_mode = config.lumina_mode;
        ...
        let mcpui = explicit_mcp_host_info
            ...
            .map(LuminaMcpHostInfo::mcpui_enabled)
            .unwrap_or_else(|| match config.lumina_platform {
                LuminaPlatform::LuminaDesktop => true,
                LuminaPlatform::LuminaCli => false,
            });
        ...
        let client_name = explicit_mcp_host_info
            ...
            .unwrap_or_else(|| lumina_platform.to_string());
        ...
        Self {
            ...
            config,
            current_lumina_mode: Mutex::new(initial_mode),
    ...
    fn stop_hook_block_cap(&self) -> u32 {
        ...
        Config::global()
            .get_param::<u32>("LUMINA_STOP_HOOK_BLOCK_CAP")
    ...
    fn create_tool_inspection_manager(
        ...
        // Add adversary inspector (LLM-based review, enabled by ~/.config/lumina/adversary.md)
    ...
    async fn prepare_reply_context(
        ...
        let lumina_mode = *self.current_lumina_mode.lock().await;

        if lumina_mode == LuminaMode::SmartApprove {
            self.tool_inspection_manager.apply_tool_annotations(&tools);
        }

        let tool_call_cut_off = match Config::global().get_param::<usize>("LUMINA_TOOL_CALL_CUTOFF")
        {
            Ok(v) => v,
            Err(_) => {
                let context_limit = match self.provider().await {
                    ...
                    Err(_) => lumina_providers::model::DEFAULT_CONTEXT_LIMIT,
                };
                let compaction_threshold = Config::global()
                    .get_param::<f64>("LUMINA_AUTO_COMPACT_THRESHOLD")
    ...
    async fn reply_internal(
        ...
        let reply_stream_span = tracing::info_span!(
            ...
            session.agent_type = "lumina",
        );
        let inner = Box::pin(async_stream::try_stream! {
            let mut turns_taken = 0u32;
            let max_turns = session_config.max_turns.unwrap_or_else(|| {
                Config::global()
                    .get_param::<u32>("LUMINA_MAX_TURNS")
                    ...
                    match next {
                        Ok((response, usage)) => {
                            ...
                            if let Some(response) = response {
                                ...
                                if lumina_mode == LuminaMode::Chat {
    ...
    pub async fn update_lumina_mode(&self, mode: LuminaMode, session_id: &str) -> Result<()> {
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
        ...
        *self.current_lumina_mode.lock().await = mode;
        self.config
            .session_manager
            .clone()
            .update(session_id)
            ...
            .apply()
            .await
            .context("Failed to persist lumina_mode to session")
    }

    pub async fn lumina_mode(&self) -> LuminaMode {
        *self.current_lumina_mode.lock().await
    }
    ...
    pub async fn recreate_provider_for_session(
        ...
        let mode = self.lumina_mode().await;
        self.update_lumina_mode(mode, session_id).await
    }
    ...
    pub(crate) async fn restore_provider_from_validated_bundle(
        ...
        let provider_name = session
            .provider_name
            .clone()
            .or_else(|| config.get_lumina_provider().ok())
        ...
        let (provider, active_model_config, provider_changed) =
                ...
                .is_ok()
            {
                ...
                let fallback_model_name = config.get_lumina_model().ok().ok_or_else(|| {
        ...
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
                .update_mode(&session.id, session.lumina_mode)
                .await
                .map_err(|e| anyhow!("Failed to propagate mode to provider: {}", e))?;
        }
        *self.current_lumina_mode.lock().await = session.lumina_mode;
        Ok(provider_changed)
    }
    ...
    pub async fn create_recipe(
        ...
        let settings = Settings {
            lumina_provider: Some(provider_name.clone()),
            lumina_model: Some(model_name.clone()),
...
mod tests {
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        async fn stream(
            &self,
            _: &lumina_providers::model::ModelConfig,
            _: &str,
...
"#,
    ...
    impl crate::providers::base::Provider for CountingTextProvider {
        async fn stream(
            &self,
            _model_config: &lumina_providers::model::ModelConfig,
            _system_prompt: &str,
            _messages: &[Message],
            _tools: &[Tool],
    ...
    async fn create_test_agent(
        ...
        agent
            .update_provider(
                provider,
                lumina_providers::model::ModelConfig::new("mock-model"),
                &session.id,
            )
            .await?;
        Ok((agent, session.id))
                ...
                matches!(
                    ...
                    MessageContent::SystemNotification(notification)
                        if notification.msg.contains("more than 2 consecutive times")
                            && notification.msg.contains("LUMINA_STOP_HOOK_BLOCK_CAP")
                )
            })
        }));
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_lumina_mode(LuminaMode::default())
            .build();
