...
use crate::agents::mcp_client::{
    LuminaMcpClientCapabilities, LuminaMcpHostInfo, McpClient, McpClientTrait,
};
...
use crate::oauth::{oauth_flow, LuminaCredentialStore};
...
fn resolve_timeout(timeout: Option<u64>) -> u64 {
    timeout.unwrap_or_else(|| {
        Config::global()
            .get_lumina_default_extension_timeout()
...
pub struct ExtensionManagerCapabilities {
    pub mcpui: bool,
    pub host_info: Option<LuminaMcpHostInfo>,
}

#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct LuminaMcpAppToolAttachment {
...
pub(crate) const TRUSTED_TOOL_UPDATE_META_KEY: &str = "__lumina_tool_update_meta";

/// Manages lumina extensions / MCP clients and their interactions
...
fn is_core_managed_docker_projection(name: &str, args: &[String]) -> bool {
    ...
    args.get(..baseline.len())
        ...
        && args.get(baseline.len()).is_some_and(|label| {
            label == &format!("io.github.hikerm.lumina.managed=managed_{suffix}")
        })
}
...
const TOOL_EXTENSION_META_KEY: &str = "lumina_extension";
...
fn remove_untrusted_mcp_app_meta(result: &mut CallToolResult) {
    ...
    let remove_lumina = meta
        .0
        .get_mut("lumina")
        .and_then(Value::as_object_mut)
        .map(|lumina_meta| {
            lumina_meta.remove("mcpApp");
            lumina_meta.is_empty()
        })
        .unwrap_or(false);

    if remove_lumina {
        meta.0.remove("lumina");
    }
...
fn insert_trusted_tool_update_meta(
    ...
    attachment: &LuminaMcpAppToolAttachment,
) {
...
async fn child_process_client(
    ...
    client_name: String,
    capabilities: LuminaMcpClientCapabilities,
...
const LUMINA_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("lumina/", env!("CARGO_PKG_VERSION")));
...
async fn connect_with_auth(
    ...
    auth_headers.insert(reqwest::header::USER_AGENT, LUMINA_USER_AGENT);
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, LUMINA_USER_AGENT);
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        LUMINA_USER_AGENT
            ...
            .unwrap_or("lumina")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("lumina")),
    );
...
impl ExtensionManager {
    fn mcp_client_capabilities(&self) -> LuminaMcpClientCapabilities {
        LuminaMcpClientCapabilities {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| std::env::var("LUMINA_WORKING_DIR").ok().map(PathBuf::from))
            ...
            ExtensionConfig::StreamableHttp {
                ...
                create_streamable_http_client(
                    ...
                    *timeout,
                    ...
                    Box::new(LuminaCredentialStore::new(name.to_string())),
                    self.provider.clone(),
    ...
    async fn hydrate_mcp_app_attachment(
        ...
        cancellation_token: CancellationToken,
    ) -> Option<LuminaMcpAppToolAttachment> {
        let resource_uri = resolved_tool.resource_uri.clone()?;

        let mut attachment = LuminaMcpAppToolAttachment {
...
pub(crate) mod tests {
    ...
    fn only_core_managed_docker_projection_receives_runtime_isolation() {
        let args = vec![
            ...
            "256",
            "--label",
            "io.github.hikerm.lumina.managed=managed_example",
        ]
        .into_iter()
        .map(str::to_string)
        .collect::<Vec<_>>();
        ...
        renderer_args[11] = "io.github.hikerm.lumina.managed=managed_other".to_string();
        assert!(!is_core_managed_docker_projection(
            "managed_mcp_example",
            &renderer_args
        ));
    ...
    fn test_insert_trusted_tool_update_meta_stores_backend_payload() {
        let mut result = CallToolResult::success(vec![]);
        let attachment = LuminaMcpAppToolAttachment {
    ...
    async fn test_invalid_header_name_returns_config_error() {
        ...
        let capabilities = LuminaMcpClientCapabilities {
            mcpui: false,
            host_info: None,
        };
