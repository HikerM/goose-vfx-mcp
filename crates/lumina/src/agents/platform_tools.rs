...
pub fn manage_schedule_tool() -> Tool {
    Tool::new(
        ...
        indoc! {r#"
            Manage lumina's internal scheduled recipe execution.

            Actions:
            - "list": List all lumina scheduled jobs
            - "create": Create a new lumina scheduled job from a recipe file
            - "run_now": Execute a lumina scheduled job immediately
            - "pause": Pause a lumina scheduled job
            - "unpause": Resume a paused lumina scheduled job
            - "delete": Remove a lumina scheduled job
            - "kill": Terminate a currently running lumina scheduled job
            - "inspect": Get details about a running lumina scheduled job
            - "sessions": List execution history for a lumina scheduled job
