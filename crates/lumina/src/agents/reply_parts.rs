...
use lumina_providers::conversation::token_usage::{
...
impl Agent {
    ...
    pub(crate) async fn stream_response_from_provider(
        ...
        let model_config = model_config
            .with_default_thinking_effort(Config::global().get_lumina_thinking_effort());
                    ...
                    if let Ok(ref mut tool_call) = coerced_req.tool_call {
                        if let Some(tool) = tools.iter().find(|t| t.name == tool_call.name) {
                            ...
                            if let Some(ref meta) = tool.meta {
                                ...
                                // lumina.external_dispatch) survive coercion.
...
mod tests {
    ...
    use lumina_providers::conversation::token_usage::{ProviderStats, ProviderUsage, Usage};
    use lumina_providers::model::ModelConfig;
    use rmcp::object;
    ...
    async fn categorize_tool_requests_skips_externally_dispatched_and_preserves_marker() {
        // External requests must (1) survive coercion with lumina.external_dispatch
        ...
        assert!(
            ...
            "lumina.external_dispatch marker was clobbered by coercion; merged tool_meta = {:?}",
            tool_req.tool_meta
        );
        let merged = tool_req
            .tool_meta
