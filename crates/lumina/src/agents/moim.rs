...
This block is generated by lumina and contains current operational context such as:
- current time
- working directory
- compaction status
- turn budget
...
mod tests {
    ...
    /// The turn-context block is produced here (`compose_moim`, in lumina) but
    /// recognized in a different crate (`is_turn_context_text`, in
    /// lumina-providers) by matching its textual shape. That recognition is the
            ...
            for (label, block) in &cases {
                assert!(
                    ...
                    "is_turn_context_text rejected real compose_moim output ({label}); the \
                     producer in lumina and the detector in lumina-providers have drifted, so the \
