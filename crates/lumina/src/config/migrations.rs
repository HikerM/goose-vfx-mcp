...
const RETIRED_CONFIG_KEYS: &[&str] = &["LUMINA_TELEMETRY_ENABLED"];

pub fn run_migrations(config: &mut Mapping) -> bool {
    let mut changed = false;
    changed |= remove_retired_config_keys(config);
...
pub fn run_read_migrations(config: &mut Mapping) {
    remove_retired_config_keys(config);
    migrate_platform_extensions(config);
}

pub fn remove_retired_config_keys(config: &mut Mapping) -> bool {
    RETIRED_CONFIG_KEYS.iter().fold(false, |changed, key| {
        config
            .shift_remove(serde_yaml::Value::String((*key).to_string()))
            .is_some()
            || changed
    })
}
...
fn cleanup_legacy_provider_keys(config: &mut Mapping) -> bool {
    ...
    let stale_keys: Vec<serde_yaml::Value> = config
        ...
        .filter(|k| {
            k.as_str()
                .map(|s| {
                    s == "LUMINA_PROVIDER" || s == "LUMINA_MODEL" || s.ends_with(configured_suffix)
                })
                .unwrap_or(false)
        })
        .cloned()
...
fn migrate_provider_config(config: &mut Mapping) -> bool {
    ...
    if config.contains_key(&providers_key) {
        ...
        if !config.contains_key(&ap_key) {
            if let Some(legacy) = config
                .get(serde_yaml::Value::String("LUMINA_PROVIDER".to_string()))
                .and_then(|v| v.as_str())
            {
    ...
    let active_model = config
        .get(serde_yaml::Value::String("LUMINA_MODEL".to_string()))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .unwrap_or_default();
    ...
    config.shift_remove(serde_yaml::Value::String("LUMINA_PROVIDER".to_string()));
    config.shift_remove(serde_yaml::Value::String("LUMINA_MODEL".to_string()));
    for name in &discovered_providers {
...
mod tests {
    ...
    #[test]
    fn retired_telemetry_key_is_removed_idempotently() {
        let mut config = Mapping::new();
        config.insert(
            serde_yaml::Value::String("LUMINA_TELEMETRY_ENABLED".to_string()),
            serde_yaml::Value::Bool(true),
        );

        assert!(remove_retired_config_keys(&mut config));
        assert!(!config.contains_key(serde_yaml::Value::String(
            "LUMINA_TELEMETRY_ENABLED".to_string()
        )));
        assert!(!remove_retired_config_keys(&mut config));
    }
    ...
    fn test_migrate_provider_config_basic() {
        let mut config = Mapping::new();
        config.insert(
            serde_yaml::Value::String("LUMINA_PROVIDER".to_string()),
            serde_yaml::Value::String("claude-acp".to_string()),
        );
        config.insert(
            serde_yaml::Value::String("LUMINA_MODEL".to_string()),
        ...
        assert!(!config.contains_key(serde_yaml::Value::String("LUMINA_PROVIDER".to_string())));
        assert!(!config.contains_key(serde_yaml::Value::String("LUMINA_MODEL".to_string())));
        assert!(!config.contains_key(serde_yaml::Value::String(
    ...
    fn test_cleanup_legacy_keys_when_providers_exists() {
        ...
        // active_provider should be backfilled from legacy LUMINA_PROVIDER
        assert_eq!(
            config
                .get(serde_yaml::Value::String("active_provider".to_string()))
                .and_then(|v| v.as_str()),
