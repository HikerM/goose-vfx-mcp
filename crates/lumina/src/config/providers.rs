...
pub fn get_active_provider(config: &Config) -> Option<String> {
    if let Ok(val) = env::var("LUMINA_PROVIDER") {
        return Some(val);
    }
    if let Ok(val) = config.get_param::<String>(ACTIVE_PROVIDER_KEY) {
        return Some(val);
    }
    config.get_param::<String>("LUMINA_PROVIDER").ok()
}

pub fn get_active_model(config: &Config) -> Option<String> {
    if let Ok(val) = env::var("LUMINA_MODEL") {
        return Some(val);
    }
    ...
    config.get_param::<String>("LUMINA_MODEL").ok()
}
...
pub fn clear_active_provider(config: &Config) -> Result<(), ConfigError> {
    for key in [ACTIVE_PROVIDER_KEY, "LUMINA_PROVIDER", "LUMINA_MODEL"] {
...
mod tests {
    ...
    fn test_clear_active_provider_removes_legacy_keys() {
        let config = new_test_config();
        config.set_param("LUMINA_PROVIDER", "anthropic").unwrap();
        config.set_param("LUMINA_MODEL", "claude").unwrap();
    ...
    #[test]
    fn test_provider_configuration_survives_config_reopen() {
        ...
        let reopened = Config::new_with_file_secrets(&config_path, &secrets_path).unwrap();
        assert_eq!(get_active_provider(&reopened), Some("openai".to_string()));
        assert_eq!(get_active_model(&reopened), Some("gpt-4o".to_string()));
        let entry = get_provider_entry(&reopened, "openai").unwrap();
        assert!(entry.enabled);
        assert!(entry.configured);
    }
}
