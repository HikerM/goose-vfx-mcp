...
use clap::Parser;
use lumina_providers::canonical::{
...
fn data_file_path(filename: &str) -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../lumina-provider-types/src/canonical/data")
        .join(filename)
}
...
async fn fetch_models_dev() -> Result<Value> {
    ...
    let response = client
        ...
        .header("User-Agent", "lumina/canonical-builder")
        .send()
        .await
...
async fn check_provider(
    ...
    let provider = match lumina::providers::create_with_named_model(provider_name, Vec::new()).await
    {
        Ok(p) => p,
        Err(e) => {
