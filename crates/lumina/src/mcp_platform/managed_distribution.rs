...
impl FetchedArtifact {
    ...
    #[cfg(windows)]
    fn from_verified_bytes(bytes: Vec<u8>, digest: String) -> Self {
...
impl ProductionDistributionEffectAdapter {
    ...
    async fn verify_materialized_outcome(
        ...
        let bytes = {
            ...
            read_limited(
                &root.join(".lumina-artifact-evidence.json"),
                MAX_METADATA_BYTES,
            )
            .await
            .map_err(|_| verification_error())?
        ...
        let descriptor = serde_json::from_slice::<ActiveRuntimeDescriptor>(
            &self
                .read_version_metadata(
                    &effect.managed_mcp_id,
                    effect.manifest.version.as_str(),
                    ".lumina-runtime-descriptor.json",
                )
                .map_err(|_| verification_error())?,
        )
        .map_err(|_| verification_error())?;
    ...
    async fn install_windows_via_directory_writer(
        ...
        let scratch = tempfile::Builder::new()
            .prefix("lumina-verified-materialization-")
            .tempdir()
    ...
    async fn install_external(
        ...
        tokio::fs::write(
            staging_root.join(".lumina-runtime-descriptor.json"),
            serde_json::to_vec(&descriptor).map_err(|_| repository_error())?,
        )
        .await
        .map_err(|_| repository_error())?;
        tokio::fs::write(
            staging_root.join(".lumina-artifact-evidence.json"),
            serde_json::to_vec(&evidence).map_err(|_| repository_error())?,
        )
        .await
        .map_err(|_| repository_error())?;
        tokio::fs::write(
            staging_root.join(".lumina-external-evidence.json"),
            serde_json::to_vec(&external_evidence).map_err(|_| repository_error())?,
        )
        .await
        .map_err(|_| repository_error())?;
        tokio::fs::write(
            staging_root.join(".lumina-installation-owner.json"),
    ...
    async fn inspect_external(
        ...
        let external_evidence_bytes = self
            .read_version_metadata(
                &effect.managed_mcp_id,
                effect.manifest.version.as_str(),
                ".lumina-external-evidence.json",
            )
            .map_err(|_| verification_error())?;
        #[cfg(not(windows))]
        let external_evidence_bytes = read_limited(
            &root.join(".lumina-external-evidence.json"),
            MAX_METADATA_BYTES,
        )
        .await?;
            ...
            let marker: InstallationMarker = serde_json::from_slice(
                &self
                    .read_version_metadata(
                        &effect.managed_mcp_id,
                        effect.manifest.version.as_str(),
                        ".lumina-installation-owner.json",
                    )
                    .map_err(|_| verification_error())?,
            )
            .map_err(|_| verification_error())?;
    ...
    fn docker_runtime_projection(
        ...
        let mut argv = vec![
            ...
            "256".to_string(),
            "--label".to_string(),
            format!("io.github.hikerm.lumina.managed={}", effect.managed_mcp_id),
        ];
        if !effect
            .manifest
            .permissions
    ...
    async fn install(
        ...
        #[cfg(not(windows))]
        {
            ...
            if tokio::fs::symlink_metadata(&version_root).await.is_err() {
                ...
                .await
                .map_err(|_| repository_error())?;
                let evidence_path = staging_root.join(".lumina-artifact-evidence.json");
...
async fn resolve_verified_entrypoint(
    ...
    match (&manifest.distribution, adapter_id) {
        ...
        (
            ...
            let launcher = root.join(".lumina-wheel-launcher.py");
            let target_path = root.join(".lumina-wheel-target.json");
...
async fn read_runtime_descriptor(root: &Path) -> McpPlatformResult<ActiveRuntimeDescriptor> {
    let bytes = read_limited(
        &root.join(".lumina-runtime-descriptor.json"),
        MAX_METADATA_BYTES,
    )
    .await
    .map_err(|_| verification_error())?;
...
async fn verify_installation_marker(
    ...
    let bytes = read_limited(
        &root.join(".lumina-installation-owner.json"),
        MAX_METADATA_BYTES,
    )
    .await
    .map_err(|_| verification_error())?;
...
async fn compute_materialized_tree_digest(root: &Path) -> McpPlatformResult<String> {
    ...
    tree_hasher.update(b"lumina-managed-tree-v1\0");
...
fn windows_directory_writer_payloads(
    ...
    let descriptor = payloads
        ...
        .find(|payload| {
            payload.relative_segments.len() == 4
                ...
                && payload.relative_segments[3] == ".lumina-runtime-descriptor.json"
        })
        .ok_or_else(verification_error)?;
    ...
    payloads.push(WindowsDirectoryWriterPayload {
        relative_segments: vec![
            ...
            "versions".to_string(),
            version.to_string(),
            ".lumina-committed-version.json".to_string(),
        ],
        bytes: commitment,
    });
    payloads.sort_by(|left, right| left.relative_segments.cmp(&right.relative_segments));
...
impl Installer for ArchiveInstaller {
    async fn materialize(
        ...
        let artifact_bytes = artifact.bytes.clone();
...
fn extract_archive_bytes(
    ...
    let result = match format {
        ArchiveFormat::Zip => extract_zip_reader(
        ...
        ArchiveFormat::TarGz => extract_tar(
            || {
                Ok(Box::new(flate2::read::GzDecoder::new(std::io::Cursor::new(
                    artifact.clone(),
        ...
        ArchiveFormat::TarXz => extract_tar(
            || {
                Ok(Box::new(xz2::read::XzDecoder::new(std::io::Cursor::new(
                    artifact.clone(),
                ))) as Box<dyn std::io::Read>)
            },
            root,
...
fn extract_zip(
    ...
    extract_zip_reader(file, root, strip, limits, cancellation)
}
...
fn extract_zip_reader<R: std::io::Read + std::io::Seek>(
    ...
    let mut archive = zip::ZipArchive::new(reader).map_err(|_| unsafe_archive())?;
    ...
    async fn fetch(
        ...
        #[cfg(windows)]
        {
            ...
            if let Some(length) = response.content_length {
                enforce_size(effect, length)?;
            }
            let initial_capacity = response
                .content_length
                .unwrap_or(0)
                .min(effect.maximum_size_bytes) as usize;
            let mut bytes = Vec::new();
            bytes
                .try_reserve_exact(initial_capacity)
                .map_err(|_| unsafe_fetch())?;
            ...
            while let Some(chunk) = tokio::select! {
                ...
                bytes.try_reserve(chunk.len()).map_err(|_| unsafe_fetch())?;
                hasher.update(&chunk);
            ...
            if effect
                ...
                || hex_digest(hasher.finalize().as_slice()) != effect.expected_sha256
            {
                return Err(verification_error());
            }
            return Ok(FetchedArtifact::from_verified_bytes(
                bytes,
                effect.expected_sha256.clone(),
            ));
        }
        ...
        #[cfg(not(windows))]
        {
            ...
            Ok(FetchedArtifact { path: destination, bytes: None, digest: effect.expected_sha256.clone(), size_bytes: size })
        }.await;
            if result.is_err()
...
mod external_adapter_tests {
    ...
    #[cfg(windows)]
    struct StaticArtifactNetwork {
        bytes: Bytes,
    }

    #[cfg(windows)]
    #[async_trait]
    impl ArtifactNetworkClient for StaticArtifactNetwork {
        async fn open(
            &self,
            ...
            Ok(ArtifactNetworkResponse::new(
                ...
                Box::pin(futures::stream::iter([Ok(self.bytes.clone())])),
            ))
        }
    }
    ...
    #[cfg(windows)]
    #[tokio::test]
    async fn windows_production_fetcher_verifies_memory_backed_artifacts_without_cache_io() {
        ...
        .unwrap();
        zip.write_all(b"verified server").unwrap();
        let bytes = zip.finish().unwrap().into_inner();
        let digest = hex_digest(Sha256::digest(&bytes).as_slice());
        let fetcher = ProductionArtifactFetcher::new_with_network(
            PathBuf::from(r"D:\\untrusted-cache"),
            Arc::new(StaticArtifactNetwork {
        ...
        assert_eq!(artifact.bytes.as_deref(), Some(bytes.as_slice()));
        assert!(artifact.server_path().as_os_str().is_empty());

        let staging = tempfile::tempdir().unwrap().keep().join("payload");
        ArchiveInstaller {
            format: ArchiveFormat::Zip,
            strip_components: 0,
            limits: ArchiveLimits::default(),
        }
        .materialize(&artifact, &staging, &CancellationToken::new())
        .await
        .unwrap();
        assert_eq!(
            std::fs::read(staging.join("server.exe")).unwrap(),
            b"verified server"
        );
        std::fs::remove_dir_all(staging.parent().unwrap()).unwrap();
    }
    ...
    fn write_committed_windows_version(
        ...
        let payloads = vec![
            WindowsDirectoryWriterPayload {
                relative_segments: vec![
                    ...
                    "versions".to_string(),
                    "1.0.0".to_string(),
                    ".lumina-runtime-descriptor.json".to_string(),
                ],
        ...
        std::fs::write(
            version_root.join(".lumina-runtime-descriptor.json"),
            descriptor_bytes,
        )
        .expect("committed runtime descriptor");
        std::fs::write(
            version_root.join(".lumina-committed-version.json"),
    ...
    async fn windows_read_paths_leave_an_initialized_root_unchanged() {
        let temporary = match tempfile::Builder::new()
            .prefix("lumina-managed-read-only-")
        ...
        for artifact in ["cache", "staging", "lumina-staging", "docker-runtime"] {
    ...
    async fn windows_production_install_fails_closed_before_cache_or_legacy_staging_writes() {
        let temporary = match tempfile::Builder::new()
            .prefix("lumina-managed-distribution-")
            .tempdir_in(std::path::Path::new(r"D:\"))
        {
            Ok(temporary) => temporary,
            Err(error) => panic!("create D managed root: {error}"),
    ...
    async fn git_fetch_is_dns_pinned_isolated_and_recursively_audited() {
        ...
        let evidence_path = installed
            .installation_root
            .join(".lumina-external-evidence.json");
