...
use lumina::mcp_platform::manifest::{Architecture, ArchiveFormat, Platform};
use lumina::mcp_platform::repository::AuditPayload;
use lumina::mcp_platform::service::{Clock, IdGenerator};
...
fn fetch_effect(operation_id: &str, bytes: &[u8], maximum_size_bytes: u64) -> ArtifactFetchEffect {
    ArtifactFetchEffect {
        ...
        redirect_policy: lumina::mcp_platform::RedirectPolicy::DenyAll,
...
#[cfg(windows)]
#[tokio::test]
async fn production_fetcher_streams_verified_memory_and_rejects_unsafe_responses() {
    let directory = tempfile::tempdir().unwrap();
    let payload = b"verified managed artifact".to_vec();
    let network = Arc::new(FixtureNetwork::new(vec![
        network_response(
    ...
    let artifact = fetcher
        ...
        .await
        .unwrap();
    assert_eq!(artifact.digest, effect.expected_sha256);
    ...
    let mismatch = fetcher
        ...
        .unwrap_err();
    assert_eq!(mismatch.code(), McpPlatformErrorCode::IntegrityError);
    assert_eq!(network.opens.load(Ordering::SeqCst), 5);
}
...
fn effect_for_manifest(
    manifest: lumina::mcp_platform::Manifest,
    ...
    let source_url = match &manifest.distribution {
        lumina::mcp_platform::Distribution::Npm { artifacts, .. }
        | lumina::mcp_platform::Distribution::PythonWheel { artifacts, .. }
        | lumina::mcp_platform::Distribution::BinaryArchive { artifacts, .. } => {
...
async fn production_npm_and_wheel_validate_metadata_dependencies_and_runtime_descriptors() {
    ...
    match &wheel_outcome.projection {
        ConnectionProjection::ManagedStdio {
            ...
            assert_eq!(args[0], "-I");
            assert!(args[1].ends_with(".lumina-wheel-launcher.py"));
        }
        _ => panic!("expected managed stdio"),
    }
...
async fn production_distribution_runner_rebuilds_rename_before_commit_and_persists_tree_authority()
{
    ...
    assert_eq!(
        started[1].status,
        lumina::mcp_platform::TaskStepStatus::Started
    );
    ...
    assert_eq!(
        ...
        lumina::mcp_platform::TaskStepStatus::Committed
    );
    let tree_digest = match committed_before_activation.evidence.unwrap() {
        lumina::mcp_platform::StepEvidence::ManagedDistributionMaterialized {
            tree_digest, ..
        } => tree_digest,
    ...
    fn outcome(effect: &ManagedInstallEffect) -> ManagedInstallOutcome {
        ManagedInstallOutcome {
            ...
            replaced_quarantine_token: (effect.operation
                == lumina::mcp_platform::TaskOperation::Repair)
                .then(|| {
                    format!(
                        "{}-{}-{}-repair",
                        effect.managed_mcp_id,
...
impl DistributionEffectAdapter for FakeDistribution {
    ...
    async fn install(
        ...
        if effect.operation == lumina::mcp_platform::TaskOperation::Repair {
            let token = format!(
                "{}-{version}-{}-repair",
...
async fn service_runner_install_update_repair_uninstall_is_atomic_and_preserves_enabled_intent() {
    ...
    assert_eq!(
        ...
        lumina::mcp_platform::service::ManagedEligibilityReason::RuntimeUnavailable
    );
    harness
        .service
        .set_default_enabled(
    ...
    assert_eq!(
        ...
        lumina::mcp_platform::service::ManagedEligibilityReason::NoUpdateAvailable
    );
    ...
    assert_eq!(
        ...
        lumina::mcp_platform::service::ManagedEligibilityReason::Incompatible
    );
