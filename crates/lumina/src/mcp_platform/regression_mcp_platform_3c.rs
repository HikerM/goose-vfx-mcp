...
use lumina::config::extensions::ExtensionEntry;
use lumina::mcp_platform::adapters::plan_for_manifest;
use lumina::mcp_platform::manifest::{Architecture, Platform};
...
fn acp_and_production_sources_expose_no_raw_external_execution_entrypoint() {
    let public_module = include_str!("mod.rs");
    let acp = include_str!("../acp/server/mcp_platform.rs");
    let service = include_str!("service/application.rs");
    ...
    fn service_with_policy(
        ...
        Arc::new(McpPlatformService::new_with_external_capabilities(
            ...
            McpPlatformServiceOptions {
                compatibility_target: lumina::mcp_platform::CompatibilityTarget {
...
#[tokio::test]
async fn docker_public_service_lifecycle_persists_evidence_and_reopens_v15() {
    ...
    assert_eq!(
        ...
        Some(
            lumina::mcp_platform::service::ManagedExternalCapabilityStatus::DockerDaemonUnverified
        )
    );
    assert!(matches!(
        detail.supply_chain,
        Some(lumina::mcp_platform::service::ManagedSupplyChainSummary::Docker {
            image_digest,
            ..
    ...
    assert_eq!(
        ...
        Some(
            lumina::mcp_platform::service::ManagedExternalCapabilityStatus::DockerDaemonPolicyDenied
        )
    );
    assert_eq!(
        policy_denied
...
async fn git_dev_mode_off_blocks_mutation_but_allows_owned_uninstall() {
    ...
    assert!(matches!(
        detail.supply_chain,
        Some(lumina::mcp_platform::service::ManagedSupplyChainSummary::GitDev { ref commit, .. })
...
async fn repository_rejects_unknown_newer_schema() {
    ...
    .await
    .unwrap();
    sqlx::query("INSERT INTO schema_version VALUES (16, 0)")
        .execute(&pool)
        .await
        .unwrap();
    pool.close().await;
...
#[tokio::test]
async fn legacy_v1_database_migrates_to_v15_and_projection_mutation_index_releases_recovery() {
    let directory = tempfile::tempdir().unwrap();
    ...
    let active_index: String = sqlx::query_scalar(
        "SELECT sql FROM sqlite_master WHERE type='index' AND name='projection_mutations_active'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert!(active_index.contains("status IN ('started','config_committed','recovery_required')"));
    let projection_columns = sqlx::query("PRAGMA table_info(projection_mutations)")
        .fetch_all(&pool)
        .await
        .unwrap()
        .into_iter()
    ...
    for column in [
        ...
        assert!(projection_columns.iter().any(|name| name == column));
    }

    sqlx::query(
        "INSERT INTO projection_mutations(managed_mcp_id,expected_revision,previous_enabled,desired_enabled,status,created_at_ms,updated_at_ms) VALUES ('legacy-managed',0,0,1,'recovery_required',10,10)",
    )
    .execute(&pool)
    .await
    .unwrap();
    sqlx::query(
        "INSERT INTO projection_mutations(managed_mcp_id,expected_revision,previous_enabled,desired_enabled,status,created_at_ms,updated_at_ms) VALUES ('legacy-managed',1,1,0,'started',11,11)",
    )
    .execute(&pool)
    .await
    .unwrap();
    let conflict = sqlx::query(
        "INSERT INTO projection_mutations(managed_mcp_id,expected_revision,previous_enabled,desired_enabled,status,created_at_ms,updated_at_ms) VALUES ('legacy-managed',2,0,1,'config_committed',12,12)",
    )
    .execute(&pool)
    .await;
    assert!(conflict.is_err());
    sqlx::query(
        "UPDATE projection_mutations SET status='resolved',updated_at_ms=13 WHERE managed_mcp_id='legacy-managed' AND status='recovery_required'",
    )
    .execute(&pool)
    .await
    .unwrap();
    sqlx::query(
        "INSERT INTO projection_mutations(managed_mcp_id,expected_revision,previous_enabled,desired_enabled,status,created_at_ms,updated_at_ms) VALUES ('legacy-managed',2,0,1,'started',14,14)",
    )
    .execute(&pool)
    .await
    .unwrap();
