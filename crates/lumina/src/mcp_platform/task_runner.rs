...
impl TaskRunner {
    ...
    pub async fn tick(&self) -> McpPlatformResult<bool> {
        ...
        if let Err(error) = result {
            ...
            eprintln!(
                "MCP integration task execution failed before settlement: task_id={}, error={error:?}",
                task.task_id
            );
    ...
    async fn authorize_effect(&self, task_id: &str) -> McpPlatformResult<ExecutionAuthorization> {
        const MAX_CHECKPOINT_RETRIES: usize = 4;
        for attempt in 0..MAX_CHECKPOINT_RETRIES {
                ...
                .await
            {
                ...
                Err(error)
                    if error.code() == McpPlatformErrorCode::RevisionConflict
                        && attempt + 1 < MAX_CHECKPOINT_RETRIES =>
                {
        ...
        unreachable!("authorization retry loop always returns")
    }
    ...
    async fn execute_register(
        ...
        if verify_status != TaskStepStatus::Committed {
            let authorization = self
                .authorize_effect(&task.task_id)
                .await
                .map_err(|error| integration_stage_error("register.verify.authorize", error))?;
            let snapshot = self
                .ports
                .projection_sink
                .get(&stable_ids.link_key)
                .await
                .map_err(|error| integration_stage_error("register.verify.config_read", error))?
                .ok_or_else(integrity_error)?;
            ...
            .await
            .map_err(|error| integration_stage_error("register.verify.commit", error))?;
        }
        self.transition(&task.task_id, TaskStatus::Succeeded, 100)
            .await?;
        Ok(())
    ...
    async fn start_step(
        ...
        let authorization = self
            .authorize_effect(&task.task_id)
            .await
            .map_err(|error| integration_stage_error("start_step.authorize", error))?;
        ...
        let step = self
            ...
            .await
            .map_err(|error| integration_stage_error("start_step.insert", error))?;
        if step.status == TaskStepStatus::NotStarted {
            let authorization = self
                .authorize_effect(&task.task_id)
                .await
                .map_err(|error| integration_stage_error("start_step.reauthorize", error))?;
            return self
                .repository
                ...
                .await
                .map_err(|error| integration_stage_error("start_step.transition", error))
    ...
    async fn commit_step(
        ...
        self.repository
            ...
            .await
            .map_err(|error| integration_stage_error("commit_step.renew_lease", error))?;
        let authorization = self
            .authorize_effect(task_id)
            .await
            .map_err(|error| integration_stage_error("commit_step.authorize", error))?;
        ...
        self.repository
            ...
            .await
            .map_err(|error| integration_stage_error("commit_step.transition", error))?;
        Ok(())
    }
...
fn integration_stage_error(stage: &'static str, error: McpPlatformError) -> McpPlatformError {
    #[cfg(feature = "integration-test-support")]
    eprintln!("MCP integration stage failed: stage={stage}, error={error:?}");
