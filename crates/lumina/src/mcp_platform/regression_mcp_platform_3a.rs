...
use lumina::agents::{ExtensionConfig, LuminaPlatform};
...
use lumina::mcp_platform::credential_authority::EnrollmentWriterOwner;
use lumina::mcp_platform::credential_enrollment::{EnrollmentFieldSubmission, EnrollmentSecret};
use lumina::mcp_platform::manifest::{Architecture, Auth, Platform};
...
#[async_trait]
impl lumina::mcp_platform::RemoteHttpNetworkPolicy for VerifiedTestRemotePolicy {
    fn validate_endpoint(&self, _endpoint: &str) -> lumina::mcp_platform::McpPlatformResult<()> {
        Ok(())
    }
    ...
    ) -> lumina::mcp_platform::McpPlatformResult<()> {
    ...
    async fn new_with_enrollment_writer(
        ...
        let service = McpPlatformService::new_with_lifecycle_ports(
            ...
            McpPlatformServiceOptions {
                compatibility_target: lumina::mcp_platform::CompatibilityTarget { platform, arch },
...
async fn started_projection_recovery_requires_manual_resolution_without_replaying_sink_write() {
    ...
    let healthy = harness
        ...
        .await
        .unwrap();
    let authority = lumina::mcp_platform::projection_runtime::bootstrap_debug_repository_authority(
        &harness.repository,
        &harness.ports,
    );
...
fn production_runner_health_and_acp_boundary_have_no_shell_or_repository_escape_hatch() {
    ...
    let wire = std::fs::read_to_string(
        root.join("../lumina-sdk-types/src/custom_requests/mcp_platform.rs"),
    )
    .unwrap();
