...
const STAGING_DIRECTORY: &str = "lumina-staging";
const STAGING_SESSION_MARKER: &str = "lumina-staging-session";
const PAYLOAD_FILE: &str = "payload";
const COMMITTED_VERSION_EVIDENCE: &str = ".lumina-committed-version.json";
const MANAGED_ANCHOR_DOMAIN: &str = "managed-installations-root-v1";
const MANAGED_ANCHOR_ROOT_PREFIX: &str = "lumina-managed-installations-anchor-v1:";
const MANAGED_LINEAGE_DOMAIN: &str = "managed-installations-lineage-v1";
const MANAGED_LINEAGE_ROOT_PREFIX: &str = "lumina-managed-installations-lineage-v1:";
...
impl ManagedRootCapability {
    ...
    pub(super) fn recover_committed_cleanup_pending(&self) -> McpPlatformResult<()> {
        ...
        let descriptor = read_relative_file(&target, ".lumina-runtime-descriptor.json")?
            .ok_or_else(integrity_error)?;
        match pending.phase {
            CleanupTransactionPhase::Prepared => {
                return self.abort_prepared_cleanup(
    ...
    pub(super) fn activate_verified_managed_installation(
        ...
        let result = (|| {
            ...
            let versions = open_relative_directory_for_mutation(&managed, "versions")?;
            let target = open_relative_directory_for_mutation(&versions, version)?;
            if read_relative_file(&target, ".lumina-runtime-descriptor.json")?.as_deref()
                != Some(descriptor)
            {
                return Err(integrity_error());
            }
...
mod tests {
    ...
    fn leaf_validation_rejects_paths_ads_and_windows_aliases() {
        ...
        assert!(validated_leaf_name("lumina-stage_1").is_ok());
    ...
    fn managed_activation_replaces_only_a_sealed_existing_descriptor_and_rolls_back() {
        ...
        let mut tree = Sha256::new();
        tree.update(b".lumina-runtime-descriptor.json");
        tree.update([0]);
            ...
            .materialize_verified_directory(&[
                DirectoryPayload {
                    relative_segments: vec![
                        ...
                        "versions".to_owned(),
                        "1.0.0".to_owned(),
                        ".lumina-runtime-descriptor.json".to_owned(),
                    ],
        ...
        std::fs::write(
            temporary
                .path()
                .join("platform.installations/managed_000001/versions/1.0.0/.lumina-runtime-descriptor.json"),
            tampered_descriptor,
        )
        .unwrap();
