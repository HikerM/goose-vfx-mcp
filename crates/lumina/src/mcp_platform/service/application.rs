...
impl McpPlatformService {
    ...
    async fn governed_source_refresh_inner(
        ...
        if source.import_kind != SourceImportKind::VerifiedSourceCatalog {
            return Err(invalid_request());
        }
        ...
        if parsed.descriptor.source_id != *verified_source_id
    ...
    #[cfg(feature = "integration-test-support")]
    pub fn new_with_remote_http_network_policy_and_config(
        repository: Arc<SqliteMcpPlatformRepository>,
        clock: Arc<dyn Clock>,
        ids: Arc<dyn IdGenerator>,
        options: McpPlatformServiceOptions,
        ...
        Self::new_internal(
            ...
            ids,
            options,
            lifecycle_ports_with_config(remote_http.clone(), config),
            false,
            remote_http,
        )
    }
...
) -> McpPlatformResult<CatalogSummary> {
    let manifest = record.verified.manifest();
    let source_id = governed_source_id(&record.source_metadata.source_ref)?;
    Ok(CatalogSummary {
        source_id,
...
fn manual_http_manifest(
    ...
    let raw = serde_json::json!({
        ...
        "description":"A manually registered remote MCP connection validated by lumina Core.",
...
#[cfg(feature = "integration-test-support")]
fn lifecycle_ports_with_config(
    remote_http: Arc<dyn RemoteHttpNetworkPolicy>,
    config: Arc<crate::config::Config>,
) -> LifecyclePorts {
    LifecyclePorts {
