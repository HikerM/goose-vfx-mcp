...
fn legacy_private_reference(kind_label: &str, raw: &str) -> String {
    ...
    hasher.update(b"lumina.mcp-platform.intake.private-reference.v1");
    hasher.update([0]);
...
fn legacy_intake_hash_hex(label: &str, parts: &[&[u8]]) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"lumina.mcp-platform.intake.v1");
    hasher.update([0]);
...
fn legacy_descriptor_digest(
    descriptor: &lumina::mcp_platform::intake::IntakeConfigurationDescriptor,
