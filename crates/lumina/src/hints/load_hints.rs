...
pub const LUMINA_HINTS_FILENAME: &str = ".luminahints";
...
pub fn get_context_filenames() -> Vec<String> {
    ...
    Config::global()
        ...
        .unwrap_or_else(|_| {
            vec![
                LUMINA_HINTS_FILENAME.to_string(),
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global lumina hints.\n");
...
mod tests {
    ...
    #[test]
    fn test_luminahints_when_present() {
        let dir = TempDir::new().unwrap();

        fs::write(dir.path().join(LUMINA_HINTS_FILENAME), "Test hint content").unwrap();
        let gitignore = create_dummy_gitignore();
        let hints = load_hint_files(dir.path(), &[LUMINA_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn test_global_agents_md_in_agents_home() {
        ...
        std::env::set_var("LUMINA_PATH_ROOT", root.path());
        ...
        std::env::remove_var("LUMINA_PATH_ROOT");
    ...
    #[test]
    fn test_luminahints_when_missing() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_luminahints_multiple_filenames() {
        let dir = TempDir::new().unwrap();
        ...
        .unwrap();
        fs::write(
            dir.path().join(LUMINA_HINTS_FILENAME),
            "Custom hints file content from .luminahints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), LUMINA_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .luminahints"));
    }

    #[test]
    fn test_luminahints_configurable_filename() {
        let dir = TempDir::new().unwrap();
        ...
        assert!(!hints.contains(".luminahints")); // Make sure it's not loading the default
    }

    #[test]
    fn test_nested_luminahints_with_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();

        fs::create_dir(project_root.join(".git")).unwrap();
        fs::write(
            project_root.join(LUMINA_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(subdir.join(LUMINA_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(LUMINA_HINTS_FILENAME),
    ...
    #[test]
    fn test_nested_luminahints_without_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let base_dir = temp_dir.path();

        fs::write(base_dir.join(LUMINA_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    #[test]
    fn test_nested_luminahints_mixed_filenames() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();
        ...
        fs::create_dir(&subdir).unwrap();
        fs::write(
            subdir.join(LUMINA_HINTS_FILENAME),
            "Subdir .luminahints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .luminahints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(LUMINA_HINTS_FILENAME), hints_content).unwrap();
...
Root level instructions"#;
        fs::write(project_root.join(LUMINA_HINTS_FILENAME), root_hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(LUMINA_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(LUMINA_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(LUMINA_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[LUMINA_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn tracker_deduplicates_directories() {
        ...
        let subdir = project_root.join("nested");
        fs::create_dir_all(&subdir).unwrap();
        fs::write(subdir.join(LUMINA_HINTS_FILENAME), "nested hints").unwrap();
