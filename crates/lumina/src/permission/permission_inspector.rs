...
use crate::config::{LuminaMode, PermissionManager};
    ...
    async fn inspect(
        ...
        for request in tool_requests {
            if let Ok(tool_call) = &request.tool_call {
                ...
                let action = match lumina_mode {
                    LuminaMode::Chat => continue,
                    LuminaMode::Auto => InspectionAction::Allow,
                    LuminaMode::Approve | LuminaMode::SmartApprove => {
                        ...
                        // 2. Check if it's a smart-approved tool (annotation or cached LLM decision)
                            ...
                            || (lumina_mode == LuminaMode::SmartApprove
                        ...
                        } else if lumina_mode == LuminaMode::SmartApprove
                            && permission_manager
                ...
                let reason = match &action {
                    InspectionAction::Allow => {
                        if lumina_mode == LuminaMode::Auto {
...
mod tests {
    ...
    #[test_case(LuminaMode::Auto, false, None, InspectionAction::Allow; "auto_allows")]
    #[test_case(LuminaMode::SmartApprove, true, None, InspectionAction::Allow; "smart_approve_annotation_allows")]
    #[test_case(LuminaMode::SmartApprove, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::Allow; "smart_approve_cached_allow")]
    #[test_case(LuminaMode::SmartApprove, false, Some(PermissionLevel::AskBefore), InspectionAction::RequireApproval(None); "smart_approve_cached_ask")]
    #[test_case(LuminaMode::SmartApprove, false, None, InspectionAction::RequireApproval(None); "smart_approve_unknown_defers")]
    #[test_case(LuminaMode::Approve, false, None, InspectionAction::RequireApproval(None); "approve_requires_approval")]
    #[test_case(LuminaMode::Approve, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::RequireApproval(None); "approve_ignores_cache")]
    #[tokio::test]
    async fn test_inspect_action(
        ...
        let results = inspector
            .inspect(lumina_test_support::TEST_SESSION_ID, &[req], &[], mode)
            .await
            .unwrap();
