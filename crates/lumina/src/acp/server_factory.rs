use crate::acp::server::{AcpProviderFactory, LuminaAcpAgent, LuminaAcpAgentOptions};
...
impl AcpServer {
    ...
    pub async fn create_agent(&self) -> Result<Arc<LuminaAcpAgent>> {
        let config = crate::config::Config::global();
        let disable_session_naming = config.get_lumina_disable_session_naming().unwrap_or(false);
        ...
        let agent = LuminaAcpAgent::new_with_trusted_mcp_platform_service_cell(
            LuminaAcpAgentOptions {
                ...
                lumina_platform: self.config.lumina_platform.clone(),
