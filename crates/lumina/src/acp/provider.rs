...
use lumina_providers::conversation::token_usage::{ProviderUsage, Usage};
...
use crate::config::{ExtensionConfig, LuminaMode};
...
use lumina_providers::errors::ProviderError;
...
pub struct AcpProviderConfig {
    ...
    pub mode_mapping: HashMap<LuminaMode, Vec<String>>,
...
pub struct AcpProvider {
    name: String,
    lumina_mode: Arc<Mutex<LuminaMode>>,
    mode_mapping: HashMap<LuminaMode, Vec<String>>,
...
impl AcpProvider {
    ...
    async fn start(
        ...
        let lumina_mode_shared = Arc::new(Mutex::new(lumina_mode));
        ...
        Ok(Self {
            name,
            lumina_mode: lumina_mode_shared,
            mode_mapping,
            session,
    ...
    pub(crate) async fn send_set_mode(&self, _lumina_id: &str, mode_id: String) -> Result<()> {
...
impl Provider for AcpProvider {
    ...
    async fn update_mode(&self, session_id: &str, mode: LuminaMode) -> Result<(), ProviderError> {
        ...
        if let Ok(mut guard) = self.lumina_mode.lock() {
            *guard = mode;
        }
        Ok(())
    }
    ...
    async fn stream(
        ...
        let lumina_mode = *self
            .lumina_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("lumina_mode lock poisoned".into()))?;

        let reject_all_tools = lumina_mode == LuminaMode::Chat;
                ...
                match update {
                    ...
                    AcpUpdate::ToolCallStart { id, name, kind, raw_input } => {
                        ...
                        if reject_all_tools {
                            ...
                            // call. lumina.acp.kind preserves ACP's stable categorization for
                    ...
                    AcpUpdate::PermissionRequest { request, response_tx } => {
                        text_run = None;
                        thought_run = None;
                        if let Some(decision) = permission_decision_from_mode(lumina_mode) {
    ...
    async fn run(
        ...
        Client
            ...
            .on_receive_notification(
                {
                    ...
                    let lumina_mode = lumina_mode.clone();
                        ...
                        match &notification.update {
                            SessionUpdate::CurrentModeUpdate(update) => {
                                if let Some(mode) = resolve_mode(
                                    ...
                                    if let Ok(mut guard) = lumina_mode.lock() {
                                        *guard = mode;
                                    }
                                }
                            }
                            ...
                            .and_then(|g| g.as_ref())
                        {
                            match notification.update {
                                ...
                                SessionUpdate::ToolCall(tool_call) => {
                                    ...
                                    // lumina.external_dispatch marker keeps `name` off the
            ...
            .connect_with(transport, async move |cx: ConnectionTo<Agent>| {
                handle_requests(config, lumina_mode, cx, rx, prompt_response_tx, init_tx).await
            })
            .await?;
                ...
                let result = match session {
                    Ok(session) => {
                        ...
                        apply_session_config_options(&config, &cx, session.session_id.clone())
                            .await?;
                        apply_session_mode(&config, &lumina_mode, &cx, session).await
                    }
...
async fn apply_session_mode(
    ...
    let current_mode = lumina_mode.lock().ok().map(|mode| *mode);
...
fn initial_mode_candidates(
    config: &AcpProviderConfig,
    current_mode: Option<LuminaMode>,
) -> Vec<String> {
    current_mode
...
fn build_handoff_context_memo(prior_messages: &[Message]) -> Option<String> {
    ...
    Some(format!(
        "Conversation context from lumina before this ACP provider session was created:\n\n\
{handoff_context}\n\n\
...
/// Convert ACP `ToolCallContent` blocks into the rmcp `Content` shape lumina's
...
fn reverse_mode_mapping(
    mode_mapping: &HashMap<LuminaMode, Vec<String>>,
) -> HashMap<String, Vec<LuminaMode>> {
    let mut reverse: HashMap<String, Vec<LuminaMode>> = HashMap::new();
...
fn resolve_mode(
    reverse_modes: &HashMap<String, Vec<LuminaMode>>,
    mode_id: &str,
    current: &Arc<Mutex<LuminaMode>>,
) -> Option<LuminaMode> {
...
fn permission_decision_from_mode(lumina_mode: LuminaMode) -> Option<PermissionDecision> {
    match lumina_mode {
        LuminaMode::Auto => Some(PermissionDecision::AllowOnce),
        LuminaMode::Chat => Some(PermissionDecision::RejectOnce),
        LuminaMode::Approve | LuminaMode::SmartApprove => None,
    }
}
...
mod tests {
    ...
    fn test_provider_with_tx(
        ...
        (
            AcpProvider {
                ...
                lumina_mode: Arc::new(Mutex::new(LuminaMode::Auto)),
    ...
    async fn get_context_limit_surfaces_captured_context_size() {
        ...
        assert_eq!(
            ...
            lumina_providers::model::DEFAULT_CONTEXT_LIMIT
        );
    ...
    #[test_case(LuminaMode::Auto)]
    #[test_case(LuminaMode::Approve)]
    #[test_case(LuminaMode::SmartApprove)]
    #[test_case(LuminaMode::Chat)]
    fn initial_mode_candidates_empty_when_mode_negotiation_disabled(mode: LuminaMode) {
    ...
    fn initial_mode_candidates_prefer_mapping_then_fallback() {
        let mapping = HashMap::from([(LuminaMode::Auto, vec!["bypassPermissions".to_string()])]);
        let config = test_acp_config(mapping, Some("default".to_string()));

        assert_eq!(
            initial_mode_candidates(&config, Some(LuminaMode::Auto)),
            vec!["bypassPermissions".to_string()]
        );
        assert_eq!(
            initial_mode_candidates(&config, Some(LuminaMode::Chat)),
    ...
    async fn update_mode_without_mapping_skips_acp_request_but_tracks_mode() {
        ...
        provider
            .update_mode("session", LuminaMode::Chat)
            .await
            .unwrap();

        assert!(rx.try_recv().is_err());
        assert_eq!(*provider.lumina_mode.lock().unwrap(), LuminaMode::Chat);
    }
    ...
    async fn update_mode_with_mapping_sends_set_mode() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(LuminaMode::Chat, vec!["plan".to_string()])]);
    ...
    async fn update_mode_sends_candidate_offered_by_agent() {
        ...
        let handle = tokio::spawn(async move {
            provider
                .update_mode("session", LuminaMode::Auto)
                .await
                .unwrap();
            provider
        });
        ...
        let provider = handle.await.unwrap();
        assert_eq!(*provider.lumina_mode.lock().unwrap(), LuminaMode::Auto);
    }
    ...
    async fn update_mode_errors_when_no_candidate_offered() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(LuminaMode::Chat, vec!["read-only".to_string()])]);
        provider.session.response = NewSessionResponse::new("test-session")
            .modes(mode_state("agent", &["agent", "agent-full-access"]));

        let result = provider.update_mode("session", LuminaMode::Chat).await;
    ...
    #[test_case(LuminaMode::Auto => Some(PermissionDecision::AllowOnce) ; "auto allows")]
    #[test_case(LuminaMode::Chat => Some(PermissionDecision::RejectOnce) ; "chat rejects")]
    #[test_case(LuminaMode::Approve => None ; "approve defers")]
    #[test_case(LuminaMode::SmartApprove => None ; "smart_approve defers")]
    fn test_permission_decision_from_mode(mode: LuminaMode) -> Option<PermissionDecision> {
        permission_decision_from_mode(mode)
    }

    #[test_case(
        HashMap::from([
            (LuminaMode::Auto, vec!["yolo".to_string()]),
            (LuminaMode::Approve, vec!["default".to_string()]),
            (LuminaMode::SmartApprove, vec!["auto_edit".to_string()]),
            (LuminaMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("yolo".to_string(), vec![LuminaMode::Auto]),
            ("default".to_string(), vec![LuminaMode::Approve]),
            ("auto_edit".to_string(), vec![LuminaMode::SmartApprove]),
            ("plan".to_string(), vec![LuminaMode::Chat]),
        ])
        ; "gemini provider mapping"
    )]
    #[test_case(
        HashMap::from([
            (LuminaMode::Auto, vec!["bypassPermissions".to_string()]),
            (LuminaMode::Approve, vec!["default".to_string()]),
            (LuminaMode::SmartApprove, vec!["acceptEdits".to_string()]),
            (LuminaMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("bypassPermissions".to_string(), vec![LuminaMode::Auto]),
            ("default".to_string(), vec![LuminaMode::Approve]),
            ("acceptEdits".to_string(), vec![LuminaMode::SmartApprove]),
            ("plan".to_string(), vec![LuminaMode::Chat]),
        ])
    ...
    #[test_case(
        HashMap::from([
            (LuminaMode::Auto, vec!["full-access".to_string(), "agent-full-access".to_string()]),
            (LuminaMode::Approve, vec!["read-only".to_string()]),
            (LuminaMode::SmartApprove, vec!["auto".to_string(), "agent".to_string()]),
            (LuminaMode::Chat, vec!["read-only".to_string()]),
        ]),
        HashMap::from([
            ("full-access".to_string(), vec![LuminaMode::Auto]),
            ("agent-full-access".to_string(), vec![LuminaMode::Auto]),
            ("read-only".to_string(), vec![LuminaMode::Approve, LuminaMode::Chat]),
            ("auto".to_string(), vec![LuminaMode::SmartApprove]),
            ("agent".to_string(), vec![LuminaMode::SmartApprove]),
        ])
        ; "codex candidates for both bridge generations"
    )]
    fn test_reverse_mode_mapping(
        forward: HashMap<LuminaMode, Vec<String>>,
        expected: HashMap<String, Vec<LuminaMode>>,
    ) {
    ...
    fn duplicate_read_only_reverse_modes() -> HashMap<String, Vec<LuminaMode>> {
        HashMap::from([
            ("full-access".to_string(), vec![LuminaMode::Auto]),
            (
                "read-only".to_string(),
                vec![LuminaMode::Approve, LuminaMode::Chat],
            ),
            ("auto".to_string(), vec![LuminaMode::SmartApprove]),
        ])
    }
    ...
    #[test_case(
        "full-access", LuminaMode::Auto, Some(LuminaMode::Auto)
        ; "unique mapping returns the only candidate"
    )]
    #[test_case(
        "read-only", LuminaMode::Approve, Some(LuminaMode::Approve)
        ; "duplicate prefers current when current is Approve"
    )]
    #[test_case(
        "read-only", LuminaMode::Chat, Some(LuminaMode::Chat)
        ; "duplicate prefers current when current is Chat"
    )]
    #[test_case(
        "read-only", LuminaMode::Auto, Some(LuminaMode::Approve)
        ; "duplicate falls back to first when current not in candidates"
    )]
    #[test_case(
        "unknown-id", LuminaMode::Auto, None
        ; "unknown mode id returns None"
    )]
    fn test_resolve_mode(mode_id: &str, current: LuminaMode, expected: Option<LuminaMode>) {
        ...
        if mode_id == "read-only" && expected == Some(LuminaMode::Approve) {
            assert!(result == Some(LuminaMode::Approve) || result == Some(LuminaMode::Chat));
        } else {
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["lumina.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "lumina.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}
