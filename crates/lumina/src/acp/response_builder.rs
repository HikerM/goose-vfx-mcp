...
///
/// Replaces the removed `SessionModelState` ACP schema type; lumina now surfaces
...
pub(super) fn build_mode_state(
    current_mode: LuminaMode,
) -> Result<SessionModeState, agent_client_protocol::Error> {
    let mut available = Vec::with_capacity(LuminaMode::VARIANTS.len());
    for &name in LuminaMode::VARIANTS {
        let lumina_mode: LuminaMode = name.parse().map_err(|_| {
            agent_client_protocol::Error::internal_error() // impossible but satisfy linters
                .data(format!("Failed to parse LuminaMode variant: {}", name))
        })?;
        let mut mode = SessionMode::new(SessionModeId::new(name), name);
        mode.description = lumina_mode.get_message().map(Into::into);
        available.push(mode);
    }
...
pub(super) async fn build_session_setup_config(
    ...
    let mode_state = build_mode_state(session.lumina_mode)?;
...
fn current_thinking_effort_value(model_config: &ModelConfig) -> String {
    if model_config.is_reasoning_model() {
        model_config
            .thinking_effort()
            .or_else(|| Config::global().get_lumina_thinking_effort())
...
mod tests {
    ...
    #[test_case(
        build_mode_state(LuminaMode::Auto).unwrap(),
        "openai",
        vec![
    ...
    #[test_case(
        build_mode_state(LuminaMode::Approve).unwrap(),
        "openai",
    ...
    fn test_build_config_options_uses_current_thinking_effort() {
        let mode_state = build_mode_state(LuminaMode::Auto).unwrap();
