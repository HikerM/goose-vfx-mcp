...
impl LuminaAcpAgent {
    pub(super) async fn on_add_session_extension(
        ...
        let config = lumina_extension_to_config_without_secrets(req.extension)?;
    ...
    pub(super) async fn on_get_config_extensions(
        ...
        let extensions = extensions
            .into_iter()
            .map(config_entry_to_lumina_entry)
    ...
    pub(super) async fn on_get_available_extensions(
        ...
        let extensions = crate::config::get_available_extensions()
            .into_iter()
            .map(|config| config_to_lumina_extension(&config))
            .collect::<Result<Vec<_>, _>>()?
            .into_iter()
            .flatten()
            .collect::<Vec<_>>();
    ...
    pub(super) async fn on_add_config_extension(
        ...
        let conversion = lumina_extension_to_config(req.extension)?;
...
fn config_to_lumina_extension(
    config: &ExtensionConfig,
) -> Result<Option<LuminaExtension>, agent_client_protocol::Error> {
...
fn lumina_extension_to_config(
    extension: LuminaExtension,
...
fn lumina_extension_to_config_without_secrets(
    extension: LuminaExtension,
) -> Result<ExtensionConfig, agent_client_protocol::Error> {
    let conversion = lumina_extension_to_config(extension)?;
...
pub(super) fn lumina_extensions_to_configs(
    extensions: Vec<LuminaExtension>,
) -> Result<Vec<ExtensionConfig>, agent_client_protocol::Error> {
    extensions
        .into_iter()
        .map(lumina_extension_to_config_without_secrets)
        .collect()
}

fn config_entry_to_lumina_entry(
    entry: ExtensionEntry,
) -> Result<Option<LuminaExtensionEntry>, agent_client_protocol::Error> {
    let config_key = entry.config.key();
    let Some(extension) = config_to_lumina_extension(&entry.config)? else {
        return Ok(None);
    };
    Ok(Some(LuminaExtensionEntry {
        extension,
...
mod tests {
    ...
    #[test]
    fn builtin_config_converts_to_lumina_builtin_extension() {
        ...
        let extension = config_to_lumina_extension(&config)
            .expect("conversion should succeed")
            .expect("builtin should be supported");

        let LuminaExtension::Builtin {
            name,
            description,
            display_name,
            timeout,
    ...
    #[test]
    fn platform_config_converts_to_lumina_platform_extension() {
    ...
    #[test]
    fn stdio_config_converts_to_lumina_mcp_extension_without_literal_envs() {
    ...
    #[test]
    fn streamable_http_config_converts_to_lumina_mcp_extension_without_literal_envs() {
    ...
    fn inline_python_config_is_skipped() {
        ...
        let extension = config_to_lumina_extension(&config).expect("conversion should succeed");
    ...
    #[test]
    fn lumina_mcp_stdio_extension_converts_to_config_without_literal_envs() {
        let extension = LuminaExtension::Mcp {
        ...
        let conversion = lumina_extension_to_config(extension).expect("conversion should succeed");
    ...
    #[test]
    fn lumina_mcp_stdio_extension_extracts_literal_envs_for_config_add() {
        let extension = LuminaExtension::Mcp {
    ...
    #[test]
    fn lumina_mcp_streamable_http_extension_converts_to_config_without_literal_envs() {
        let extension = LuminaExtension::Mcp {
    ...
    #[test]
    fn lumina_builtin_extension_converts_to_config() {
        let builtin = LuminaExtension::Builtin {
            name: "developer".to_string(),
        ...
        let conversion = lumina_extension_to_config(builtin).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn lumina_platform_extension_converts_to_config() {
        let platform = LuminaExtension::Platform {
            name: "todo".to_string(),
        ...
        let conversion = lumina_extension_to_config(platform).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn lumina_mcp_sse_extension_is_rejected_for_config_add() {
        let extension = LuminaExtension::Mcp {
        ...
        assert!(lumina_extension_to_config(extension).is_err());
    }
}
