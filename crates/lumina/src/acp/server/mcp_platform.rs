        ...
        match command.action {
            HostRuntimeAction::Stop => {
                ...
                if failed {
                    return Err(crate::mcp_platform::McpPlatformError::new(
                        ...
                        "Lumina could not close every owned managed MCP connection; stopped sessions can be restarted and the remaining connection can be retried",
                    ));
                }
                Ok(stopped)
            }
            HostRuntimeAction::Start => {
                ...
                if target_sessions.is_empty() {
                    return Err(crate::mcp_platform::McpPlatformError::new(
                        crate::mcp_platform::McpPlatformErrorCode::RuntimeControlUnavailable,
                        "No Lumina-owned stopped MCP runtime is available; reopen the session that previously used this MCP",
                    ));
                }
                ...
                if failed {
                    return Err(crate::mcp_platform::McpPlatformError::new(
                        crate::mcp_platform::McpPlatformErrorCode::RuntimeControlUnavailable,
                        "Lumina could not restart every owned managed MCP connection; retry the remaining stopped sessions",
                    ));
                }
                if started {
                    Ok(true)
        ...
        let result = match service {
            Ok(_) if expected_binding.as_deref() != Some(req.runtime_binding.as_str()) => {
                Err(McpPlatformError::new(
                    ...
                    "managed MCP runtime binding does not match this authenticated Lumina connection",
                ))
            }
            Ok(service) => service
...
impl LuminaAcpAgent {
    ...
    async fn test_profile_model(
        ...
        let model = lumina_providers::model::ModelConfig::new(model_id);
...
fn runtime_control_binding(managed_mcp_id: &str, revision: i64) -> Option<String> {
    ...
    hasher.update(b"lumina-managed-runtime-control-v1\0");
...
mod tests {
    ...
    async fn agent(data_dir: &Path, config_dir: &Path) -> LuminaAcpAgent {
        ...
        LuminaAcpAgent::new_with_trusted_mcp_platform_service_cell(
            LuminaAcpAgentOptions {
                provider_factory,
                builtins: Vec::new(),
    ...
    async fn managed_item_count(agent: &LuminaAcpAgent) -> usize {
    ...
    async fn catalog_item_count(agent: &LuminaAcpAgent) -> usize {
        let (context, service) = agent.mcp_platform_context_and_service_read_only().await;
    ...
    async fn catalog_page_via_transport(agent: &LuminaAcpAgent) -> McpCatalogPage {
        let listed = agent
    ...
    async fn source_policy_via_transport(agent: &LuminaAcpAgent) -> McpSourcesPolicyState {
        let listed = agent
            .test_dispatch_transport_custom_request_without_authority(
    ...
    #[tokio::test]
    async fn mcp_center_empty_state_routes_are_available_without_write_authority() {
        ...
        let manual_sources = agent
            .test_dispatch_transport_custom_request_without_authority(
                MCP_MANUAL_STDIO_SOURCES_LIST_METHOD,
                serde_json::json!({}),
            )
            .await
            .expect("manual stdio source discovery should succeed");
        let manual_sources: McpManualStdioSourcesListResponse =
            serde_json::from_value(manual_sources).unwrap();
        assert!(matches!(
            manual_sources.outcome,
            McpPlatformOutcome::Success { .. }
        ));
    }

    async fn provision_source_via_transport(agent: &LuminaAcpAgent, source_dir: &Path) -> String {
    ...
    async fn governed_source_refresh_transport_updates_catalog_without_installing_any_mcp() {
        ...
        let value = match refreshed.outcome {
            McpPlatformOutcome::Success { value } => value,
            McpPlatformOutcome::Error { error } => {
                panic!("source refresh should succeed: {error:?}")
            }
        };
    ...
    fn platform_error_wire_hides_sensitive_low_level_message_parts() {
        ...
        let envelope = error_to_wire(
            ...
            McpPlatformError::new(
                ...
                "path=/tmp/secret file not found -- token=abc123 hash=sha256:deadbeefcafebabe cmd=lumina run",
            ),
        );
        let serialized = serde_json::to_value(envelope).expect("error envelope serializes");
    ...
    fn runtime_control_error_wire_hides_sensitive_low_level_message_parts() {
        let sensitive_payloads = [
            r"C:\\Users\\test\\lumina-runtime.exe",
        ...
        let envelope = error_to_wire(
            ...
            McpPlatformError::new(
                ...
                "runtime command failed: C:\\Users\\test\\lumina-runtime.exe --token=secret-token hash=sha256:0123456789abcdef",
            ),
        );
        let serialized = serde_json::to_value(envelope).expect("error envelope serializes");
        let serialized = serialized.to_string();
