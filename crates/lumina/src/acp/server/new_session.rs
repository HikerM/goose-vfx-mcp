use crate::acp::custom_requests::LuminaExtension;
...
impl LuminaAcpAgent {
    pub(super) async fn handle_new_session(
        ...
        let work_item_id = meta_string(args.meta.as_ref(), "workItemId")?;
        let session_type = match meta_string(args.meta.as_ref(), "client")? {
            Some(_) => SessionType::User,
            None => SessionType::Acp,
        };
        let current_mode: LuminaMode = config.get_lumina_mode().unwrap_or_default();
    ...
    async fn configure_new_session(
        ...
        let lumina_extensions = meta_lumina_extensions(args.meta.as_ref())?;
    ...
    async fn prepare_profile_application(
        ...
        if token.is_none()
            ...
            && lumina_extensions.as_ref().is_none_or(Vec::is_empty)
        ...
        let lumina_extensions = lumina_extensions.unwrap_or_default();
    ...
    async fn resolve_provider_and_model(
        ...
        let recipe_provider = recipe_settings.and_then(|s| s.lumina_provider.clone());
        let recipe_model = recipe_settings.and_then(|s| s.lumina_model.clone());
            ...
            None => match meta_string(meta, "provider")? {
                ...
                None => {
                    if let Some(model) = recipe_model.as_deref() {
                        let provider = config.get_lumina_provider().map_err(|error| {
    ...
    async fn apply_initial_session_config(
        ...
        if let Some(project_id) = config.project_id.as_ref() {
            builder = builder.project_id(Some(project_id.clone()));
        }
        builder
            .apply()
            .await
            .internal_err_ctx("Failed to update session")?;
        match (config.project_id.as_deref(), config.work_item_id.as_deref()) {
            (Some(project_id), Some(work_item_id)) => {
                crate::project::ProjectService::new(self.session_manager.storage().clone())
                    .attach_session(project_id, work_item_id, session_id)
                    .await
                    .internal_err_ctx("Failed to attach project work item")?;
            }
            (None, Some(_)) => {
                return Err(agent_client_protocol::Error::invalid_params()
                    .data("workItemId requires projectId"));
            }
            _ => {}
        }
        Ok(())
...
fn contains_managed_injection(
    ...
    lumina_extensions: &[LuminaExtension],
    ...
    let managed_names = provenance
        ...
        .collect::<std::collections::HashSet<_>>();
    if lumina_extensions.iter().any(|extension| {
        let name = match extension {
            LuminaExtension::Builtin { name, .. } | LuminaExtension::Platform { name, .. } => name,
            LuminaExtension::Mcp { server, .. } => mcp_server_name(server),
        };
    ...
    let mut client_configs =
        super::extensions::lumina_extensions_to_configs(lumina_extensions.to_vec())?;
...
fn meta_lumina_extensions(
    meta: Option<&Meta>,
) -> Result<Option<Vec<LuminaExtension>>, agent_client_protocol::Error> {
...
mod profile_tests {
    ...
    fn direct_platform_managed_mcp_injection_is_detected() {
        ...
        let ordinary = LuminaExtension::Builtin {
        ...
        let injected = LuminaExtension::Platform {
    ...
    fn non_token_profile_errors_keep_public_semantics() {
        assert_eq!(
            profile_application_public_error(McpPlatformError::new(
                ...
                "trusted Lumina launcher or authenticated transport is required for MCP platform mutations",
            )),
            agent_client_protocol::Error::invalid_params()
                .data("profile application rejected: policy_denied")
        );
