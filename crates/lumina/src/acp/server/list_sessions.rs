use super::{build_session_info, meta_string, LuminaAcpAgent, ResultExt};
...
struct SessionListCursorToken {
    ...
    // Lumina stores timestamps with second precision in common write paths, so the
...
fn include_last_message_snippet_from_meta(
    ...
    let Some(value) = meta.and_then(|meta| meta.get("lumina")) else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    let Some(lumina_meta) = value.as_object() else {
        return Err(agent_client_protocol::Error::invalid_params().data("lumina must be an object"));
    };
    let Some(value) = lumina_meta.get("includeLastMessageSnippet") else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    value.as_bool().ok_or_else(|| {
        agent_client_protocol::Error::invalid_params()
            .data("lumina.includeLastMessageSnippet must be a boolean")
    })
}

fn include_empty_sessions_from_meta(
    meta: Option<&Meta>,
) -> Result<bool, agent_client_protocol::Error> {
    let Some(value) = meta.and_then(|meta| meta.get("lumina")) else {
        return Ok(false);
    ...
    let Some(value) = lumina_meta.get("includeEmpty") else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    value.as_bool().ok_or_else(|| {
        agent_client_protocol::Error::invalid_params().data("lumina.includeEmpty must be a boolean")
    })
}
...
fn decode_session_list_cursor(
    ...
    let expected_filter_hash =
        session_list_filter_hash(cwd, session_types, keyword, only_sessions_with_messages)?;
...
impl LuminaAcpAgent {
    pub(super) async fn on_list_sessions(
        ...
        let only_sessions_with_messages = !include_empty_sessions_from_meta(req.meta.as_ref())?;
