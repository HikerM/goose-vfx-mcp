...
impl LuminaAcpAgent {
    ...
    pub(super) async fn on_config_read(
        ...
        if req.key == "LUMINA_PROVIDER" || req.key == "active_provider" {
            let value = config
                .get_lumina_provider()
    ...
    pub(super) async fn on_config_upsert(
        ...
        if req.key == "LUMINA_MODEL" {
            if let Some(model) = req.value.as_str() {
                if let Ok(provider) = config.get_lumina_provider() {
    ...
    pub(super) async fn on_config_remove(
        ...
        } else if req.key == "LUMINA_PROVIDER" || req.key == "active_provider" {
            config.delete("active_provider").internal_err()?;
            config.delete("LUMINA_PROVIDER").internal_err()?;
        } else if req.key == "LUMINA_MODEL" {
            if let Ok(provider) = config.get_lumina_provider() {
                crate::config::set_active_provider(config, &provider, "").internal_err()?;
            }
            config.delete("LUMINA_MODEL").internal_err()?;
        } else {
    ...
    pub(super) async fn on_defaults_read(
        ...
        Ok(DefaultsReadResponse {
            provider_id: config.get_lumina_provider().ok(),
            model_id: config.get_lumina_model().ok(),
        })
    }
...
const PREFERENCE_DEFS: &[PreferenceDef] = &[
    PreferenceDef {
        ...
        config_key: "LUMINA_AUTO_COMPACT_THRESHOLD",
        prepare: prepare_auto_compact_threshold,
    },
    PreferenceDef {
        key: PreferenceKey::LuminaThinkingEffort,
        config_key: "LUMINA_THINKING_EFFORT",
...
fn prepare_thinking_effort(
    ...
    let Some(value) = value.as_str() else {
        return Err(agent_client_protocol::Error::invalid_params()
            .data("luminaThinkingEffort must be a string"));
    };
    let effort = value.parse::<ThinkingEffort>().map_err(|err| {
        agent_client_protocol::Error::invalid_params()
            .data(format!("Invalid luminaThinkingEffort: {err}"))
    })?;
