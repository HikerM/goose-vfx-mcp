...
impl LuminaAcpAgent {
    ...
    pub(super) async fn handle_fork_session(
        ...
        let (agent, extension_results) = self
            .prepare_acp_session_agent(cx, &lumina_session, None)
            .await?;
        self.apply_session_recipe(&agent, &lumina_session).await?;
        self.register_acp_session(lumina_session.id.clone(), agent, HashMap::new())
            .await;

        let acp_session_id = SessionId::new(new_session_id.clone());
        let mut meta = session_meta(&lumina_session);
        if let Ok(v) = serde_json::to_value(&extension_results) {
            meta.insert("extensionResults".to_string(), v);
        }

        let (mode_state, config_options) =
            build_session_setup_config(&self.provider_inventory, &lumina_session).await?;
        ...
        self.notify_session_setup(cx, &lumina_session).await?;
        Ok(response)
    }
}
