...
use crate::acp::server::LuminaAgentConnection;
use crate::acp::server_factory::AcpServer;

// The upstream ACP HTTP server only supports exact origin allowlists for
// WebSocket upgrades; Lumina applies its richer loopback predicate before this.
const UPSTREAM_WS_ALLOWED_ORIGIN: &str = "http://lumina.local";
...
/// `lumina serve`. This allows the `x-secret-key` auth header the proxy routes
/// rely on.
...
fn create_acp_router_inner(
    ...
    AcpHttpServer::new(move || {
        if authenticated_transport {
            LuminaAgentConnection::authenticated_http(server.clone())
        } else {
            LuminaAgentConnection::new(server.clone())
        }
    })
...
/// or lumina-specific auxiliary routes.
...
/// The full standalone ACP server router used by `lumina serve`: ACP transport,
