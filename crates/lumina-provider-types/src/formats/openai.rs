...
pub fn create_request_with_options(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since lumina uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }
    ...
    // LUMINA_MAX_TOKENS) or a canonical model record has supplied a value.
...
/// hosted aliases like `databricks-gpt-5.4`, `tenant-o3-mini`, or
/// `headless-tenant-o3-mini` work without provider-specific normalization.
...
mod tests {
    ...
    fn test_create_request_omits_max_tokens_when_unset() -> anyhow::Result<()> {
        ...
        // lmstudio) have no canonical record and no LUMINA_MAX_TOKENS, so the
...
data: [DONE]"#;
    ...
    fn test_extract_reasoning_effort_for_responses_models() {
        for (model, expected_name, expected_effort) in [
            ...
            ("tenant-gpt-5-high", "tenant-gpt-5", Some("high")),
            ("gpt-4o", "gpt-4o", None),
        ] {
