...
pub fn map_provider_name(provider: &str) -> &str {
    match provider {
        // Lumina provider names that differ from models.dev names
        "xai" => "x-ai",
        "azure_openai" => "azure",
...
mod tests {
    ...
    fn test_map_to_canonical_model() {
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "tenant-claude-sonnet-4-5-bedrock", r),
            Some("anthropic/claude-sonnet-4.5".to_string())
        );
        assert_eq!(
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "tenant-gpt-4o", r),
            Some("openai/gpt-4o".to_string())
        );
        assert_eq!(
        ...
        // === OpenAI O-series ===
        assert_eq!(
            map_to_canonical_model("databricks", "tenant-o1", r),
            Some("openai/o1".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "tenant-o3", r),
            Some("openai/o3".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "headless-tenant-o3-mini", r),
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "tenant-grok-4.3", r),
            Some("x-ai/grok-4.3".to_string())
        );
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "tenant-command-r-08-2024", r),
