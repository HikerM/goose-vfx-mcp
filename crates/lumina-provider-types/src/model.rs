...
mod tests {
    ...
    mod thinking_effort_tests {
        ...
        fn xhigh_suffix_stripped_from_model_name() {
            let _guard = env_lock::lock_env([
                ("LUMINA_THINKING_EFFORT", Some("low")),
                ("LUMINA_MAX_TOKENS", None::<&str>),
                ("LUMINA_TEMPERATURE", None::<&str>),
                ("LUMINA_CONTEXT_LIMIT", None::<&str>),
                ("LUMINA_TOOLSHIM", None::<&str>),
    ...
    mod is_openai_reasoning_model {
        ...
        const ENV_LOCK_KEYS: [(&str, Option<&str>); 5] = [
            ("LUMINA_MAX_TOKENS", None),
            ("LUMINA_TEMPERATURE", None),
            ("LUMINA_CONTEXT_LIMIT", None),
            ("LUMINA_TOOLSHIM", None),
            ("LUMINA_TOOLSHIM_OLLAMA_MODEL", None),
        ];
        ...
        #[test]
        fn tenant_prefixed_reasoning_models() {
            let _guard = env_lock::lock_env(ENV_LOCK_KEYS);
            assert!(ModelConfig::new("tenant-o3-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new("tenant-o4-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new("tenant-gpt-5").is_openai_reasoning_model());
        }
        ...
        fn non_reasoning_models() {
            ...
            assert!(!ModelConfig::new("tenant-claude-sonnet-4").is_openai_reasoning_model());
